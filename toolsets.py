...
_NEWROZ_CORE_TOOLS = [
    ...
    # read-only terminal tab (both gated on NEWROZ_DESKTOP via check_fn —
    ...
    # spawned as a kanban worker (NEWROZ_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `newroz tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (NEWROZ_KANBAN_TASK env "
    ...
    # Coding posture (base Newroz — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in newroz_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Newroz toolsets (CLI + messaging platforms)
    #
    ...
    # the `newroz send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "newroz-cli": {
        ...
        "tools": _NEWROZ_CORE_TOOLS,
        "includes": []
    },

    "newroz-cron": {
        # Mirrors newroz-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `newroz tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as newroz-cli; gated by `newroz tools`",
        "tools": _NEWROZ_CORE_TOOLS,
        "includes": []
    },
    ...
    "newroz-email": {
        "description": "Email bot toolset - interact with Newroz via email (IMAP/SMTP)",
        "tools": _NEWROZ_CORE_TOOLS,
        "includes": []
    },
    ...
    "newroz-sms": {
        "description": "SMS bot toolset - interact with Newroz via SMS (Twilio)",
        "tools": _NEWROZ_CORE_TOOLS,
        "includes": []
    },
    ...
    "newroz-webhook": {
        ...
        "tools": _NEWROZ_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "newroz-gateway": {
        ...
        "tools": [],
        "includes": ["newroz-telegram", "newroz-discord", "newroz-whatsapp", "newroz-slack", "newroz-signal", "newroz-bluebubbles", "newroz-homeassistant", "newroz-email", "newroz-sms", "newroz-mattermost", "newroz-matrix", "newroz-dingtalk", "newroz-feishu", "newroz-wecom", "newroz-wecom-callback", "newroz-weixin", "newroz-qqbot", "newroz-webhook", "newroz-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``newroz-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_NEWROZ_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``newroz-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (newroz-<name>).
        # Gives them _NEWROZ_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("newroz-"):
            platform_name = name[len("newroz-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_NEWROZ_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
