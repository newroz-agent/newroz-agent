"""CLI entry point for the newroz-agent ACP adapter.

Loads environment variables from ``~/.newroz/.env``, configures logging
...
try:
    ...
    # shadowing Newroz's own modules — ``newroz acp`` can be started from any
...
def _load_env() -> None:
    """Load .env from NEWROZ_HOME (default ``~/.newroz``)."""
    from newroz_cli.env_loader import load_newroz_dotenv

    newroz_home = get_newroz_home()
    loaded = load_newroz_dotenv(newroz_home=newroz_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", newroz_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="newroz-acp",
        description="Run Newroz Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Newroz version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
...
def _print_version() -> None:
    ...
    print(newroz_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import NewrozACPAgent  # noqa: F401

    print("Newroz ACP check OK")


def _run_setup() -> None:
    from newroz_cli.main import main as newroz_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "newroz", "model"]
        newroz_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``newroz postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting newroz-agent ACP adapter")
    ...
    import acp
    from .server import NewrozACPAgent
