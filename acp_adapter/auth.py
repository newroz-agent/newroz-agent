"""ACP auth helpers — detect and advertise Newroz authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "newroz-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Newroz runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Newroz can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Newroz.
    ...
    not have Newroz provider credentials configured yet, so Newroz always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Newroz using the currently configured "
