...
Owns one Codex thread per Newroz session. Drives `turn/start`, consumes
...
# Newroz' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_NEWROZ_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Newroz/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Newroz session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _NEWROZ_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("NEWROZ_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="newroz",
            client_title="Newroz Agent",
            client_version=_get_newroz_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Newroz' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Newroz' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own newroz-tools callback we
            # auto-accept — the user already approved Newroz' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Newroz approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Newroz returns 'once', 'session', 'always', or 'deny'.
...
def _get_newroz_version() -> str:
    """Best-effort Newroz version string for codex's userAgent line."""
    try:
