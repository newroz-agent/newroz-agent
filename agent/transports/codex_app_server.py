...
projection into Newroz' display, approval bridging, transcript projection into
...
"codex_app_server"`. Newroz' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Newroz secret — gateway
        ...
        spawn_env = newroz_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("NEWROZ_KANBAN_TASK"):
            kanban_db = spawn_env.get("NEWROZ_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "NEWROZ_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("NEWROZ_HOME", os.path.expanduser("~/.newroz")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "newroz",
        client_title: str = "Newroz Agent",
