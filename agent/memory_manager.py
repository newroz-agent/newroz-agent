...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_NEWROZ_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Newroz expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``newroz_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_newroz_home()`` themselves.
        """
        if "newroz_home" not in kwargs:
            from newroz_constants import get_newroz_home
            kwargs["newroz_home"] = str(get_newroz_home())
        for provider in self._providers:
            try:
