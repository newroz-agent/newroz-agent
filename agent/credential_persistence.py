...
``auth.json``.  They intentionally have no dependency on ``newroz_cli.auth`` so
...
# Sources Newroz owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Newroz-owned OAuth/device-code state)
