...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Newroz starts and finds ``~/.openclaw/``.

    Points users at ``newroz claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``newroz claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Newroz, run "
        "`newroz claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `newroz claw cleanup` (renames it to ~/.openclaw.pre-migration — "
