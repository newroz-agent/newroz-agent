...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the NEWROZ_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Newroz runtime / deployment
    "NEWROZ_HOME", "NEWROZ_PROFILE", "NEWROZ_GATEWAY_LOCK_DIR",
    "NEWROZ_MAX_ITERATIONS", "NEWROZ_MAX_TOKENS", "NEWROZ_API_TIMEOUT",
    "NEWROZ_REDACT_SECRETS", "NEWROZ_NOUS_TIMEOUT_SECONDS",
    "_NEWROZ_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "NEWROZ_KANBAN_DB", "NEWROZ_KANBAN_WORKSPACES_ROOT", "NEWROZ_KANBAN_BOARD",
})
_GLOBAL_ENV_PREFIXES = (
    "NEWROZ_KANBAN_",
    "NEWROZ_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def build_profile_secret_scope(newroz_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(newroz_home) / ".env")
