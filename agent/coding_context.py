"""Coding-context awareness — base Newroz, every interactive surface.

When the user runs Newroz inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), Newroz shifts into a **coding posture**. This module is the
...
logger = logging.getLogger("newroz.coding_context")
...
_EDIT_FORMAT_GUIDANCE: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    "replace": (
        ("claude", "sonnet", "opus", "haiku",
         ...
         "newroz", "llama", "mistral", "devstral", "minimax"),
        "- Edit format: author new files with `write_file`; for edits to "
...
# in _NEWROZ_CORE_TOOLS, so they're present on every surface this fires on.
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        selection (``--toolsets``, ``NEWROZ_TUI_TOOLSETS``, …); they never
...
def is_coding_context(
    *,
    ...
    """Whether Newroz should operate in its coding posture right now."""
