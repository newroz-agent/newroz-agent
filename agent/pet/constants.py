...
row taxonomy is inferred from the atlas shape so Newroz can render both legacy
...
# rows (1536x1872), while older Newroz/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Newroz' activity state names. They are not always identical to the
...
# Legacy Newroz/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Newroz activity names -> accepted row-name aliases in descending
