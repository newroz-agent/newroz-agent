...
Every Newroz request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Newroz Agent and bucket it by client release.
...
``newroz_cli.__version__`` is the canonical source of truth.
"""
...
def _newroz_version() -> str:
    """Return the current Newroz release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``newroz_cli`` cannot be imported (should
