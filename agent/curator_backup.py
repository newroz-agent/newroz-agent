...
A pre-run snapshot of ``~/.newroz/skills/`` (excluding ``.curator_backups/``
itself) is taken before any mutating curator pass. Snapshots are tar.gz
files under ``~/.newroz/skills/.curator_backups/<utc-iso>/`` with a
...
``~/.newroz/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
def _backups_dir() -> Path:
    return get_newroz_home() / "skills" / ".curator_backups"
...
def _cron_jobs_file() -> Path:
    """Source path for the live cron jobs store (``~/.newroz/cron/jobs.json``)."""
    return get_newroz_home() / "cron" / "jobs.json"
...
def snapshot_skills(reason: str = "manual", *, protect_ids: Optional[Set[str]] = None) -> Optional[Path]:
    """Create a tar.gz snapshot of ``~/.newroz/skills/`` and prune old ones.
    ...
    if not skills.exists():
        logger.debug("No ~/.newroz/skills/ directory — nothing to back up")
        return None
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    """Restore ``~/.newroz/skills/`` from a snapshot.
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.newroz/skills/``.
    ...
    if target is None:
        return (
            ...
            + " (use `newroz curator rollback --list` to see available snapshots)",
            None,
        )
