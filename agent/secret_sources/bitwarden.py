...
Newroz pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.newroz/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<newroz_home>/bin/bws`` on
  first use.  Newroz pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.newroz/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``newroz`` invocations don't
  hammer the API.
* Failures NEVER block Newroz startup.  Missing binary, no network,
...
# In-process cache so repeated load_newroz_dotenv() calls (CLI startup,
# gateway hot-reload, test suites) don't re-fetch from BSM.
_CacheKey = Tuple[str, str, str]  # (access_token_fingerprint, project_id, server_url)
_CACHE: Dict[_CacheKey, _CachedFetch] = {}

# Disk-persisted cache so back-to-back CLI invocations (e.g. `newroz chat -q ...`
...
# <newroz_home>/cache/bws_cache.json. The file holds only the secret VALUES,
# never the access token. It's plaintext-equivalent to ~/.newroz/.env (which
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the disk cache path under newroz_home/cache/.

    Thin wrapper over the shared DiskCache, kept for tests and any direct
    callers; falls back to `$NEWROZ_HOME` / `~/.newroz` when home is None.
    """
...
def _newroz_bin_dir() -> Path:
    """Where Newroz stores its managed binaries.  Profile-aware."""
    from newroz_constants import get_newroz_home

    return get_newroz_home() / "bin"
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<newroz_home>/bin/bws``  (our managed copy — preferred)
    ...
    """
    managed = _newroz_bin_dir() / _platform_binary_name()
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``newroz secrets bitwarden setup``
    surface lets them propagate so the wizard can show a clear error.
    """
    bin_dir = _newroz_bin_dir()
    ...
    with tempfile.TemporaryDirectory(prefix="newroz-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "newroz-agent"})
    try:
...
def fetch_bitwarden_secrets(
    *,
    ...
    ``<newroz_home>/cache/bws_cache.json`` (for back-to-back CLI invocations).
...
# ---------------------------------------------------------------------------
# Public entry point — called from newroz_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_newroz_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `newroz secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `newroz secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `newroz secrets bitwarden setup` to install."
        )
        return result
