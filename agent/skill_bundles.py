...
Storage
-------
Bundles live in ``~/.newroz/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``NEWROZ_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``newroz bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under NEWROZ_HOME.

    Honors ``NEWROZ_BUNDLES_DIR`` for tests; falls back to
    ``<NEWROZ_HOME>/skill-bundles``.
    """
    override = os.environ.get("NEWROZ_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_newroz_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `newroz bundles` CLI subcommand.
# ---------------------------------------------------------------------------
