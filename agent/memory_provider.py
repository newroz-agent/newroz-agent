...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `newroz backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - newroz_home (str): The active NEWROZ_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.newroz``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "newroz").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'newroz memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], newroz_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'newroz memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``newroz_home`` is the active NEWROZ_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE NEWROZ_HOME.

        ``newroz backup`` only walks NEWROZ_HOME, so any provider state kept
        ...
        of the archive, and ``newroz import`` restores them to their original
