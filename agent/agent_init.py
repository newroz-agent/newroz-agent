...
def _build_codex_gpt5_autoraise_notice(autoraise: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: newroz config set compression.codex_gpt55_autoraise false"
    )
...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$NEWROZ_HOME`` (which is profile-scoped) alongside the other
    ...
    """
    return get_newroz_home() / ".codex_gpt55_autoraise_notice"
...
def _record_codex_gpt55_autoraise_notice(autoraise: Dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$NEWROZ_HOME`` just means the notice
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .newroz.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / NEWROZ_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.newroz/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from newroz_cli.config import load_config as _load_pc_cfg
    ...
    # report cumulative micros spent.  Surfaced behind NEWROZ_DEV_CREDITS.
    ...
    # both live under ~/.newroz/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from newroz_logging import setup_logging, setup_verbose_logging
    setup_logging(newroz_home=_ra()._newroz_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that newroz_logging.setup_logging() does not
        ...
        # handler level inside newroz_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        try:
            from newroz_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `newroz model` to "
                        "select a provider, or run `newroz setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff NEWROZ_KANBAN_TASK is set).
    ...
    try:
        ...
        os.environ["NEWROZ_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.newroz/sessions/ alongside gateway sessions
    newroz_home = get_newroz_home()
    agent.logs_dir = newroz_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.newroz/sessions/session_{sid}.json)
    ...
    try:
        from newroz_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from newroz_cli.config import load_config as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "newroz"
                    except Exception:
                        pass
    ...
    # Lets an enterprise admin append to or replace Newroz' built-in
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from newroz_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from newroz_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Newroz Agent.  Choose a model with at least "
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            from newroz_cli.model_switch import _check_newroz_model_warning

            _newroz_warn = _check_newroz_model_warning(agent.model or "")
            if _newroz_warn:
                _user_msg = (
                ...
                if hasattr(agent, "_emit_warning"):
                    ...
                    print(f"\n{_user_msg}\n", file=sys.stderr)
                _ra().logger.warning(_newroz_warn)
        except Exception:
            pass
