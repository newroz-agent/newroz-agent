...
upstream prefix cache warm.  See ``newroz-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Newroz never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want NEWROZ_HOME persona while keeping
    ...
    # Pointer to the newroz-agent skill + docs for user questions about Newroz itself.
    stable_parts.append(NEWROZ_AGENT_HELP_GUIDANCE)
    ...
    # NEWROZ_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Newroz, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Newroz profile the agent is running
    # under so it doesn't conflate ~/.newroz/skills/ (default profile) with
    # ~/.newroz/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Newroz profile: default. Other profiles (if any) live "
            "under ~/.newroz/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Newroz profile: {active_profile}. This session reads "
            f"and writes ~/.newroz/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.newroz/skills/, ~/.newroz/plugins/, "
            f"~/.newroz/cron/, ~/.newroz/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Newroz never rebuilds or reinjects parts of it
