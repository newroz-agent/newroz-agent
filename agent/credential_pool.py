...
import newroz_cli.auth as auth_mod
...
def _write_through_provider_state_to_global_root(
    ...
    ``newroz_cli.auth._write_through_xai_oauth_to_global_root`` (which covers
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``newroz model`` / ``newroz auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Newroz process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        ``newroz`` invocation that defaults to the active provider
        (e.g. setup wizard, ``newroz auth status``) would land on
        ...
        try:
            with _auth_store_lock():
                ...
                # newroz_cli.auth._save_xai_oauth_tokens (#43589); the pool
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sequence below must run atomically across Newroz processes: otherwise
        ...
        if self.provider == "openai-codex":
            refresh_timeout_seconds = auth_mod.env_float(
                "NEWROZ_CODEX_REFRESH_TIMEOUT_SECONDS", 20
            )
            lock_timeout = max(
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("newroz_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # refresh_token — single-use tokens consumed by another Newroz
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("newroz_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/nous — another Newroz process
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Newroz profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `newroz model` / `newroz auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `newroz model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``newroz auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `newroz auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `newroz auth remove <provider> <N>` is stable across all source types.
    try:
        from newroz_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Newroz PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `newroz setup` ("Claude
        ...
        # Claude Code CLI, newroz_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.newroz/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `newroz setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"newroz_pkce", "claude_code"}
            ]
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_newroz_oauth_credentials

        for source_name, creds in (
            ("newroz_pkce", read_newroz_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `newroz auth add nous --label <name>`).  Fall back to the
        ...
        # Newroz auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from newroz_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.newroz/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `newroz auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `newroz auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Newroz auth store.  Without this gate the removal is instantly
        ...
        # Newroz owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``newroz model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``newroz auth list`` reflects the logged-in state and so the pool
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.newroz/.env over os.environ — the user's config file is the
    # authoritative source for Newroz credentials. Stale env vars from parent
    ...
    def _get_env_prefer_dotenv(key: str) -> str:
        ...
        # load_newroz_dotenv() -> apply_onepassword_secrets()).  The raw
    ...
    # Honour user suppression — `newroz auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.newroz/.env.
    ...
    try:
        from newroz_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from newroz_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.newroz/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `newroz auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and Newroz
