...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Newroz with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.newroz/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Newroz
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``newroz tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``newroz setup`` for
