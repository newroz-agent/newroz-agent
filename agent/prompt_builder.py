...
from newroz_constants import get_newroz_home, get_skills_dir, is_wsl
...
_NEWROZ_MD_NAMES = (".newroz.md", "NEWROZ.md")


def _find_newroz_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.newroz.md`` or ``NEWROZ.md``.
    ...
    # could pick up a .newroz.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _NEWROZ_MD_NAMES:
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.newroz/kanban.db`. Your task id is in "
    "`$NEWROZ_KANBAN_TASK`; your workspace is `$NEWROZ_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $NEWROZ_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $NEWROZ_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${NEWROZ_KANBAN_BRANCH:-wt/$NEWROZ_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$NEWROZ_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`newroz profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `newroz kanban <verb>` for board operations. Use "
...
#
# The newroz-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to newroz-agent's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "run `newroz computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Newroz "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Newroz terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "webui": (
        "You are in the Newroz WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Newroz itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Newroz runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Newroz itself is running. The host OS, home, and cwd "
                f"of the Newroz process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Newroz "
    ...
    # Newroz desktop GUI — any agent running under the desktop app should know
    # it. NEWROZ_DESKTOP marks the backend powering the chat; NEWROZ_DESKTOP_TERMINAL
    # marks a newroz launched in the embedded terminal pane. Both set by main.cjs.
    _truthy = ("1", "true", "yes")
    _in_desktop = (os.getenv("NEWROZ_DESKTOP") or "").strip().lower() in _truthy
    _in_desktop_term = (os.getenv("NEWROZ_DESKTOP_TERMINAL") or "").strip().lower() in _truthy
    if _in_desktop or _in_desktop_term:
        _desktop_hint = "Runtime surface: you're running inside the Newroz desktop GUI app."
        if _in_desktop_term:
            _desktop_hint += (
    ...
    # Embedder-supplied environment description. Lets a host that wraps Newroz
    ...
    extra = (os.getenv("NEWROZ_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_newroz_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.newroz/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("NEWROZ_PLATFORM")
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Newroz Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `newroz-agent` skill "
            "first. It has the actual commands (e.g. `newroz config set …`, `newroz tools`, "
            "`newroz setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: newroz setup, newroz setup tools, newroz setup terminal, newroz status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from NEWROZ_HOME and return its content, or None.
    ...
    """
    try:
        from newroz_cli.config import ensure_newroz_home
        ensure_newroz_home()
    except Exception as e:
        logger.debug("Could not ensure NEWROZ_HOME before loading SOUL.md: %s", e)

    soul_path = get_newroz_home() / "SOUL.md"
...
def _load_newroz_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.newroz.md / NEWROZ.md — walk to git root."""
    newroz_md_path = _find_newroz_md(cwd_path)
    if not newroz_md_path:
        return ""
    try:
        content = newroz_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = newroz_md_path.name
        try:
            rel = str(newroz_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".newroz.md", context_length=context_length,
            read_path=str(newroz_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", newroz_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .newroz.md / NEWROZ.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from NEWROZ_HOME is independent and always included when present.
    ...
    project_context = (
        _load_newroz_md(cwd_path, context_length)
    ...
    # SOUL.md from NEWROZ_HOME only — skip when already loaded as identity
    if not skip_soul:
