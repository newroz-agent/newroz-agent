...
as ``kind: backend``) or ``~/.newroz/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``newroz tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$NEWROZ_HOME/cache/videos/``, creating parents as needed."""
    from newroz_constants import get_newroz_home

    path = get_newroz_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$NEWROZ_HOME/cache/videos/``.
