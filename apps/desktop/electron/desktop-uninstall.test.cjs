...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'newroz_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'newroz_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'newroz_cli.uninstall', '--mode', 'full'])
})
...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/Newroz.app/Contents/MacOS/Newroz', 'darwin'),
    '/Applications/Newroz.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/Newroz.app/Contents/MacOS/Newroz', 'darwin'),
    '/Users/x/Applications/Newroz.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\Newroz\\Newroz.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\Newroz'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\newroz-desktop\\Newroz.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\newroz-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\Newroz.exe', 'win32'), null)
})

test('resolveRemovableAppPath uses APPIMAGE on Linux when set', () => {
  assert.equal(
    resolveRemovableAppPath('/tmp/.mount_NewrozXXXX/newroz', 'linux', { APPIMAGE: '/home/x/Apps/Newroz.AppImage' }),
    '/home/x/Apps/Newroz.AppImage'
  )
})

test('resolveRemovableAppPath finds the unpacked dir on Linux', () => {
  assert.equal(resolveRemovableAppPath('/opt/newroz/linux-unpacked/newroz', 'linux', {}), '/opt/newroz/linux-unpacked')
  // A system-package install (/usr/bin) → null, left to apt/dnf.
  assert.equal(resolveRemovableAppPath('/usr/bin/newroz', 'linux', {}), null)
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/Newroz.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/Newroz.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.newroz/newroz-agent/venv/bin/python',
    pythonPath: null,
    agentRoot: '/home/x/.newroz/newroz-agent',
    uninstallArgs: ['-m', 'newroz_cli.uninstall', '--mode', 'gui'],
    appPath: '/opt/newroz/linux-unpacked',
    newrozHome: '/home/x/.newroz'
  })
  ...
  assert.match(script, /'-m' 'newroz_cli\.uninstall' '--mode' 'gui'/)
  assert.match(script, /rm -rf '\/opt\/newroz\/linux-unpacked'/)
  assert.match(script, /export NEWROZ_HOME='\/home\/x\/\.newroz'/)
})
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.newroz/newroz-agent',
    agentRoot: '/home/x/.newroz/newroz-agent',
    uninstallArgs: ['-m', 'newroz_cli.uninstall', '--mode', 'full'],
    appPath: null,
    newrozHome: '/home/x/.newroz'
  })
  // System python + source on PYTHONPATH so import newroz_cli works while the
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.newroz\/newroz-agent'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'newroz_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: null,
    agentRoot: '/a',
    uninstallArgs: ['-m', 'newroz_cli.uninstall', '--mode', 'lite'],
    appPath: null,
    newrozHome: '/h'
  })
  assert.doesNotMatch(script, /rm -rf '\//)
  // Still runs the uninstall.
  assert.match(script, /'-m' 'newroz_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    agentRoot: 'C:\\newroz',
    uninstallArgs: ['-m', 'newroz_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\Newroz',
    newrozHome: 'C:\\Users\\x\\AppData\\Local\\newroz'
  })
  assert.match(script, /@echo off/)
  assert.match(script, /set "PID=9988"/)
  // PYTHONPATH set so a system python can import newroz_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\newroz;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "newroz_cli\.uninstall" "--mode" "full"/)
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\Newroz" >nul 2>&1/)
