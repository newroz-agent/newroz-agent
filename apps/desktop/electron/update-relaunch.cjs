 ...
 * Background
 * ----------
 * After `newroz update` + `newroz desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything NEWROZ_DESKTOP_* is preserved
// plus NEWROZ_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['NEWROZ_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['NEWROZ_DESKTOP_']
