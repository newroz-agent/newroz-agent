...
const { canImportNewrozCli, verifyNewrozCli } = require('./backend-probes.cjs')
const {
...
const { buildDesktopBackendEnv, normalizeNewrozHomeRoot } = require('./backend-env.cjs')
...
const USER_DATA_OVERRIDE = process.env.NEWROZ_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.NEWROZ_DESKTOP_DEV_SERVER
...
// switches only apply pre-launch. Override with NEWROZ_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[newroz] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[newroz] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}

ipcMain.handle('newroz:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[newroz] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
          continue
        }
...
if (INSTALL_STAMP) {
  console.log(
    `[newroz] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[newroz] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// NEWROZ_HOME — the user-facing root for everything Newroz-related. Mirrors
// scripts/install.ps1's $NewrozHome and scripts/install.sh's $NEWROZ_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\newroz (matches install.ps1)
//   macOS / Linux: ~/.newroz (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.newroz directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\newroz yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// NEWROZ_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// NEWROZ_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.newroz / %LOCALAPPDATA%\newroz.
function resolveNewrozHome() {
  if (process.env.NEWROZ_HOME) return normalizeNewrozHomeRoot(process.env.NEWROZ_HOME)
  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'newroz-home')
  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a NEWROZ_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\newroz and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('NEWROZ_HOME')
    if (fromRegistry) return normalizeNewrozHomeRoot(fromRegistry)
  }
  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'newroz')
    const legacy = path.join(app.getPath('home'), '.newroz')
    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.newroz setup (no LOCALAPPDATA install yet) so users don't lose state.
    if (!directoryExists(localappdata) && directoryExists(legacy)) return legacy
    return localappdata
  }
  return path.join(app.getPath('home'), '.newroz')
}

const NEWROZ_HOME = resolveNewrozHome()

function newrozManagedNodePathEntries() {
  // NOTE: keep this ordering in sync with iter_newroz_node_dirs() in
  // newroz_constants.py — this Node main process cannot import the Python
  // module, so the platform-ordering rule is mirrored here.
  const root = path.join(NEWROZ_HOME, 'node')
  const bin = path.join(root, 'bin')
  const entries = IS_WINDOWS ? [root, bin] : [bin, root]
  return entries.filter(directoryExists)
}

function pathWithNewrozManagedNode(...entries) {
  return [...newrozManagedNodePathEntries(), ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_NEWROZ_ROOT — the canonical mutable Newroz install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_NEWROZ_ROOT = path.join(NEWROZ_HOME, 'newroz-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_NEWROZ_ROOT, 'venv')
...
// boots, going straight to `resolveNewrozBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_NEWROZ_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_NEWROZ_ROOT, '.newroz-bootstrap-complete')
const BOOTSTRAP_MARKER_SCHEMA_VERSION = 1

const DESKTOP_CONNECTION_CONFIG_PATH = path.join(app.getPath('userData'), 'connection.json')
const DESKTOP_UPDATE_CONFIG_PATH = path.join(app.getPath('userData'), 'updates.json')
const DESKTOP_WINDOW_STATE_PATH = path.join(app.getPath('userData'), 'window-state.json')
// active-profile.json records which Newroz profile the desktop launches its
// local backend as. When set, startNewroz() passes `newroz --profile <name>
// dashboard …`, which deterministically pins NEWROZ_HOME (see
// _apply_profile_override in newroz_cli/main.py) and bypasses the sticky
// ~/.newroz/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors newroz_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// newrozDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under NEWROZ_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by newroz_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(NEWROZ_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (newroz_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.NEWROZ_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.NEWROZ_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
  if (!Number.isFinite(raw) || raw <= 0) return 650
  return Math.max(120, raw)
})()
const APP_NAME = 'Newroz'
...
// 'newroz:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.nousresearch.newroz) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nousresearch.newroz')
}
// Seed the native About panel with the live Newroz version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `newroz update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveNewrozVersion(),
...
const MEDIA_PROTOCOL = 'newroz-media'
// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(newroz-media://…)`.
...
let mainWindow = null
let newrozProcess = null
let connectionPromise = null
// Additional per-profile backends, keyed by profile name. The PRIMARY backend
// (the desktop's launch profile) stays managed by newrozProcess +
// connectionPromise + startNewroz(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.NEWROZ_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.NEWROZ_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startNewroz() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning newroz serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Newroz backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  if (!text) return
  const lines = text.split(/\r?\n/).map(line => `[newroz] ${line}`)
  newrozLog.push(...lines)
  if (newrozLog.length > 300) {
    newrozLog.splice(0, newrozLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-newroz-wsl-windows-fonts.conf')
    let existing = ''
    try {
...
function broadcastBootProgress() {
  ...
  webContents.send('newroz:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the newroz:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('newroz:bootstrap:event', ev)
}
...
async function waitForUpdateToFinish() {
  let marker = readLiveUpdateMarker(NEWROZ_HOME)
  ...
  while (marker && Date.now() < deadline) {
    await advanceBootProgress(
      'backend.update-wait',
      'An update is finishing — Newroz will start automatically when it completes…',
      12
    )
    await new Promise(r => setTimeout(r, UPDATE_WAIT_POLL_MS))
    marker = readLiveUpdateMarker(NEWROZ_HOME)
  }
  if (marker) {
...
function findOnPath(command) {
  ...
  // shell-script shim named `newroz` — must not shadow `newroz.cmd`/`newroz.exe`.
...
function unwrapWindowsVenvNewrozCommand(command, backendArgs) {
  if (!IS_WINDOWS || !command || isCommandScript(command)) return null

  const resolved = path.resolve(String(command))
  if (!/^newroz(?:\.exe)?$/i.test(path.basename(resolved))) return null
  ...
  // died mid-`pip install` still has python.exe + newroz.exe on disk, but the
  ...
  if (
    ...
    rememberLog(
      `Ignoring venv Newroz at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
    return null
  }

  return {
    label: `existing Newroz Python at ${python}`,
    command: python,
    args: ['-m', 'newroz_cli.main', ...backendArgs],
    bootstrap: false,
    env: buildDesktopBackendEnv({
      newrozHome: NEWROZ_HOME,
...
// spawns `newroz serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `newroz` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'newroz_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      execFileSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, NEWROZ_HOME, ...(backend.env || {}) },
        timeout: 15000,
        stdio: 'ignore',
        windowsHide: true
      })
...
function isNewrozSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'newroz_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.NEWROZ_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Newroz deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Newroz' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\newroz\git\... — checked
  // first so users who installed via install.ps1 are detected before we
  // start probing system-wide locations.
  const localAppData = process.env.LOCALAPPDATA || ''
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'newroz', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'newroz', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// normal NEWROZ_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\newroz\git (never on
...
function resolveGitBinary() {
  ...
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'newroz', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'newroz', 'git', 'bin', 'git.exe'))
  }
...
function recentNewrozLog() {
  return newrozLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's newroz root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_NEWROZ_ROOT.
// NEWROZ_DESKTOP_NEWROZ_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.NEWROZ_DESKTOP_NEWROZ_ROOT && path.resolve(process.env.NEWROZ_DESKTOP_NEWROZ_ROOT),
    !IS_PACKAGED && isNewrozSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isNewrozSourceRoot(ACTIVE_NEWROZ_ROOT) ? ACTIVE_NEWROZ_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_NEWROZ_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('newroz:updates:progress', merged)
  }
}
...
// NEWROZ_HOME/newroz-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_newroz_home). That binary owns
// ALL repo mutation — running `newroz update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'newroz-setup.exe' : 'newroz-setup'
  const candidate = path.join(NEWROZ_HOME, name)
...
// Path to the venv shim whose lock decides whether `newroz update` can write
// fresh entry points. On Windows this is the file the running backend
// `newroz.exe` holds open; on POSIX it's never mandatory-locked.
function venvNewrozShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'newroz.exe')
    : path.join(updateRoot, 'venv', 'bin', 'newroz')
}
...
// only signals the direct child, so on Windows a backend `newroz.exe` that
// spawned its own grandchildren (a `newroz` REPL, a pty terminal session, the
...
// `newrozProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `newroz.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a newroz
// REPL, a pty terminal, the gateway) keep `newroz.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  if (newrozProcess && Number.isInteger(newrozProcess.pid)) pids.push(newrozProcess.pid)
  ...
  if (newrozProcess && !newrozProcess.killed) {
    try {
      newrozProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvNewrozShimPath(updateRoot)
  ...
  while (Date.now() < deadlineMs) {
    ...
    const stragglers = []
    if (newrozProcess && Number.isInteger(newrozProcess.pid)) stragglers.push(newrozProcess.pid)
    for (const entry of backendPool.values()) {
...
// `newroz update`). Instead we spawn the staged Newroz-Setup binary with
// --update and quit, so it can run `newroz update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri newroz-setup. Unlike Windows
      ...
      // whole update itself: `newroz update` (backend) + `newroz desktop
    ...
    if (!updater) {
      ...
      // `newroz desktop`, never the Tauri installer that self-copies
      // newroz-setup.exe into NEWROZ_HOME). They DO have a working `newroz`
      ...
      // checkout they're on — bare `newroz update` defaults to main and would
      ...
      const updateRoot = resolveUpdateRoot()
      let command = 'newroz update'
      try {
        const head = await runGit(['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: updateRoot })
        const current = (head.stdout || '').trim()
        if (head.code === 0 && current && current !== 'HEAD') {
          const branch = await resolveHealedBranch(updateRoot, current)
          if (branch !== 'main') command = `newroz update --branch ${branch}`
        }
      } catch {
        // Best-effort: fall back to bare `newroz update` if branch detection fails.
      }
      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })
      return { ok: true, manual: true, command, newrozRoot: updateRoot }
    }

    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Newroz — this window will close and the updater will open. Don’t reopen Newroz yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    // newroz.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running newroz, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Newroz install open ' +
        '(a second Newroz window or a terminal running newroz?). Close it and retry.'
    ...
    // `newroz update` will run (the venv shim is locked while we live).
    const child = spawn(updater, updaterArgs, {
      cwd: NEWROZ_HOME,
      env: {
        ...process.env,
        NEWROZ_HOME,
        PATH: pathWithNewrozManagedNode(venvBin)
      },
      detached: true,
      stdio: 'ignore',
      windowsHide: false
    ...
    if (Number.isInteger(child.pid)) {
      writeUpdateMarker(NEWROZ_HOME, child.pid)
    }
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvNewroz = path.join(venvBin, IS_WINDOWS ? 'newroz.exe' : 'newroz')
  ...
  // not just the `newroz.exe` console-script shim. That shim is generated at the
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvNewroz) || fileExists(path.join(updateRoot, '.newroz-bootstrap-complete'))
...
// Resolve the newroz CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `newroz` on PATH.
function resolveNewrozCliBinary(updateRoot) {
  const venvNewroz = path.join(updateRoot, 'venv', 'bin', 'newroz')
  if (fileExists(venvNewroz)) return venvNewroz
  return findOnPath('newroz') || null
}
...
// macOS/Linux in-app update: backend (`newroz update`) + OS-aware GUI rebuild
// (`newroz desktop --build-only`), then atomically swap the running .app bundle
// with the freshly built one and relaunch. Degrades to "backend updated,
// restart to load the new GUI" if the swap can't be performed.
async function applyUpdatesPosixInApp() {
  const updateRoot = resolveUpdateRoot()
  const newroz = resolveNewrozCliBinary(updateRoot)
  if (!newroz) {
    emitUpdateProgress({ stage: 'manual', message: 'newroz update', percent: null })
    return { ok: true, manual: true, command: 'newroz update', newrozRoot: updateRoot }
  }

  // Put the Newroz-managed Node and the venv on PATH so `newroz desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\newroz\node, not node\bin.
  const env = {
    NEWROZ_HOME,
    PATH: pathWithNewrozManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `newroz update` reaps stale `newroz serve` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads NEWROZ_DESKTOP_CHILD_PID and excludes
  ...
  if (newrozProcess && Number.isInteger(newrozProcess.pid)) {
    desktopChildPids.push(newrozProcess.pid)
  }
  for (const entry of backendPool.values()) {
  ...
  if (desktopChildPids.length) {
    env.NEWROZ_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Newroz (git + dependencies)…', percent: 10 })
  const updated = await runStreamedUpdate(newroz, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'newroz update failed.', error: updated.error || 'update-failed' })
    return { ok: false, error: 'newroz update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(newroz, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Newroz to retry.',
      error: rebuilt.error || 'rebuild-failed'
    })
    return { ok: false, backendUpdated: true, error: 'desktop rebuild failed' }
  }

  // Linux in-app update terminal state (#45205). `newroz desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting Newroz…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `newroz-desktop-update-${Date.now()}.sh`)
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen Newroz to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the Newroz desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen Newroz to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Newroz.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Newroz.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Newroz to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.newroz-update-new"; then
    rm -rf "$DST.newroz-update-old" 2>/dev/null || true
    mv "$DST" "$DST.newroz-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.newroz-update-new" "$DST"
    rm -rf "$DST.newroz-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Newroz to load the new version.',
      percent: 100
    })
...
// Marker-independent: is the canonical install at ACTIVE_NEWROZ_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isNewrozSourceRoot(ACTIVE_NEWROZ_ROOT) &&
    fileExists(venvPython) &&
    canImportNewrozCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_NEWROZ_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `newroz
...
function resolveWebDist() {
  const override = process.env.NEWROZ_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set NEWROZ_DESKTOP_WEB_DIST.`
    )
  }
  return fallback
}
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: newroz desktop --force-build`
  )
  return candidates[0]
}
...
function resolveNewrozCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Newroz.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.NEWROZ_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveNewrozCwd(), sanitized: Boolean(trimmed) }
  }
...
// createActiveBackend — build a backend pointing at ACTIVE_NEWROZ_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Newroz at ${ACTIVE_NEWROZ_ROOT}`,
    command,
    args: ['-m', 'newroz_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      newrozHome: NEWROZ_HOME,
      pythonPathEntries: [ACTIVE_NEWROZ_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_NEWROZ_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveNewrozBackend(backendArgs) {
  // 1. Explicit override -- NEWROZ_DESKTOP_NEWROZ_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.NEWROZ_DESKTOP_NEWROZ_ROOT && path.resolve(process.env.NEWROZ_DESKTOP_NEWROZ_ROOT)
  if (overrideRoot && isNewrozSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Newroz source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `newroz` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isNewrozSourceRoot.)
  if (!IS_PACKAGED && isNewrozSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Newroz source at ${SOURCE_REPO_ROOT}`, backendArgs)
    if (backend) return backend
  }

  // 3. Bootstrap-complete ACTIVE_NEWROZ_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\newroz\newroz-agent (Windows) or ~/.newroz/newroz-agent.
  ...
  //    to spawning newroz. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `newroz update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(backendArgs)
  }

  // 4. Existing `newroz` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    NEWROZ_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.NEWROZ_DESKTOP_IGNORE_EXISTING !== '1') {
    let newrozCommand = null
    const newrozOverride = process.env.NEWROZ_DESKTOP_NEWROZ

    if (newrozOverride) {
      const resolvedOverride = findOnPath(newrozOverride)
      if (resolvedOverride) {
        newrozCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(newrozOverride, { isWsl: IS_WSL })) {
        newrozCommand = newrozOverride
      } else {
        rememberLog(`Ignoring Windows Newroz override under WSL: ${newrozOverride}`)
      }
    } else {
      newrozCommand = findOnPath('newroz')
    }

    if (newrozCommand) {
      if (looksLikeDesktopAppBinary(newrozCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Newroz CLI: ${newrozCommand}`)
        newrozCommand = null
      }
    }

    if (newrozCommand) {
      const unwrapped = unwrapWindowsVenvNewrozCommand(newrozCommand, backendArgs)
      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `newroz` shim
      ...
      const shellForProbe = isCommandScript(newrozCommand)
      if (verifyNewrozCli(newrozCommand, { shell: shellForProbe })) {
        return (
          unwrapWindowsVenvNewrozCommand(newrozCommand, backendArgs) || {
            label: `existing Newroz CLI at ${newrozCommand}`,
            command: newrozCommand,
            args: backendArgs,
            bootstrap: false,
            env: {},
            kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Newroz CLI at ${newrozCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed newroz_cli module via system Python.
  ...
  if (python) {
    ...
    // having newroz_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\newroz\newroz-agent\venv.
    if (canImportNewrozCli(python)) {
      return {
        kind: 'python',
        label: `installed newroz_cli module via ${python}`,
        command: python,
        args: ['-m', 'newroz_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: newroz_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveNewrozBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Newroz Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveNewrozBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Newroz install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError = new Error(
        'Newroz recovery was handed off to Newroz Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      newrozHome: NEWROZ_HOME,
      logRoot: path.join(NEWROZ_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Newroz install was cancelled.')
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Newroz bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(NEWROZ_HOME, 'logs', 'desktop.log')} for the full transcript.`
      )
      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startNewroz() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // newroz:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveNewrozBackend(backend.args))
  }
  ...
  if (!isNewrozSourceRoot(ACTIVE_NEWROZ_ROOT)) {
    throw new Error(
      `Newroz install at ${ACTIVE_NEWROZ_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Newroz' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\newroz\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `newroz` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Newroz on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch Newroz.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // isNewrozSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `Newroz venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Newroz at ${ACTIVE_NEWROZ_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Newroz runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Newroz backend URL protocol: ${parsed.protocol}`))
      return
    }
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Newroz backend.'
              )
            )
            return
          }
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Newroz backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options = {}) {
  ...
  // NO ``X-Newroz-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('newroz:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveNewrozCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('newroz:preview-file-changed', payload)
}
...
async function waitForNewroz(baseUrl, token) {
  ...
  throw new Error(`Newroz backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function sendBackendExit(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('newroz:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('newroz:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Newroz...".
function sendPowerResume() {
  if (!mainWindow || mainWindow.isDestroyed()) return
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('newroz:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('newroz:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen) {
  ...
  webContents.send('newroz:window-state-changed', state)
}
...
const ZOOM_STORAGE_KEY = 'newroz:desktop:zoomLevel'
...
//
// Hosted Newroz gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``newroz_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / newroz #37247).
//     Both are set as HttpOnly cookies (``newroz_session_at`` ~15 min,
//     ``newroz_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:newroz-remote-oauth'
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Newroz gateway',
...
function fetchJsonViaOauthSession(url, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Newroz backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// a valid stored value (pins the root NEWROZ_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.NEWROZ_DESKTOP_REMOTE_URL)

  const remoteToken = decryptDesktopSecret(block.token)
  const authMode = normAuthMode(block.authMode)
  const remoteUrl = envOverride ? String(process.env.NEWROZ_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    // scope is never overridden by NEWROZ_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (newroz #37247), and the
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote Newroz gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote Newroz gateway is selected, but no session token is saved. ' +
...
//   2. env override (NEWROZ_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.NEWROZ_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.NEWROZ_DESKTOP_REMOTE_TOKEN
  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'NEWROZ_DESKTOP_REMOTE_URL is set but NEWROZ_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Newroz backend.'
      )
    }
...
function globalRemoteActive() {
  if (process.env.NEWROZ_DESKTOP_REMOTE_URL) {
    return true
  }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Newroz
...
async function testDesktopConnectionConfig(input = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startNewroz())
  ...
  // connect to Newroz gateway". Mirror the renderer's connect here so the test
...
function resetNewrozConnection() {
  connectionPromise = null
  backendStartFailure = null

  stopBackendChild(newrozProcess)
...
// startNewroz() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait() {
  // Capture the reference before resetNewrozConnection() nulls newrozProcess.
  const dying = newrozProcess && !newrozProcess.killed ? newrozProcess : null
  resetNewrozConnection()
...
// profile to startNewroz() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startNewroz() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForNewroz(remote.baseUrl, remote.token)
    return {
      ...remote,
      profile,
      logs: newrozLog.slice(-80),
      ...getWindowState()
    }
  }

  const token = crypto.randomBytes(32).toString('base64url')
  // --profile wins over the inherited NEWROZ_HOME env (see _apply_profile_override
  // step 3 in newroz_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveNewrozBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const newrozCwd = resolveNewrozCwd()
  const webDist = resolveWebDist()
  const readyFile = backend.readyFile ? makeDashboardReadyFile() : null

  rememberLog(`Starting Newroz backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: newrozCwd,
        NEWROZ_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        NEWROZ_DESKTOP: '1',
        NEWROZ_WEB_DIST: webDist,
        ...(readyFile ? { NEWROZ_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Newroz backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Newroz backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)
    if (!ready) {
      rejectStart?.(
        new Error(`Newroz backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForNewroz(baseUrl, token), startFailed])
  ready = true
  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Newroz backend for profile "${profile}"`,
    rememberLog
  })
...
// backend whose ensure_newroz_home() recreates the deleted profile directory.
...
async function startNewroz() {
  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startNewroz() call re-throws the same error
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving Newroz backend', 8)
    // Resolve for the desktop's primary profile so a per-profile remote
    // override on the active profile is honored (falls back to env / global).
    const remote = await resolveRemoteBackend(primaryProfileKey())
    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote Newroz backend at ${remote.baseUrl}`, 24)
      await waitForNewroz(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Newroz backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    // deterministic (it wins over the sticky ~/.newroz/active_profile file) and
    // resolves NEWROZ_HOME the same way `newroz -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Newroz runtime', 28)
    const backend = await ensureRuntime(resolveNewrozBackend(backendArgs))
    // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
    backend.args = getBackendArgsForRuntime(backend)
    const newrozCwd = resolveNewrozCwd()
    ...
    await advanceBootProgress('backend.spawn', `Starting Newroz backend via ${backend.label}`, 84)
    rememberLog(`Starting Newroz backend via ${backend.label}`)
    ...
    newrozProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin NEWROZ_HOME for the child so Python's get_newroz_home()
          // resolves to the SAME location our resolveNewrozHome() picked. Without
          // this pin, Python falls back to ~/.newroz on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\newroz, which differs from C:\Users\<u>\.newroz.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets NEWROZ_HOME via setx; the desktop
    ...
    newrozProcess.stdout.on('data', rememberLog)
    newrozProcess.stderr.on('data', rememberLog)
    let backendReady = false
    let rejectBackendStart = null
    const backendStartFailed = new Promise((_resolve, reject) => {
      rejectBackendStart = reject
    })
    newrozProcess.once('error', error => {
      rememberLog(`Newroz backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Newroz backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    newrozProcess.once('exit', (code, signal) => {
      rememberLog(`Newroz backend exited (${signal || code})`)
      newrozProcess = null
      connectionPromise = null
      sendBackendExit({ code, signal })
      if (!backendReady) {
        const message = `Newroz backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Newroz backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentNewrozLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Newroz backend to launch', 86)
    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(newrozProcess, { readyFile }),
      backendStartFailed
    ])
    if (readyFile) {
      fs.unlink(readyFile, () => {})
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Newroz backend to become ready', 90)
    await Promise.race([waitForNewroz(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null
    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      // The exit/error handlers null newrozProcess when the child dies.
      childAlive: () => newrozProcess !== null && newrozProcess.exitCode === null && !newrozProcess.killed,
      rememberLog
    })
    updateBootProgress({
      phase: 'backend.ready',
      message: 'Newroz backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Newroz is
...
// pushes pet state over IPC (newroz:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via newroz:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Newroz app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Newroz icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see newroz:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Newroz in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('newroz:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
function createWindow() {
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    ...
    startNewroz().catch(error => rememberLog(error.stack || error.message))
  })
}

ipcMain.handle('newroz:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting Newroz…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('newroz:connection:revalidate', async () => {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetNewrozConnection only
    // nulls connectionPromise for a remote (no child to SIGTERM).
    rememberLog('Cached remote Newroz backend failed liveness probe; dropping stale connection.')
    resetNewrozConnection()
    return { ok: true, rebuilt: true }
  }
})
ipcMain.handle('newroz:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)
  return { ok: true }
})
ipcMain.handle('newroz:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
ipcMain.handle('newroz:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('newroz:window:openNewSession', async () => {
...
ipcMain.handle('newroz:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('newroz:pet-overlay:close', async () => {
  closePetOverlay()
...
ipcMain.on('newroz:pet-overlay:set-bounds', (_event, bounds) => {
...
ipcMain.on('newroz:pet-overlay:ignore-mouse', (_event, ignore) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
...
ipcMain.on('newroz:pet-overlay:set-focusable', (_event, focusable) => {
  if (!petOverlayWindow || petOverlayWindow.isDestroyed()) {
    return
  }
...
ipcMain.on('newroz:pet-overlay:state', (_event, payload) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
    petOverlayWindow.webContents.send('newroz:pet-overlay:state', payload)
  }
})
// Overlay → main renderer: control messages (pop back in, composer submit).
ipcMain.on('newroz:pet-overlay:control', (_event, payload) => {
  if (!mainWindow || mainWindow.isDestroyed()) {
    return
  }
  ...
  mainWindow.webContents.send('newroz:pet-overlay:control', payload)
})
ipcMain.handle('newroz:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startNewroz() call restarts the
...
ipcMain.handle('newroz:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startNewroz() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('newroz:bootstrap:cancel', async () => {
...
ipcMain.handle('newroz:boot-progress:get', async () => bootProgressState)
ipcMain.handle('newroz:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('newroz:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('newroz:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('newroz:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('newroz:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('newroz:connection-config:oauth-logout', async (_event, rawUrl) => {
...
ipcMain.handle('newroz:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('newroz:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('newroz:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('newroz:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new NEWROZ_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('newroz:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('newroz:requestMicrophoneAccess', async () => {
...
ipcMain.handle('newroz:api', async (_event, request) => {
  ...
  // backend calls ensure_newroz_home() which recreates the profile directory,
...
ipcMain.handle('newroz:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('newroz:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('newroz:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
  return true
...
ipcMain.handle('newroz:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('newroz:readFileText', async (_event, filePath) => {
...
ipcMain.handle('newroz:selectPaths', async (_event, options = {}) => {
...
ipcMain.handle('newroz:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))
  return true
})

ipcMain.handle('newroz:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('newroz:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('newroz:saveClipboardImage', async () => {
...
ipcMain.handle('newroz:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('newroz:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('newroz:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('newroz:titlebar-theme', (_event, payload) => {
...
ipcMain.on('newroz:native-theme', (_event, mode) => {
...
ipcMain.on('newroz:translucency', (_event, payload) => {
...
ipcMain.handle('newroz:openExternal', (_event, url) => {
  if (!openExternalUrl(url)) {
    throw new Error('Invalid external URL')
  }
})

ipcMain.handle('newroz:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveNewrozCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('newroz:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveNewrozCwd()
}))

ipcMain.handle('newroz:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('newroz:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('newroz:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('newroz:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('newroz:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('newroz:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: newrozLog.slice(-200) }))
...
function terminalShellCommand() {
  // NEWROZ_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.NEWROZ_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Newroz' TUI into
  ...
  env.TERM_PROGRAM = 'Newroz'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a newroz/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from NEWROZ_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.NEWROZ_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `newroz:terminal:${id}:${suffix}`
}
...
ipcMain.handle('newroz:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('newroz:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

// Reveal a path in the OS file manager (Finder / Explorer / Files).
ipcMain.handle('newroz:fs:reveal', async (_event, targetPath) => {
...
ipcMain.handle('newroz:fs:rename', async (_event, targetPath, newName) => {
...
ipcMain.handle('newroz:fs:writeText', async (_event, filePath, content) => {
...
ipcMain.handle('newroz:fs:trash', async (_event, targetPath) => {
  const target = String(targetPath || '').trim()
...
ipcMain.handle('newroz:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

ipcMain.handle('newroz:git:worktreeAdd', async (_event, repoPath, options) =>
  addWorktree(repoPath, options || {}, resolveGitBinary())
)

ipcMain.handle('newroz:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
  removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
)

ipcMain.handle('newroz:git:branchSwitch', async (_event, repoPath, branch) =>
  switchBranch(repoPath, branch, resolveGitBinary())
)

ipcMain.handle('newroz:git:branchList', async (_event, repoPath) => listBranches(repoPath, resolveGitBinary()))
...
ipcMain.handle('newroz:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
...
ipcMain.handle('newroz:git:review:list', async (_event, repoPath, scope, baseRef) =>
  reviewList(repoPath, scope, baseRef, resolveGitBinary())
)
ipcMain.handle('newroz:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
...
ipcMain.handle('newroz:git:fileDiff', async (_event, repoPath, filePath) =>
  fileDiffVsHead(repoPath, filePath, resolveGitBinary())
)
ipcMain.handle('newroz:git:review:stage', async (_event, repoPath, filePath) =>
  reviewStage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('newroz:git:review:unstage', async (_event, repoPath, filePath) =>
  reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('newroz:git:review:revert', async (_event, repoPath, filePath) =>
  reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('newroz:git:review:revParse', async (_event, repoPath, ref) =>
  reviewRevParse(repoPath, ref, resolveGitBinary())
)
ipcMain.handle('newroz:git:review:commit', async (_event, repoPath, message, push) =>
  reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
)
ipcMain.handle('newroz:git:review:commitContext', async (_event, repoPath) =>
  reviewCommitContext(repoPath, resolveGitBinary())
)
ipcMain.handle('newroz:git:review:push', async (_event, repoPath) => reviewPush(repoPath, resolveGitBinary()))
ipcMain.handle('newroz:git:review:shipInfo', async (_event, repoPath) => reviewShipInfo(repoPath, resolveGhBinary()))
ipcMain.handle('newroz:git:review:createPr', async (_event, repoPath) =>
...
ipcMain.handle('newroz:git:scanRepos', async (_event, roots, options) => {
  try {
...
ipcMain.handle('newroz:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart Newroz.')
  }
...
ipcMain.handle('newroz:terminal:write', (_event, id, data) => {
...
ipcMain.handle('newroz:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('newroz:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('newroz:updates:check', async () =>
...
ipcMain.handle('newroz:updates:apply', async (_event, payload) =>
...
ipcMain.handle('newroz:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('newroz:updates:branch:set', async (_event, name) => {
  const branch = typeof name === 'string' && name.trim() ? name.trim() : DEFAULT_UPDATE_BRANCH
  writeDesktopUpdateConfig({ branch })
  return { branch }
})

// Resolve the canonical Newroz version (the one `release.py` bumps in
// newroz_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Newroz version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveNewrozVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'newroz_cli', '__init__.py')
...
// Re-resolve the live Newroz version and push it into the native About panel
// just before showing it, so an in-place `newroz update` is reflected without
...
ipcMain.handle('newroz:version', async () => ({
  appVersion: resolveNewrozVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  newrozRoot: resolveUpdateRoot()
}))
...
// the actual removal via `newroz uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (newroz_cli/uninstall.py + newroz_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_NEWROZ_ROOT
  ...
  const fallback = () => ({
    newroz_home: NEWROZ_HOME,
    agent_installed: isNewrozSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'newroz_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, NEWROZ_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
      child.stdout.on('data', chunk => {
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Newroz agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import newroz_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_NEWROZ_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `newroz-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `newroz-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('newroz:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('newroz:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('newroz:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('newroz:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// newroz:// deep links (e.g. newroz://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const NEWROZ_PROTOCOL = 'newroz'
let _pendingDeepLink = null
let _rendererReadyForDeepLink = false

function _extractDeepLink(argv) {
  if (!Array.isArray(argv)) return null
  return argv.find(a => typeof a === 'string' && a.startsWith(`${NEWROZ_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // newroz://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('newroz:deep-link', payload)
...
ipcMain.handle('newroz:deep-link-ready', () => {
  _rendererReadyForDeepLink = true
  if (_pendingDeepLink) {
    const queued = _pendingDeepLink
    _pendingDeepLink = null
    handleDeepLink(
      `${NEWROZ_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(NEWROZ_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(NEWROZ_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `newroz://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching newroz:// URL is in our own argv.
