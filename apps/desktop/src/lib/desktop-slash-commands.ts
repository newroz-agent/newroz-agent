...
/** Why a known Newroz command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Newroz profile', surface: action('profile') },
  ...
  { name: '/version', description: 'Show Newroz Agent version', surface: exec() },
...
function isKnownNewrozSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Newroz' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownNewrozSlashCommand(normalized)
}
