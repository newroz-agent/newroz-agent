import type {
  NewrozConnection,
  NewrozReadDirResult,
  NewrozReadFileTextResult,
  NewrozSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: NewrozConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export async function readDesktopDir(path: string): Promise<NewrozReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<NewrozReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<NewrozReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<NewrozReadFileTextResult>(fsPath('read-text', path))
}
...
export async function selectDesktopPaths(options?: NewrozSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
