...
import { getProfiles, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/newroz'
...
import type { ProfileInfo } from '@/types/newroz'
...
// /api/profiles/active `current`). "default" is the root ~/.newroz. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'newroz.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'newroz.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.newrozDesktop.api<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new NEWROZ_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.newrozDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.newrozDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'newroz.desktop.showAllProfiles'
...
// Switch to the default (root ~/.newroz) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.newrozDesktop?.touchBackend?.(target).catch(() => undefined)
}
