import { type ConnectionState, type GatewayEvent, resolveGatewayWsUrl } from '@newroz/shared'
...
const isOpen = (gateway: NewrozGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<NewrozGateway | null>(null)
...
let primaryGateway: NewrozGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: NewrozGateway | null, profile = 'default'): void {
...
export function activeGateway(): NewrozGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<NewrozGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
