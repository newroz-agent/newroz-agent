...
const STORAGE_KEY = 'newroz:native-notifications'
...
// "Backgrounded" = the user isn't on Newroz. `document.hidden` only flips when
...
export function dispatchNativeNotification(input: NativeNotificationInput): void {
  ...
  void window.newrozDesktop?.notify({
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.newrozDesktop
