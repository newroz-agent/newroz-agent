...
import type { NewrozReviewFile, NewrozReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Newroz' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'newroz.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'newroz.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'newroz.desktop.reviewTreeMode'
const SELECTED_KEY = 'newroz.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<NewrozReviewFile[]>([])
...
export const $reviewShipInfo = atom<NewrozReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['newrozDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: NewrozReviewFile): Promise<void> {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.newrozDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.newrozDesktop?.openExternal?.(url)
    }
