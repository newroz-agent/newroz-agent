...
import type { ProjectInfo, ProjectsPayload } from '@/types/newroz'
...
const PROJECT_SCOPE_KEY = 'newroz.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Newroz gateway is not connected')
  }
...
// repo, returning where Newroz should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<NewrozGitBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.newrozDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.newrozDesktop?.writeClipboard?.(path)
  }
}
