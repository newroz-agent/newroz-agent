 ...
 * app's bounds and stays visible while Newroz is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'newroz.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'newroz.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.newrozDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.newrozDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.newrozDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.newrozDesktop?.petOverlay?.close()
}
