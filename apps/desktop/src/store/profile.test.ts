...
const { getProfiles } = await import('@/newroz')
...
const profile = (name: string, isDefault = false): ProfileInfo => ({
  ...
  model: null,
  name,
  path: `/tmp/newroz/${name}`,
  provider: null,
  skill_count: 0
})

const remoteConn = (over: Partial<NewrozConnection> = {}): NewrozConnection =>
  ({ baseUrl: 'https://newroz-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as NewrozConnection

const localConn = (over: Partial<NewrozConnection> = {}): NewrozConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as NewrozConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<NewrozConnection>>()
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { newrozDesktop: { getConnection } })
