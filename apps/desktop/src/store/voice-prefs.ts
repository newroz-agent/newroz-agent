...
import { getNewrozConfigRecord, saveNewrozConfig } from '@/newroz'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getNewrozConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveNewrozConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
