...
const NEWROZ_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type NewrozRefType = (typeof NEWROZ_REF_TYPES)[number]
...
const ICON_PATHS: Record<NewrozRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as NewrozRefType] ?? ICON_FALLBACK
...
const NEWROZ_DIRECTIVE_RE = new RegExp(
...
export const newrozDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(NEWROZ_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as NewrozRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: NewrozRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...newrozDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Newroz directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.newrozDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.newrozDesktop?.readFileDataUrl(id)
