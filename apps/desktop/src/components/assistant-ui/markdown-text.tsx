...
async function mediaSrc(path: string): Promise<string> {
  ...
  if (window.newrozDesktop && ['audio', 'video'].includes(mediaKind(path))) {
  ...
  if (window.newrozDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.newrozDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.newrozDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
function MarkdownLink({ children, className, href, ...props }: ComponentProps<'a'>) {
  ...
  if (window.newrozDesktop && text && normalizeExternalUrl(text) === target) {
