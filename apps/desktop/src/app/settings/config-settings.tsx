...
import { getElevenLabsVoices, getNewrozConfigSchema, saveNewrozConfig } from '@/newroz'
import { useI18n } from '@/i18n'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import type { ConfigFieldSchema, NewrozConfigRecord } from '@/types/newroz'

import { setNewrozConfigCache, useNewrozConfigRecord } from '../hooks/use-config-record'
...
export function voiceFieldVisible(key: string, config: NewrozConfigRecord): boolean {
...
export function ConfigSettings({
  ...
  const [config, setConfig] = useState<NewrozConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useNewrozConfigRecord()
  ...
  const {
    ...
    queryKey: ['newroz-config-schema'],
    queryFn: getNewrozConfigSchema,
    staleTime: 5 * 60 * 1000
  })
  ...
  const updateConfig = (next: NewrozConfigRecord) => {
