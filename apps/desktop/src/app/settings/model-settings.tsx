...
import { invalidateNewrozConfig, setNewrozConfigCache, useNewrozConfigRecord } from '../hooks/use-config-record'
...
// Newroz' reasoning levels (VALID_REASONING_EFFORTS); `none` = thinking off.
// Empty config = Newroz default (medium), shown as Medium.
...
// backend now surfaces the full `newroz model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in newroz_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const { data: config } = useNewrozConfigRecord()
  const setConfig = setNewrozConfigCache
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `newroz model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Newroz runs the flow for you.`}
          </p>
        )}
