import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@newroz/shared'
import { useEffect, useRef } from 'react'

import type { NewrozConnection } from '@/global'
import { NewrozGateway } from '@/newroz'
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.newrozDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: NewrozGateway | null) => void
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: NewrozConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Newroz...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Newroz…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Newroz gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshNewrozConfig().catch(() => undefined)
    ...
    const gateway = new NewrozGateway()
    ...
    async function boot() {
      try {
        ...
        await callbacksRef.current.refreshNewrozConfig()
