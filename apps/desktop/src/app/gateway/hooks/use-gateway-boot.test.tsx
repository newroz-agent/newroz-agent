...
// bug that drives the REAL useGatewayBoot hook + REAL NewrozGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshNewrozConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { newrozDesktop?: unknown }).newrozDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { newrozDesktop?: unknown }).newrozDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForNewroz) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startNewroz()'s remote branch awaits waitForNewroz() for 45s before it
    ...
    ;(window as { newrozDesktop?: unknown }).newrozDesktop = desktop
    ...
    // After ~45s waitForNewroz gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Newroz backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
