...
import type { NewrozReadDirEntry, NewrozReadDirResult } from '@/global'
...
export type ProjectTreeEntry = NewrozReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: NewrozReadDirEntry) {
...
async function filterIgnored(entries: NewrozReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<NewrozReadDirResult> {
  if (!window.newrozDesktop) {
