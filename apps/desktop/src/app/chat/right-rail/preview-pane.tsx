        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.newrozDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Newroz to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.newrozDesktop?.watchPreviewFile ||
      !window.newrozDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.newrozDesktop.onPreviewFileChanged(payload => {
    ...
    void window.newrozDesktop
      .watchPreviewFile(target.url)
      .then(watch => {
        if (!active) {
          void window.newrozDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.newrozDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:newroz-preview')
