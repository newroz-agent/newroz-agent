...
import { getActionStatus, getLogs, getStatus, getUsageAnalytics, restartGateway, updateNewroz } from '@/newroz'
import type { ActionStatusResponse, AnalyticsResponse, StatusResponse } from '@/newroz'
...
export function CommandCenterView({ initialSection, onClose, onDeleteSession, onOpenSession }: CommandCenterViewProps) {
  ...
  // Client-side substring filter over the fetched tail (matches `newroz logs --search`).
  ...
  const runSystemAction = useCallback(
    async (kind: 'restart' | 'update') => {
      ...
      try {
        const started = kind === 'restart' ? await restartGateway() : await updateNewroz()
                    ...
                    <div className="flex items-start justify-between gap-3 max-[47.5rem]:flex-col max-[47.5rem]:gap-2">
                      <div className="min-w-0">
                        ...
                        <div className="mt-1 text-[length:var(--conversation-caption-font-size)] text-(--ui-text-tertiary)">
                          {cc.newrozActiveSessions(status.version, status.active_sessions)}
                        </div>
                      </div>
