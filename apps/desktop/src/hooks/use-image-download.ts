...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'newroz:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.newrozDesktop?.saveImageFromUrl) {
        if (await window.newrozDesktop.saveImageFromUrl(src)) {
