 ...
 * product name (`Newroz`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Newroz'
 *
 * This is a hard failure with no obvious cause for the user — `newroz desktop`
 ...
 * (the mac-unpacked Newroz.app bundle) and Windows (win-unpacked), so we
