...
// no longer bundles the Newroz Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Newroz.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Newroz'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Newroz.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'newroz'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default NEWROZ_HOME for non-sandboxed runs -- matches main.cjs's
// resolveNewrozHome(). On Windows it's %LOCALAPPDATA%\newroz; elsewhere
// it's ~/.newroz. The fresh-install sandbox launchFresh() sets its own
// NEWROZ_HOME and never touches this.
const DEFAULT_NEWROZ_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'newroz')
  }
  return path.join(os.homedir(), '.newroz')
})()
const VENV_ROOT = path.join(DEFAULT_NEWROZ_HOME, 'newroz-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'newroz-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.NEWROZ_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Newroz-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Newroz-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Newroz-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Newroz-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.NEWROZ_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.NEWROZ_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const newrozHome = path.join(sandbox, 'newroz-home')
  const cwd = path.join(sandbox, 'workspace')

  fs.mkdirSync(userDataDir, { recursive: true })
  fs.mkdirSync(newrozHome, { recursive: true })
  ...
  env.NEWROZ_DESKTOP_CWD = cwd
  env.NEWROZ_DESKTOP_IGNORE_EXISTING = '1'
  env.NEWROZ_DESKTOP_TEST_MODE = 'fresh-install'
  env.NEWROZ_DESKTOP_USER_DATA_DIR = userDataDir
  env.NEWROZ_HOME = newrozHome
  delete env.NEWROZ_DESKTOP_NEWROZ
  delete env.NEWROZ_DESKTOP_NEWROZ_ROOT
  ...
  console.log(`  NEWROZ_HOME: ${newrozHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(newrozHome, 'newroz-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Newroz Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of newroz_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'newroz-agent', 'newroz_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Newroz
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + NEWROZ_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  NEWROZ_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
