 ...
 *
 * Stamps the Newroz icon + identity onto the packed Windows Newroz.exe via
 * rcedit (delegated to set-exe-identity.cjs). This runs for EVERY packed build
 * — first install, `newroz desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Newroz')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Newroz'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Newroz.exe keeps the stock Electron icon`)
  }
}
