...
//!
//! Mirrors `newroz_constants.get_newroz_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\newroz
//!   macOS:   ~/.newroz
//!   Linux:   ~/.newroz  (override via $NEWROZ_HOME)
//!
//! NOTE (macOS): Python's get_newroz_home(), scripts/install.sh, and the
//! Electron desktop's resolveNewrozHome() ALL use ~/.newroz on macOS — there
...
/// Returns the canonical Newroz home directory, respecting $NEWROZ_HOME if set.
pub fn newroz_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("NEWROZ_HOME") {
    ...
    {
        // %LOCALAPPDATA%\newroz — matches scripts/install.ps1's $NewrozHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("newroz");
        }
    }

    // macOS + Linux + fallback: ~/.newroz (matches Python get_newroz_home(),
    // install.sh, and the Electron desktop's resolveNewrozHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".newroz");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    newroz_home().join("bootstrap-cache")
}
...
/// NEWROZ_HOME so it survives repo checkout deletion (unlike anything under
/// newroz-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\newroz\newroz-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_newroz`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under NEWROZ_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves NEWROZ_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    newroz_home().join(".newroz-update-in-progress")
}
...
pub fn copy_self_to_newroz_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to NEWROZ_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_NEWROZ_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".newroz-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under NEWROZ_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[newroz-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("NEWROZ_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_newroz_home() -> String {
    newroz_home().to_string_lossy().into_owned()
}
