...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.newroz/newroz-agent/utils.py)
...
2. ``newroz update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``newroz_cli/main.py`` (the ``newroz update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``newroz_constants`` after the
