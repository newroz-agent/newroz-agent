...
""",
        ...
        from newroz_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        newroz_home = tmp_path / ".newroz"
        newroz_home.mkdir()
        (newroz_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("NEWROZ_HOME", str(newroz_home))
        from newroz_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from newroz_cli.plugins import PluginContext, PluginManifest, PluginManager
