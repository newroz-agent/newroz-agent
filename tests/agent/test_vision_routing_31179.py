...
def isolated_home(monkeypatch):
    """Temp NEWROZ_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="newroz_test_31179_")
    newroz_home = os.path.join(test_home, ".newroz")
    os.makedirs(newroz_home)
...
def _fresh_modules():
    """Drop cached newroz modules so each test reloads against current env."""
