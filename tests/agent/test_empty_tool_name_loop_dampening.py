...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated NEWROZ_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="newroz_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".newroz"))
    prev_home = os.environ.get("NEWROZ_HOME")
    os.environ["NEWROZ_HOME"] = os.path.join(test_home, ".newroz")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("newroz_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["NEWROZ_HOME"] = prev_home
