...
path via ``newroz chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp NEWROZ_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point NEWROZ_HOME at a temp dir and turn moa.save_traces on."""
    newroz_home = tmp_path / ".newroz"
    newroz_home.mkdir()
    monkeypatch.setenv("NEWROZ_HOME", str(newroz_home))

    # save_moa_turn reads config via newroz_cli.config.load_config; stub it to
