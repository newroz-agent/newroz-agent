...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate NEWROZ_HOME under tmp_path."""
    monkeypatch.setenv("NEWROZ_HOME", str(tmp_path / ".newroz"))
    (tmp_path / ".newroz").mkdir()

    # Force the constants/image cache helpers to re-read NEWROZ_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("newroz_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_newroz_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under NEWROZ_HOME — gateway cleanup
