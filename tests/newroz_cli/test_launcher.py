"""Tests for the top-level `./newroz` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./newroz` should use `newroz_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "newroz"
    called = []

    fake_main_module = types.ModuleType("newroz_cli.main")

    def fake_main():
        called.append("newroz_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "newroz_cli.main", fake_main_module)
