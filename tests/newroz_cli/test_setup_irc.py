"""Tests for IRC gateway configuration via `newroz setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Newroz install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Newroz install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#newroz")
            monkeypatch.setenv("IRC_NICKNAME", "newroz-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `newroz setup gateway` experience with IRC present."""
