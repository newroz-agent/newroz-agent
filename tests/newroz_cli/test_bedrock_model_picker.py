...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (NEWROZ_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via NEWROZ_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in NEWROZ_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from newroz_cli.providers import NEWROZ_OVERLAYS
        assert "bedrock" in NEWROZ_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from newroz_cli.providers import NEWROZ_OVERLAYS
        assert NEWROZ_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from newroz_cli.providers import NEWROZ_OVERLAYS
        assert NEWROZ_OVERLAYS["bedrock"].auth_type == "aws_sdk"
