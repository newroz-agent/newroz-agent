...
the SAME provider universe as ``newroz model`` (the CLI/TUI picker).
...
def test_every_newroz_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `newroz model` universe."""
    ...
    assert not missing, (
        "providers shown in `newroz model` but not configurable in the desktop "
