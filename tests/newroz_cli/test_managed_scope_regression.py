...
def newroz_home(tmp_path, monkeypatch):
    ...
    # /etc/newroz on the dev/CI box can't influence the test.
    monkeypatch.setenv("NEWROZ_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(newroz_home, monkeypatch):
    from newroz_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(newroz_home, monkeypatch):
    from newroz_cli.config import load_config, cfg_get
...
def test_no_managed_dir_means_user_value_wins(newroz_home):
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_newroz_dotenv(newroz_home=str(home))
