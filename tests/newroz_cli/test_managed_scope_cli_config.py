...
newroz_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `newroz config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_newroz_home = get_newroz_home()`` at import time (module
    singleton), so monkeypatching NEWROZ_HOME after import doesn't move it.
    ...
    this only matters for tests that swap NEWROZ_HOME.)
    """
    import cli

    cli._newroz_home = home
