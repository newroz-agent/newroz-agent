...
When ``NEWROZ_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.newroz/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.newroz            (has its own auth.json fixture)
    * Profile     -> tmp_path/.newroz/profiles/coder   (active, NEWROZ_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Newroz is not
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from newroz_cli.auth import _load_auth_store, _load_provider_state
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    newroz_home = tmp_path / "classic"
    newroz_home.mkdir()
    monkeypatch.setenv("NEWROZ_HOME", str(newroz_home))

    _write(newroz_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (NEWROZ_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.newroz as the "real home" — which is NOT equal
    # to the NEWROZ_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    newroz_home = tmp_path / "classic"
    ...
    _write(newroz_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from newroz_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: NEWROZ_HOME is set to a custom path that is NOT under
    # ~/.newroz/profiles/ — get_default_newroz_root() returns NEWROZ_HOME
