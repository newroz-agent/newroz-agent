...
def container_env(tmp_path, monkeypatch):
    """Set up a fake NEWROZ_HOME with .container-mode file."""
    newroz_home = tmp_path / ".newroz"
    newroz_home.mkdir()
    monkeypatch.setenv("NEWROZ_HOME", str(newroz_home))
    monkeypatch.delenv("NEWROZ_DEV", raising=False)

    container_mode = newroz_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=newroz-agent\n"
        "exec_user=newroz\n"
        "newroz_bin=/data/current-package/bin/newroz\n"
    )
    return newroz_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("newroz_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "newroz-agent"
    assert info["exec_user"] == "newroz"
    assert info["newroz_bin"] == "/data/current-package/bin/newroz"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("newroz_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_newroz_dev(container_env, monkeypatch):
    """Returns None when NEWROZ_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("NEWROZ_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_newroz_dev_zero(container_env, monkeypatch):
    """NEWROZ_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("NEWROZ_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        newroz_home = Path(tmpdir) / ".newroz"
        newroz_home.mkdir()
        (newroz_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("newroz_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_newroz_home": lambda: newroz_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("NEWROZ_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=newroz-custom\n"
        "exec_user=myuser\n"
        "newroz_bin=/opt/newroz/bin/newroz\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "newroz-custom"
    assert info["exec_user"] == "myuser"
    assert info["newroz_bin"] == "/opt/newroz/bin/newroz"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from newroz_cli.main import _exec_in_container
