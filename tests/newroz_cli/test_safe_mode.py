"""Tests for `newroz chat --safe-mode` — pristine troubleshooting runs.
...
_VARS = ("NEWROZ_SAFE_MODE", "NEWROZ_IGNORE_USER_CONFIG", "NEWROZ_IGNORE_RULES")
...
class TestSafeModeEnvWiring:
    ...
    def test_safe_mode_sets_all_gates(self):
        # Mirrors the cmd_chat logic in newroz_cli/main.py.
        class Args:
            safe_mode = True
        ...
        assert os.environ.get("NEWROZ_SAFE_MODE") == "1"
...
class TestSafeModePluginDiscovery:
    """Plugin discovery must be a no-op under NEWROZ_SAFE_MODE=1."""

    def test_discovery_skipped(self, monkeypatch):
        monkeypatch.setenv("NEWROZ_SAFE_MODE", "1")
    ...
    def test_discovery_runs_without_safe_mode(self, monkeypatch):
        monkeypatch.delenv("NEWROZ_SAFE_MODE", raising=False)
        from newroz_cli.plugins import PluginManager
...
class TestSafeModeMCP:
    """_load_mcp_config must return no servers under NEWROZ_SAFE_MODE=1."""
...
class TestSafeModeParser:
    """--safe-mode must parse on both the root parser and `newroz chat`."""
