...
Uses a throwaway NEWROZ_HOME so it never touches ~/.newroz. No real names.
"""
...
def _fresh_home():
    """Point NEWROZ_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_newroz_home() at import time)."""
...
# Import AFTER NEWROZ_HOME is set.
...
# a different get_newroz_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"NEWROZ_HOME (throwaway): {HOME}")
