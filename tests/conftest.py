"""Shared fixtures for the newroz-agent test suite.
...
2. **Isolated NEWROZ_HOME.** NEWROZ_HOME points to a per-test tempdir so
   code reading ``~/.newroz/*`` via ``get_newroz_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".newroz"`` instead of the canonical
   ``get_newroz_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No NEWROZ_SESSION_* inheritance** — the agent's current gateway
...
# NEWROZ_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_NEWROZ_BEHAVIORAL_VARS = frozenset({
    "NEWROZ_YOLO_MODE",
    "NEWROZ_INTERACTIVE",
    "NEWROZ_QUIET",
    "NEWROZ_TOOL_PROGRESS",
    "NEWROZ_TOOL_PROGRESS_MODE",
    "NEWROZ_MAX_ITERATIONS",
    "NEWROZ_SESSION_PLATFORM",
    "NEWROZ_SESSION_CHAT_ID",
    "NEWROZ_SESSION_CHAT_NAME",
    "NEWROZ_SESSION_THREAD_ID",
    "NEWROZ_SESSION_SOURCE",
    "NEWROZ_SESSION_KEY",
    "NEWROZ_GATEWAY_SESSION",
    "NEWROZ_CRON_SESSION",
    "_NEWROZ_GATEWAY",
    "NEWROZ_PLATFORM",
    "NEWROZ_MODEL",
    "NEWROZ_INFERENCE_MODEL",
    "NEWROZ_INFERENCE_PROVIDER",
    "NEWROZ_TUI_PROVIDER",
    "NEWROZ_MANAGED",
    "NEWROZ_MANAGED_DIR",
    "NEWROZ_DEV",
    ...
    "NEWROZ_TIMEZONE",
    "NEWROZ_REDACT_SECRETS",
    "NEWROZ_BACKGROUND_NOTIFICATIONS",
    "NEWROZ_EXEC_ASK",
    "NEWROZ_HOME_MODE",
    "NEWROZ_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.newroz/kanban.db instead of the per-test NEWROZ_HOME.
    "NEWROZ_KANBAN_DB",
    "NEWROZ_KANBAN_BOARD",
    "NEWROZ_KANBAN_HOME",
    "NEWROZ_KANBAN_WORKSPACES_ROOT",
    "NEWROZ_KANBAN_LOGS_ROOT",
    "NEWROZ_KANBAN_TASK",
    "NEWROZ_KANBAN_WORKSPACE",
    "NEWROZ_KANBAN_RUN_ID",
    "NEWROZ_KANBAN_CLAIM_LOCK",
    "NEWROZ_KANBAN_DISPATCH_IN_GATEWAY",
    "NEWROZ_TENANT",
    ...
    "NEWROZ_DASHBOARD_OAUTH_CLIENT_ID",
    "NEWROZ_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and NEWROZ_HOME to per-test tempdirs so code that
    reads ``~/.newroz/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral NEWROZ_* vars that could change test semantics.
    for name in _NEWROZ_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect NEWROZ_HOME to a per-test tempdir. Code that reads
    #    ``~/.newroz/*`` via ``get_newroz_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.newroz/*`` via
    #    ``Path.home() / ".newroz"`` instead of ``get_newroz_home()``
    #    is a bug to fix at the callsite.
    fake_newroz_home = tmp_path / "newroz_test"
    fake_newroz_home.mkdir()
    (fake_newroz_home / "sessions").mkdir()
    (fake_newroz_home / "cron").mkdir()
    (fake_newroz_home / "memories").mkdir()
    (fake_newroz_home / "skills").mkdir()
    monkeypatch.setenv("NEWROZ_HOME", str(fake_newroz_home))
    ...
    #    ~/.newroz/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import newroz_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_newroz_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal newroz config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``newroz-gateway`` process
...
# correlated with a ``tests/newroz_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> newroz-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart newroz-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting newroz/python patterns are also blocked.
    """
    ...
    def _matches_newroz_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _NEWROZ_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_newroz_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch newroz-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m newroz_cli.main".
                if (
                    "newroz" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live newroz-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting newroz/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `newroz update` (or the
        # equivalent `python -m newroz_cli.main update`).  These commands
        ...
        if "update" in low and (
            # newroz update / newroz update --gateway / setsid bash -c ... newroz update
            ("newroz" in low and "update" in low.split())
            or
            # python -m newroz_cli.main update --gateway
            ("newroz_cli" in low and "update" in low.split())
            or
            # venv/bin/newroz update  (absolute path variant used in tests)
            (".venv/bin/newroz" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`newroz update` against the real checkout, fetching "
