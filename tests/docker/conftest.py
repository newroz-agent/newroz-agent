...
Override the image with ``NEWROZ_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("NEWROZ_TEST_IMAGE", "newroz-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``NEWROZ_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("NEWROZ_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged newroz user
# ---------------------------------------------------------------------------
#
# Background: every Newroz runtime path inside the container drops to UID
# 10000 (the ``newroz`` user) via ``s6-setuidgid newroz``. ``docker exec``
...
# fails (PermissionError swallowed) for newroz, so a test that ran as root
...
# helpers below run every probe under ``-u newroz`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: newroz).
