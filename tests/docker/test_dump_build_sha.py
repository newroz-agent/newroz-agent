"""Regression test: ``newroz dump`` reports a real git SHA inside the container.

Background: ``.dockerignore`` excludes ``.git``, so ``git rev-parse HEAD``
fails inside the published image and ``newroz dump`` used to report
``version: ... [(unknown)]``.  The Dockerfile now writes the build-time
``$NEWROZ_GIT_SHA`` build-arg to ``/opt/newroz/.newroz_build_sha`` and
``newroz_cli/build_info.py`` reads it as a fallback.
...
is absent and ``newroz dump`` correctly falls back to ``(unknown)``.

This test handles both cases:

* If ``/opt/newroz/.newroz_build_sha`` exists in the image, assert that
  ``newroz dump`` surfaces its content as the version SHA (not
  ``(unknown)``).
...
def _run_dump(image: str) -> str:
    ...
    Dockerfile) so the container's newroz user (UID 10000) can bootstrap
    ...
    assert r.returncode == 0, (
        f"newroz dump exited {r.returncode}: "
...
def _read_baked_sha_from_image(image: str) -> str | None:
    """Return the ``/opt/newroz/.newroz_build_sha`` content, or None if absent."""
    r = subprocess.run(
        [
            "docker", "run", "--rm", "--entrypoint", "cat", image,
            "/opt/newroz/.newroz_build_sha",
        ],
...
def test_dump_reports_baked_sha_when_present(built_image: str) -> None:
    """When the image was built with ``NEWROZ_GIT_SHA``, dump must surface it.
    ...
    # CI path: build-arg was set, baked file exists.  ``newroz dump``
