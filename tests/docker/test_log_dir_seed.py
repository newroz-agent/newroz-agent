"""Runtime smoke test for Docker $NEWROZ_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the newroz user after container boot.
...
registered later runs its log service as the dropped newroz user and
...
def test_logs_gateways_seeded_and_newroz_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by newroz after boot."""
    ...
    assert "logs=newroz" in r.stdout, (
        f"logs/ not owned by newroz: {r.stdout}"
    )
    assert "gateways=newroz" in r.stdout, (
        f"logs/gateways/ not owned by newroz: {r.stdout}"
    )
