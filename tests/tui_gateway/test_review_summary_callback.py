...
def server():
    with patch.dict(
        "sys.modules",
        {
            "newroz_constants": MagicMock(
                get_newroz_home=MagicMock(return_value="/tmp/newroz_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'newroz-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'newroz-release' patched"
    }
