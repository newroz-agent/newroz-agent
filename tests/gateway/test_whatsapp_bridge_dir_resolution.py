...
(/opt/newroz/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable NEWROZ_HOME location instead.
"""
import importlib
...
def test_writable_install_returns_install_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "newroz_constants.get_newroz_home", lambda: newroz_home
    )

    resolved = whatsapp_common.resolve_whatsapp_bridge_dir()
    assert resolved == install_bridge
    # Nothing mirrored into NEWROZ_HOME.
    assert not (newroz_home / "scripts" / "whatsapp-bridge").exists()


def test_readonly_install_mirrors_to_newroz_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable NEWROZ_HOME."""
    install_root = tmp_path / "install"
    install_bridge = install_root / "scripts" / "whatsapp-bridge"
    _seed_install_tree(install_bridge)
    ...
    expected = newroz_home / "scripts" / "whatsapp-bridge"
...
def test_readonly_install_reuses_existing_mirror(tmp_path, monkeypatch):
    """If the NEWROZ_HOME mirror already exists, return it without re-copying."""
    install_root = tmp_path / "install"
    install_bridge = install_root / "scripts" / "whatsapp-bridge"
    _seed_install_tree(install_bridge)

    newroz_home = tmp_path / "newroz_home"
    mirror = newroz_home / "scripts" / "whatsapp-bridge"
