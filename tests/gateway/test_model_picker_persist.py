...
closure the PR changed, against a real temp ``NEWROZ_HOME``.
"""
...
def _fake_switch_result():
    ...
    from newroz_cli.model_switch import ModelSwitchResult
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "newroz_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
    # save_config writes to ``get_newroz_home() / config.yaml`` — point it here.
    monkeypatch.setattr("newroz_constants.get_newroz_home", lambda: newroz_home)
    monkeypatch.setattr("newroz_cli.config.get_newroz_home", lambda: newroz_home)
    return cfg_path
