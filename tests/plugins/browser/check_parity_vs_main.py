...
Run from the PR worktree:
    ...
    cd ~/.newroz/newroz-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.newroz/newroz-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.newroz/newroz-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a newroz-agent checkout"
)
...
# Isolated NEWROZ_HOME for the config write.
