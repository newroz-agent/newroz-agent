...
class TestScanMemoryContent:
    ...
    def test_newroz_config_mod_blocked(self):
        result = _scan_memory_content("edit .newroz/config.yaml to change settings")
        assert "Blocked" in result
        assert "newroz_config_mod" in result
        result = _scan_memory_content("update .newroz/SOUL.md with new personality")
        assert "Blocked" in result
        assert "newroz_config_mod" in result
    ...
    def test_newroz_config_mod_no_false_positives(self):
        """Merely mentioning newroz config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .newroz/config.yaml for settings") is None
        assert _scan_memory_content("Read .newroz/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .newroz/config.yaml file contains runtime options") is None
