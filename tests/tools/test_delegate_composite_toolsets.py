...
class TestExpandParentToolsets(unittest.TestCase):
    ...
    def test_composite_newroz_cli_expands_web(self):
        """newroz-cli includes web_search/web_extract → 'web' should be in expansion."""
        expanded = _expand_parent_toolsets({"newroz-cli"})
        self.assertIn("web", expanded)
        self.assertIn("terminal", expanded)
        self.assertIn("browser", expanded)
        # Original composite is preserved
        self.assertIn("newroz-cli", expanded)
    ...
    def test_intersection_with_expanded_composite(self):
        """End-to-end: requesting ['web'] from parent with ['newroz-cli'] yields ['web']."""
        parent_toolsets = {"newroz-cli"}
