"""Tests for --yolo (NEWROZ_YOLO_MODE) approval bypass."""
...
class TestYoloMode:
    """When NEWROZ_YOLO_MODE is set, all dangerous commands are auto-approved."""
    ...
    def test_dangerous_command_blocked_normally(self, monkeypatch):
        ...
        monkeypatch.setenv("NEWROZ_SESSION_KEY", "test-session")
    ...
    def test_dangerous_command_approved_in_yolo_mode(self, monkeypatch):
        """With NEWROZ_YOLO_MODE, dangerous commands are auto-approved."""
    ...
    def test_yolo_mode_not_set_by_default(self):
        """NEWROZ_YOLO_MODE should not be set by default."""
        # Clean env check — if it happens to be set in test env, that's fine,
        # we just verify the mechanism exists
        assert os.getenv("NEWROZ_YOLO_MODE") is None or True  # no-op, documents intent

    def test_yolo_mode_empty_string_does_not_bypass(self, monkeypatch):
        """Empty string for NEWROZ_YOLO_MODE should not trigger bypass."""
        monkeypatch.setenv("NEWROZ_YOLO_MODE", "")
        monkeypatch.setenv("NEWROZ_INTERACTIVE", "1")
        monkeypatch.setenv("NEWROZ_SESSION_KEY", "test-session")

        # Empty string is falsy in Python, so getenv("NEWROZ_YOLO_MODE") returns ""
    ...
    def test_false_like_yolo_values_do_not_bypass_dangerous_command(self, monkeypatch, value):
        ...
        monkeypatch.setenv("NEWROZ_YOLO_MODE", value)
        monkeypatch.setenv("NEWROZ_INTERACTIVE", "1")
        monkeypatch.setenv("NEWROZ_SESSION_KEY", "test-session")
