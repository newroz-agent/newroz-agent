...
def _clean_state():
    ...
    saved = {}
    for k in ("NEWROZ_INTERACTIVE", "NEWROZ_GATEWAY_SESSION", "NEWROZ_EXEC_ASK", "NEWROZ_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["NEWROZ_INTERACTIVE"] = "1"
        session_key = os.getenv("NEWROZ_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No NEWROZ_INTERACTIVE or NEWROZ_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
