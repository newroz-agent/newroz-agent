...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad NEWROZ_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: NEWROZ_GATEWAY_SESSION set, a bound session
    ...
    monkeypatch.delenv("NEWROZ_CRON_SESSION", raising=False)
...
def test_guard_cron_deny_blocks(monkeypatch):
    monkeypatch.setenv("NEWROZ_CRON_SESSION", "1")
    monkeypatch.delenv("NEWROZ_GATEWAY_SESSION", raising=False)
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
...
def test_env_scrub_newroz_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "NEWROZ_HOME": "/h", "NEWROZ_PROFILE": "p",
        "NEWROZ_CONFIG": "/c.yaml", "NEWROZ_ENV": "/e",
        # other NEWROZ_* → dropped (broad prefix removed)
        "NEWROZ_BASE_URL": "https://x", "NEWROZ_INTERACTIVE": "1",
        "NEWROZ_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("NEWROZ_HOME", "NEWROZ_PROFILE", "NEWROZ_CONFIG", "NEWROZ_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "NEWROZ_BASE_URL", "NEWROZ_INTERACTIVE", "NEWROZ_KANBAN_DB",
...
def test_env_scrub_logs_dropped_newroz_vars(caplog):
    """Dropping a non-allowlisted, non-secret NEWROZ_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset NEWROZ_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "NEWROZ_HOME": "/h",          # allowlisted → kept, not logged
        "NEWROZ_BASE_URL": "https://x",   # dropped → logged
        "NEWROZ_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "NEWROZ_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "NEWROZ_HOME" in out and "PATH" in out
    assert "NEWROZ_BASE_URL" not in out and "NEWROZ_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "NEWROZ_BASE_URL" in msgs and "NEWROZ_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "NEWROZ_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped NEWROZ_* vars."""
    import logging
