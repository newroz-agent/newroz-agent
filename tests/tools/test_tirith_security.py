...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._newroz_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(newroz_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(newroz_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_newroz_bin(self):
        """After _INSTALL_FAILED, manual install in NEWROZ_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        newroz_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(newroz_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(newroz_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._newroz_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == newroz_bin
            assert _tirith_mod._resolved_path == newroz_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# NEWROZ_HOME isolation
# ---------------------------------------------------------------------------

class TestNewrozHomeIsolation:
    def test_newroz_bin_dir_respects_newroz_home(self):
        """_newroz_bin_dir must use NEWROZ_HOME, not hardcoded ~/.newroz."""
        from tools.tirith_security import _newroz_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"NEWROZ_HOME": tmpdir}):
            result = _newroz_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_newroz_home(self):
        """_failure_marker_path must use NEWROZ_HOME, not hardcoded ~/.newroz."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"NEWROZ_HOME": "/custom/newroz"}):
            result = _failure_marker_path()
        assert result == "/custom/newroz/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets NEWROZ_HOME; verify it's active."""
        newroz_home = os.getenv("NEWROZ_HOME")
        assert newroz_home is not None, "NEWROZ_HOME should be set by conftest"
        assert "newroz_test" in newroz_home, "Should point to test temp dir"

    def test_get_newroz_home_fallback(self):
        """Without NEWROZ_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_newroz_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove NEWROZ_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".newroz")
            result = _get_newroz_home()
    ...
    def test_mkdtemp_oserror_propagates_to_ensure_installed(self):
        ...
        with patch("tools.tirith_security.tempfile.mkdtemp",
             ...
             patch("tools.tirith_security.shutil.which",
                   return_value=None), \
             patch("tools.tirith_security._newroz_bin_dir",
