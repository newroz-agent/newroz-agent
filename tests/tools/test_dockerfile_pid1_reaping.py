...
- Signal forwarding runs through the init so ``docker stop`` triggers
  newroz's own graceful-shutdown path.
...
def test_dockerfile_installs_an_init_for_zombie_reaping(dockerfile_text):
    ...
    Without a PID-1 init that handles SIGCHLD, newroz accumulates zombie
    ...
    assert installed, (
        ...
        "reap orphaned subprocesses, newroz accumulates zombies in Docker "
...
def test_dockerfile_entrypoint_routes_through_the_init(dockerfile_text):
    ...
    directly, the shell becomes PID 1 and will ``exec`` into newroz,
    ...
    assert routes_through_init, (
        ...
        "not wired into ENTRYPOINT, newroz still runs as PID 1 and zombies "
...
def test_dockerfile_installs_tui_dependencies(dockerfile_text):
    ...
    # resolve dependencies. The bundled ``newroz-ink`` workspace package is
    ...
    assert "ui-tui/packages/newroz-ink/" in dockerfile_text
...
def test_dockerfile_preinstalls_hindsight_memory_dependency(dockerfile_text):
    ...
    assert any("--extra hindsight" in step for step in sync_steps), (
        ...
        "into /opt/newroz/.venv. It lazy-installs into the image layer (not the "
...
def test_dockerfile_materializes_local_tui_ink_package(dockerfile_text):
    # ``newroz-ink`` is a bundled workspace package referenced from
    # ``ui-tui/package.json`` via ``file:`` — not pulled from the npm
    # registry. The contract this test pins is just that the image
    # actually carries the package source so ``await import('@newroz/ink')``
    # can resolve at runtime; the previous, much pickier assertion (manual
    # ``rm -rf`` + ``npm install --omit=dev --prefix node_modules/@newroz/ink``)
    ...
    assert "ui-tui/packages/newroz-ink/" in dockerfile_text, (
        "Dockerfile must COPY the bundled newroz-ink workspace package "
        "so ``await import('@newroz/ink')`` resolves at runtime."
    )
