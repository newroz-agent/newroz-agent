...
class TestWriteDenyExactPaths:
    ...
    def test_newroz_env(self):
        # ``.env`` under the active NEWROZ_HOME (profile-aware, not just
        # ``~/.newroz``) must be write-denied. The hermetic test conftest
        # points NEWROZ_HOME at a tempdir — resolve via get_newroz_home()
        # to match the denylist.
        from newroz_constants import get_newroz_home
        path = str(get_newroz_home() / ".env")
        assert _is_write_denied(path) is True

    def test_newroz_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.newroz/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "newroz_root"
        ...
        # Sanity check: NEWROZ_HOME does point to the profile dir, not the root.
        from newroz_constants import get_newroz_home, get_default_newroz_root
        assert get_newroz_home() == profile_home
        assert get_default_newroz_root() == root
...
class TestWriteAllowed:
    ...
    def test_newroz_control_files_requested_writable(self):
        from newroz_constants import get_newroz_home
