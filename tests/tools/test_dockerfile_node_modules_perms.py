...
Older images made /opt/newroz/ui-tui and /opt/newroz/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV NEWROZ_TUI_DIR=/opt/newroz/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R newroz:newroz /opt/newroz/ui-tui" not in text
    assert "chown -R newroz:newroz /opt/newroz/node_modules" not in text
