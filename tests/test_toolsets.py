...
class TestToolsetConsistency:
    ...
    def test_newroz_platforms_share_core_tools(self):
        """All newroz-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on newroz-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["newroz-cli", "newroz-telegram", "newroz-discord", "newroz-whatsapp", "newroz-slack", "newroz-signal", "newroz-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_newroz_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("newroz-whatsapp")

    def test_newroz_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("newroz-api-server")
