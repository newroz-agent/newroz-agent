"""Tests for newroz_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Newroz entry point
(newroz, newroz-agent, newroz-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Newroz entry point imports newroz_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported newroz_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("newroz_bootstrap", None)
    import newroz_bootstrap  # noqa: WPS433
    return newroz_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Newroz CLI
...
class TestEntryPointsImportBootstrap:
    """Every Newroz entry point must import newroz_bootstrap as its
    ...
    # Entry points that invoke Newroz as a process.  Each one must
    # import newroz_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "newroz_cli/main.py",   # newroz CLI (console_script)
        "run_agent.py",          # newroz-agent (console_script)
        "acp_adapter/entry.py",  # newroz-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import newroz_bootstrap' and that
        ...
        half-finished ``newroz update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``newroz_bootstrap`` as a top-level module) leaves newroz
        ...
        """
        # Resolve relative to the newroz-agent repo root.  Tests live
        # at tests/test_newroz_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # newroz start even when newroz_bootstrap hasn't been
        ...
        assert first_import_name == "newroz_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'newroz_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import newroz_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Newroz's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("NEWROZ_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["NEWROZ_PYTHON_SRC_ROOT"] = env
            elif "NEWROZ_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["NEWROZ_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/newroz")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("NEWROZ_PYTHON_SRC_ROOT", None)
            else:
                os.environ["NEWROZ_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/newroz", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Newroz root.  The guard must relocate Newroz to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/newroz"])
        assert result[0] == "/opt/newroz"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Newroz root.
        assert result.index("/opt/newroz") < result.index("/home/user/tg-ws-proxy")

    def test_src_root_not_duplicated(self):
        hb = _fresh_import()
        result = self._run(hb, ["/opt/newroz", "/opt/newroz", ""])
        assert result.count("/opt/newroz") == 1
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["NEWROZ_PYTHON_SRC_ROOT"] = "/env/newroz"
            hb.harden_import_path()
            assert sys.path[0] == "/env/newroz"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("NEWROZ_PYTHON_SRC_ROOT", None)
