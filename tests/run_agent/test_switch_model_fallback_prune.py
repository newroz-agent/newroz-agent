...
Reported: "switched from openrouter provider to anthropic api key via newroz
...
def _switch_to_anthropic(agent):
    with (
        ...
        patch("newroz_cli.timeouts.get_provider_request_timeout", return_value=None),
    ):
        agent.switch_model(
...
def test_switch_within_same_provider_preserves_chain():
    ...
    with patch("newroz_cli.timeouts.get_provider_request_timeout", return_value=None):
        agent.switch_model(
