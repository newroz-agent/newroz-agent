...
binary download are exercised manually by `newroz secrets bitwarden
...
def newroz_home(tmp_path, monkeypatch):
    """Point Newroz at an isolated home directory."""
    home = tmp_path / ".newroz"
    home.mkdir()
    monkeypatch.setenv("NEWROZ_HOME", str(home))
    # Some modules cache get_newroz_home; clear if needed.
    import newroz_constants
    if hasattr(newroz_constants, "_NEWROZ_HOME_CACHE"):
        newroz_constants._NEWROZ_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(newroz_home, monkeypatch):
...
def test_install_bws_happy_path(newroz_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(newroz_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(newroz_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from newroz_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under newroz_home/cache/."""
    home = tmp_path / ".newroz"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
