"""
Tests for mcp_serve — Newroz MCP server.
...
def _isolate_newroz_home(tmp_path, monkeypatch):
    """Redirect NEWROZ_HOME to a temp directory."""
    monkeypatch.setenv("NEWROZ_HOME", str(tmp_path))
    try:
        import newroz_constants
        monkeypatch.setattr(newroz_constants, "get_newroz_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking newroz_state schema."""
...
class TestEventBridgePollE2E:
    ...
    def test_poll_picks_up_new_conversation_on_db_change(
        ...
        # _poll_once reads <NEWROZ_HOME>/state.db for its mtime gate; the autouse
        # fixture points NEWROZ_HOME at tmp_path.
        db_path = tmp_path / "state.db"
