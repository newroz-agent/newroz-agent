...
def test_root_session_no_compression(db):
    ...
    assert prov["currentNewrozSessionId"] == "root1"
    assert prov["rootNewrozSessionId"] == "root1"
    assert prov["parentNewrozSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_newroz_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentNewrozSessionId"] == "old"
    assert prov["rootNewrozSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousNewrozSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootNewrozSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootNewrozSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_newroz_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousNewrozSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"newroz"}
    assert "sessionProvenance" in meta["newroz"]
    assert meta["newroz"]["sessionProvenance"]["currentNewrozSessionId"] == "root1"
