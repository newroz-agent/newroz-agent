"""Tests for ACP Registry metadata shipped with Newroz."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["website"].startswith("https://newroz-agent.nousresearch.com/")
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"newroz-agent[acp]=={data['version']}"
    assert uvx["args"] == ["newroz-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"newroz-agent[acp]=={_pyproject_version()}"
    )
