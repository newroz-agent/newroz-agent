...
- Unlimited timeout (NEWROZ_CRON_TIMEOUT=0)
- Backward compat: NEWROZ_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_unlimited_timeout(self):
        """NEWROZ_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """NEWROZ_CRON_TIMEOUT env var is respected."""
    ...
    def test_timeout_zero_means_unlimited(self, monkeypatch):
        """NEWROZ_CRON_TIMEOUT=0 yields None (unlimited)."""
    ...
    def test_timeout_invalid_value_falls_back_to_default(self, monkeypatch):
        """NEWROZ_CRON_TIMEOUT=abc should fall back to 600s, not raise ValueError."""
        monkeypatch.setenv("NEWROZ_CRON_TIMEOUT", "abc")
        raw = os.getenv("NEWROZ_CRON_TIMEOUT", "").strip()
        _cron_timeout = self._parse_cron_timeout(raw)
    ...
    def test_timeout_empty_string_uses_default(self, monkeypatch):
        """NEWROZ_CRON_TIMEOUT='' (empty) should use the 600s default."""
        monkeypatch.setenv("NEWROZ_CRON_TIMEOUT", "")
        raw = os.getenv("NEWROZ_CRON_TIMEOUT", "").strip()
        _cron_timeout = self._parse_cron_timeout(raw)
        assert _cron_timeout == 600.0
...
class TestSysPathOrdering:
    ...
    def test_newroz_time_importable(self):
        """newroz_time should be importable when cron.scheduler loads."""
        # This import would fail if sys.path.insert comes after the import
        from cron.scheduler import _newroz_now
        assert callable(_newroz_now)

    def test_newroz_constants_importable(self):
        """newroz_constants should be importable from cron context."""
        from newroz_constants import get_newroz_home
        assert callable(get_newroz_home)
