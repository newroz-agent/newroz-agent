...
profile's NEWROZ_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.newroz/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active NEWROZ_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever NEWROZ_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    NEWROZ_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import newroz_constants

    monkeypatch.setattr(
        newroz_constants, "_get_platform_default_newroz_home", lambda: root
    )
    monkeypatch.setenv("NEWROZ_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile NEWROZ_HOME (<root>/profiles/<name>), the cron store
    resolves to <profile>/cron, NOT the shared <root>/cron."""
    root = tmp_path / "newroz_home"
    ...
    assert newroz_constants.get_newroz_home().resolve() == profile_home.resolve()
    assert newroz_constants.get_default_newroz_root().resolve() == root.resolve()

    # cron/jobs.py computes NEWROZ_DIR from get_newroz_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.NEWROZ_DIR.resolve() == profile_home.resolve()
        assert (
...
def test_cron_execution_home_follows_active_profile(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(scheduler, "_newroz_home", None, raising=False)
    assert scheduler._get_newroz_home().resolve() == profile_home.resolve()
    assert scheduler._get_newroz_home().resolve() != root.resolve()


def test_cron_storage_unaffected_when_no_profile(tmp_path, monkeypatch):
    """With no profile (NEWROZ_HOME == root), the store is the root's cron dir
    ...
    importlib.reload(jobs)
    try:
        assert jobs.NEWROZ_DIR.resolve() == root.resolve()
