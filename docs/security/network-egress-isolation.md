...
When running Newroz inside Docker, the default `network_mode: host` gives the
...
The Newroz [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   NEWROZ_UID=$(id -u) NEWROZ_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=newroz,newroz-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://newroz-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Newroz trust model and vulnerability reporting
