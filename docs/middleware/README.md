...
Newroz middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Newroz'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Newroz calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Newroz retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Newroz guardrails,
...
- `telemetry_schema_version`: currently `newroz.observer.v1`
- `middleware_schema_version`: currently `newroz.middleware.v1`
...
Newroz stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Newroz runs
them as a nested chain in registration order. Middleware failures are fail-open:
Newroz logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Newroz applies middleware in this order:
...
For each tool call, Newroz applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Newroz pre-execution path against the effective arguments:
...
```bash
newroz plugins enable <plugin-name>
```

For isolated local testing, use one `NEWROZ_HOME` for plugin enablement and the
agent run:

```bash
export NEWROZ_HOME=/tmp/newroz-middleware-test
mkdir -p "$NEWROZ_HOME"
newroz plugins enable <plugin-name>
newroz chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run newroz plugins enable <plugin-name>
uv run newroz chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["newroz_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/newroz-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Newroz expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Newroz treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Newroz preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Newroz does not convert downstream
