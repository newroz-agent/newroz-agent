...
Newroz supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.newroz/config.yaml`:
...
Or set the env var: `NEWROZ_KANBAN_DISPATCH_IN_GATEWAY=false`
