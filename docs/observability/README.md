# Newroz Observer Hooks

Newroz observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Newroz' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Newroz catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Newroz only builds
