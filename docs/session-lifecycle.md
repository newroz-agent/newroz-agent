...
The canonical transcript store is SQLite via `SessionDB` (from `newroz_state`). The
...
Counts consecutive restarts via a JSON file (`{NEWROZ_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `newroz update`, `newroz gateway restart`,
or `/restart`.
