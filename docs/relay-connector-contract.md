...
This document is the formal interface between the **Newroz gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `newroz gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`newroz gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `newroz` CLI.
