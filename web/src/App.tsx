...
const SIDEBAR_COLLAPSED_KEY = "newroz-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="newroz-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="newroz-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateNewroz = status?.can_update_newroz === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'newroz update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "newroz update";
    return (
      t.status.updateNewrozConfirmMessage ??
      `This will run 'newroz update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateNewrozConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateNewroz) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateNewroz,
      runningLabel: t.status.updatingNewroz,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Newroz gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateNewrozConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateNewrozConfirmTitle ?? `${t.status.updateNewroz}?`}
    />
    </>
  );
}
