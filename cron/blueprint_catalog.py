...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``newroz://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``newroz://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"newroz://blueprint/{quote(blueprint.key)}{qs}"
