...
``newroz gateway restart`` (or ``launchctl kickstart ai.newroz.gateway``
or ``systemctl restart newroz-gateway``).  When the cron fires, the
...
``newroz cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``newroz gateway``, ``launchctl ... newroz-gateway``,
``systemctl ... newroz-gateway``, ``pkill`` against the gateway) so it
...
blocks these commands at *execution* time when ``_NEWROZ_GATEWAY=1``, and
``newroz gateway stop|restart`` refuse to self-target from inside the
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `newroz gateway restart|stop` — the canonical foot-gun.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    r"(?:newroz\s+gateway\s+(?:restart|stop))"
    # Branch B: launchctl ops on a newroz-gateway label. macOS launchd
    # labels look like `ai.newroz.gateway` / `newroz-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated newroz services (e.g.
    # `launchctl unload ai.newroz.update-checker.plist`).
    r"|(?:launchctl\s+(?:kickstart|unload|load|stop|restart)\b[^\n]*\bnewroz[.\-]?gateway)"
    # Branch C: systemctl ops on a newroz-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\bnewroz[.\-]?gateway)"
    # Branch D: pkill / kill targeting the newroz gateway process. Both
...
def _resolve_script_path(script_path: str) -> Path:
    ...
    under ``<NEWROZ_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.newroz/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    return get_newroz_home() / "scripts" / raw
...
def check_gateway_lifecycle(
    ...
    if contains_gateway_lifecycle_command(combined):
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `newroz gateway restart` from a shell outside "
