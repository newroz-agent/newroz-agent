# nix/nixosModules.nix — NixOS module for newroz-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: newroz runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $NEWROZ_HOME/.env
# and read by newroz at startup — no container recreation needed for env changes.
#
# Tool resolution: the newroz wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.newroz-agent;
    effectivePackage =
    ...
    newroz-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-newroz-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "newroz-config-attrs";
      description = "Newroz YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "newroz-config.yaml" configJson;
    configFile = if cfg.configFile != null then cfg.configFile else generatedConfigFile;

    configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

    # config.yaml mode: group-writable (0660) when interactive users share this
    # NEWROZ_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "newroz-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'NEWROZ_DOC_EOF'\n${value}\nNEWROZ_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "newroz-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/newroz";
    ...
    # newroz user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "newroz-container-entrypoint" ''
      set -eu

      NEWROZ_UID="''${NEWROZ_UID:?NEWROZ_UID must be set}"
      NEWROZ_GID="''${NEWROZ_GID:?NEWROZ_GID must be set}"

      # ── Group: ensure a group with GID=$NEWROZ_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$NEWROZ_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="newroz"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$NEWROZ_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$NEWROZ_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$NEWROZ_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$NEWROZ_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="newroz"
        TARGET_HOME="/home/newroz"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$NEWROZ_UID" -g "$NEWROZ_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$NEWROZ_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$NEWROZ_UID:$NEWROZ_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure NEWROZ_HOME is owned by the target user.
      ...
      if [ -n "''${NEWROZ_HOME:-}" ] && [ -d "$NEWROZ_HOME" ]; then
        find "$NEWROZ_HOME" \! -user "$NEWROZ_UID" -exec chown "$NEWROZ_UID:$NEWROZ_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/newroz-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/newroz-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/newroz ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/newroz
        chmod 0440 /etc/sudoers.d/newroz
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$NEWROZ_UID" --regid="$NEWROZ_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $NEWROZ_HOME/.env.
    ...
    # Default: /var/lib/newroz/workspace → /data/workspace.
  ...
  in {
    options.services.newroz-agent = with lib; {
      enable = mkEnableOption "Newroz Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = newroz-agent;
        description = "The newroz-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/newroz";
        description = "State directory. Contains .newroz/ subdir (NEWROZ_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Newroz config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $NEWROZ_HOME/.env at activation time.
          Newroz reads this file on every startup via load_newroz_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $NEWROZ_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $NEWROZ_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `newroz gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the newroz user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the newroz plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Newroz discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          newroz_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as newroz (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in newroz-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the newroz CLI to environment.systemPackages and export
          NEWROZ_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.newroz symlink to the service
            stateDir. These users are automatically added to the newroz group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.newroz-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the newroz CLI to system PATH and export NEWROZ_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.newroz/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.NEWROZ_HOME = "${cfg.stateDir}/.newroz";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.newroz-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the newroz user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.newroz-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed newroz
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure newroz is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.newroz        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.newroz/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.newroz/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.newroz/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.newroz/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.newroz/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."newroz-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.newroz
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.newroz ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.newroz ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.newroz -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.newroz/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.newroz/$_subdir"
            chmod 2770 "${cfg.stateDir}/.newroz/$_subdir"
            find "${cfg.stateDir}/.newroz/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # newroz-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.newroz/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.newroz/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.newroz/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.newroz/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.newroz/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.newroz/.managed
          chmod 0644 ${cfg.stateDir}/.newroz/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.newroz/.container-mode <<'NEWROZ_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    newroz_bin=${containerDataDir}/current-package/bin/newroz
    NEWROZ_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.newroz/.container-mode
            chmod 0644 ${cfg.stateDir}/.newroz/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.newroz/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.newroz";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.newroz" ]; then
                  rm -f "${symlinkPath}"
                  echo "newroz-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.newroz -> stateDir/.newroz for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.newroz";
                target = "${cfg.stateDir}/.newroz";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "newroz-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.newroz/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.newroz/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.newroz/auth.json
              fi
            ''}
          ''}
          ...
          # Newroz reads $NEWROZ_HOME/.env at startup via load_newroz_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.newroz/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'NEWROZ_NIX_ENV_EOF'
    ${envFileContent}
    NEWROZ_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.newroz/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.newroz/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.newroz/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.newroz-agent = {
          description = "Newroz Agent Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            NEWROZ_HOME = "${cfg.stateDir}/.newroz";
            NEWROZ_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $NEWROZ_HOME/.env by the activation script. load_newroz_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/newroz"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the newroz group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.newroz-agent = {
          description = "Newroz Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              NEWROZ_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              NEWROZ_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env NEWROZ_UID="$NEWROZ_UID" \
                --env NEWROZ_GID="$NEWROZ_GID" \
                --env NEWROZ_HOME=${containerDataDir}/.newroz \
                --env NEWROZ_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/newroz gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
