#!/usr/bin/env bash
# Canonical test runner for newroz-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.newroz/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.newroz/newroz-agent/venv"; do
...
if [ -f "$HOME/.newroz/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.newroz"
...
exec env -i \
  ...
  ${NEWROZ_RUN_SLOW_PET_TESTS:+NEWROZ_RUN_SLOW_PET_TESTS="$NEWROZ_RUN_SLOW_PET_TESTS"} \
