# ============================================================================
# Newroz Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$NewrozHome = $(if ($env:NEWROZ_HOME) { $env:NEWROZ_HOME } else { "$env:LOCALAPPDATA\newroz" }),
    [string]$InstallDir = $(if ($env:NEWROZ_HOME) { "$env:NEWROZ_HOME\newroz-agent" } else { "$env:LOCALAPPDATA\newroz\newroz-agent" }),
    ...
    # builds apps/desktop into a launchable Newroz.exe.
    #
    # Why opt-in:
    #   * Newroz-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Newroz.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Newroz.exe
    ...
    #     `newroz desktop` already builds on demand.
...
$RepoUrlSsh = "git@github.com:newroz-agent/newroz-agent.git"
$RepoUrlHttps = "https://github.com/newroz-agent/newroz-agent.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * Newroz Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $NewrozHome)) {
        New-Item -ItemType Directory -Force -Path $NewrozHome | Out-Null
    }
    $envFile = Join-Path $NewrozHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $NewrozHome "node"
...
function Install-Uv {
    # Newroz owns its own uv at $NewrozHome\bin\uv.exe.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (newroz_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `newroz update` stay in sync.
    $managedUv = Join-Path $NewrozHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $NewrozHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $NewrozHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $NewrozHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $NewrozHome "bin"
...
function Resolve-UvCmd {
    ...
    # sibling process and NEWROZ_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Newroz-Setup.exe
...
function Install-Git {
    ...
    which Newroz uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\newroz\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Newroz needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Newroz copy of Git ourselves is predictable and
    ...
    ``NEWROZ_GIT_BASH_PATH`` (User scope) so Newroz can find it in a fresh
    ...
    # Download PortableGit into $NewrozHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $NewrozHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Newroz features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Newroz needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``NEWROZ_GIT_BASH_PATH`` (User env scope) so Newroz can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $NewrozHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $NewrozHome\git\usr\bin\bash.exe
    $candidates += "$NewrozHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$NewrozHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("NEWROZ_GIT_BASH_PATH", $candidate, "User")
            $env:NEWROZ_GIT_BASH_PATH = $candidate
            Write-Info "Set NEWROZ_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Newroz may not find Git Bash."
    Write-Info "If needed, set NEWROZ_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$NewrozHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$NewrozHome\node;$env:Path"
        Write-Success "Node.js $version found (Newroz-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Newroz-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $NewrozHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $NewrozHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            if ($extractedDir) {
                if (Test-Path "$NewrozHome\node") { Remove-Item -Recurse -Force "$NewrozHome\node" }
                Move-Item $extractedDir.FullName "$NewrozHome\node"
                ...
                $version = & "$NewrozHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $NewrozHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\newroz-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `newroz
                    # update` path (#4735).
                    ...
                    $stashName = "newroz-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # ff-only pull cannot succeed — mirror ``newroz update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            ...
                            Write-Warn "Review git diff / git status if Newroz behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running newroz processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/newroz-agent/newroz-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/newroz-agent/newroz-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/newroz-agent/newroz-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\newroz-agent-$zipLabel.zip"
    ...
    # next `newroz update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Newroz-Setup.exe
    ...
    if (Test-Path "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running newroz process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running newroz processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Newroz_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (newroz.exe) plus its child tree.
            & taskkill /F /T /IM newroz.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM newroz.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m newroz_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not newroz.exe.
            ...
            # user (see newroz_cli/gateway_windows.py), so the installer always
    ...
    if (Test-Path $venvPythonExe) {
        ...
        # the task fires normally on next logon and `newroz update` / the
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `newroz.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install newroz-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and newroz.exe broken with
    ...
    # here, not 30 seconds later when the user runs `newroz`.
    if (-not $NoVenv) {
    ...
    if (-not $NoVenv) {
        # uv on Windows can register newroz.exe in dist-info/RECORD but fail to
        ...
        # `newroz` command while newroz-agent.exe / newroz-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m newroz_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `newroz dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\newroz_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `newroz dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `newroz dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: newroz_cli/web_server.py"
        }
    }
...
function Set-PathVariable {
    Write-Info "Setting up newroz command..."
    
    if ($NoVenv) {
        $newrozBin = "$InstallDir"
    } else {
        $newrozBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so newroz is globally available
    # On Windows, the newroz.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$newrozBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$newrozBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $newrozBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set NEWROZ_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\newroz instead
    # of the Unix default ~/.newroz
    $currentNewrozHome = [Environment]::GetEnvironmentVariable("NEWROZ_HOME", "User")
    if (-not $currentNewrozHome -or $currentNewrozHome -ne $NewrozHome) {
        [Environment]::SetEnvironmentVariable("NEWROZ_HOME", $NewrozHome, "User")
        Write-Success "Set NEWROZ_HOME=$NewrozHome"
    }
    $env:NEWROZ_HOME = $NewrozHome
    
    # Update current session
    $env:Path = "$newrozBin;$env:Path"
    
    Write-Success "newroz command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.newroz-bootstrap-complete which tells the Newroz
    ...
    # Newroz-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `newroz update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".newroz-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the NEWROZ_HOME directory structure ($NewrozHome, default %LOCALAPPDATA%\newroz)
    New-Item -ItemType Directory -Force -Path "$NewrozHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NewrozHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NewrozHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NewrozHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NewrozHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NewrozHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NewrozHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NewrozHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NewrozHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$NewrozHome\.env"
    ...
    # Create config.yaml
    $configPath = "$NewrozHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Newroz's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$NewrozHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in newroz_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $NewrozHome"
    
    # Seed bundled skills into $NewrozHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $NewrozHome\skills ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to $NewrozHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$NewrozHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $NewrozHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Newroz-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'newroz setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\newroz-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\newroz-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
...
# the final `electron` -> `Newroz` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Newroz.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_newroz_desktop command
    # resolves apps/desktop/release/win-unpacked/Newroz.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Newroz-managed
    ...
    # The --dir mode produces an unpacked Newroz.exe in
    ...
    # are NOT trying to work around). The Newroz icon + product name are
    # stamped onto Newroz.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\newroz-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Newroz.exe",
        "$desktopDir\release\win-arm64-unpacked\Newroz.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Newroz.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Newroz icon + identity are stamped onto Newroz.exe by the
    ...
    #    Newroz.exe. We deliberately do NOT point them at `newroz desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Newroz.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Newroz.lnk')
        )
        ...
        # with the Newroz icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.newroz/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'newroz setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m newroz_cli.main setup
    } else {
        python -m newroz_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $newrozCmd = "$InstallDir\venv\Scripts\newroz.exe"
    if (-not (Test-Path $newrozCmd)) {
        $newrozCmd = "newroz"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$NewrozHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'newroz whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: newroz gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$NewrozHome\logs\gateway.log"
            Start-Process -FilePath $newrozCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$NewrozHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: newroz gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: newroz gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$NewrozHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$NewrozHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$NewrozHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$NewrozHome\newroz-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   newroz              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   newroz setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   newroz config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   newroz config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   newroz gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   newroz update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Newroz repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Newroz-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Newroz to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://newroz-agent.vercel.app/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
