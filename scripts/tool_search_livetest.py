#!/usr/bin/env python3
"""Live test harness for Newroz Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any newroz imports.
ORIGINAL_HOME = os.environ.get("NEWROZ_HOME")
ORIGINAL_AUTH = Path.home() / ".newroz" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.newroz/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.newroz/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="newroz_ts_live_"))
    newroz_home = home_dir / ".newroz"
    newroz_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, newroz_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".newroz" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, newroz_home / ".env")
        ...
        load_newroz_dotenv(newroz_home=str(Path.home() / ".newroz"))
    ...
    (newroz_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return newroz_home
...
def reset_module_state():
    """Drop cached modules so the new NEWROZ_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "newroz_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["NEWROZ_HOME"] = str(home)
...
def main():
    ...
    # Restore original NEWROZ_HOME
    if ORIGINAL_HOME is not None:
        os.environ["NEWROZ_HOME"] = ORIGINAL_HOME
    else:
