...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.NEWROZ_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Newroz on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via NEWROZ_TUI_TRUECOLOR=1.
