...
const logDir = join(process.env.NEWROZ_HOME?.trim() || join(homedir(), '.newroz'), 'logs')
...
// a real ~/.newroz (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('newroz-tui: parent lifecycle log unavailable\n')
    }
  }
}
