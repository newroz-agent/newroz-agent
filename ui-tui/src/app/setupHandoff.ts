import type { RunExternalProcess } from '@newroz/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`newroz ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching newroz: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`newroz ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
