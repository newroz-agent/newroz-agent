...
React + Ink terminal UI for Newroz. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `NEWROZ_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `newroz setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.newroz/.newroz_history` or under `NEWROZ_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `newroz setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/newroz-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external newroz setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.newroz/tui-parent.log
    ...
    types/
      newroz-ink.d.ts            type declarations for @newroz/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent NewrozCLI subprocess for slash commands
```