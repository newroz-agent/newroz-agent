...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/newroz-agent/newroz-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    newroz_meta = (fm.get("metadata") or {}).get("newroz") or {}
    tags = newroz_meta.get("tags") or []
    related = newroz_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `newroz skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Newroz Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Newroz ships with a large built-in skill library copied into `~/.newroz/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Newroz also syncs bundled skills on `newroz update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.newroz/skills/` tree, it is still shipped with Newroz; restore it with `newroz skills reset <name> --restore`.",
        "",
