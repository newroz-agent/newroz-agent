#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Newroz docs site.
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use Newroz as a Python Library", None),
        ("guides/use-mcp-with-newroz", "Use MCP with Newroz", None),
        ("guides/use-voice-mode-with-newroz", "Use Voice Mode with Newroz", None),
        ("guides/use-soul-with-newroz", "Use SOUL.md with Newroz", None),
        ("guides/build-a-newroz-plugin", "Build a Newroz Plugin", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with Newroz"),
...
def emit_llms_index() -> str:
    ...
    lines: list[str] = []
    lines.append("# Newroz Agent")
    lines.append("")
    lines.append(
        "> The self-improving AI agent. A terminal-native "
    ...
    lines.append(
        ...
        "newroz-agent/main/scripts/install.sh | bash`  "
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# Newroz Agent — Full Documentation\n",
        (
            "This file is the entire Newroz Agent documentation concatenated for LLM "
