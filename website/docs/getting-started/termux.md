...
description: "Run Newroz Agent directly on an Android phone with Termux"
---

# Newroz on Android with Termux
...
Newroz Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Newroz from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
- links `newroz` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `newroz` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/newroz" "$PREFIX/bin/newroz"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `newroz` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.newroz/.env`.
...
### `newroz doctor` says ripgrep or Node is missing
