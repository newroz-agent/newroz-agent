...
Newroz fetches curated model lists for **OpenRouter** and **Nous Portal** from a JSON manifest hosted alongside the docs site. This lets maintainers update picker lists without shipping a new `newroz-agent` release.

When the manifest is unreachable (offline, network blocked, hosting failure), Newroz silently falls back to the in-repo snapshot that ships with the CLI. The manifest never breaks the picker — worst case you see whatever list was bundled with your installed version.
...
- **`version`** — integer schema version. Future schemas bump this; Newroz refuses manifests with versions it doesn't understand and falls back to the hardcoded snapshot.
- **`metadata`** — free-form dict at the manifest, provider, and model level. Any keys. Newroz ignores unknown fields, so you can annotate entries (`"tier": "paid"`, `"tags": [...]`, etc.) without coordinating a schema change.
...
| When | What happens |
|---|---|
| `/model` or `newroz model` | Fetches if disk cache is stale, else uses cache |
...
Cache location: `~/.newroz/cache/model_catalog.json`.
...
model_catalog:
  enabled: true
  url: https://newroz-agent.github.io/docs/api/model-catalog.json
  ttl_hours: 1
  providers: {}
```
...
# editing OPENROUTER_MODELS or _PROVIDER_MODELS["nous"] in newroz_cli/models.py).
