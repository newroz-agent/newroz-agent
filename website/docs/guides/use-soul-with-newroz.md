---
sidebar_position: 7
title: "Use SOUL.md with Newroz"
description: "How to use SOUL.md to shape Newroz Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Newroz

`SOUL.md` is the **primary identity** for your Newroz instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Newroz to feel like the same assistant every time you talk to it — or if you want to replace the Newroz persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Newroz should be
- what Newroz should avoid stylistically
- how Newroz should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Newroz is and how Newroz speaks
...
Newroz now uses only the global SOUL file for the current instance:

```text
~/.newroz/SOUL.md
```

If you run Newroz with a custom home directory, it becomes:
...
Newroz automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Newroz does not overwrite it
- if the file exists but is empty, Newroz adds nothing from it to the prompt

## How Newroz uses it

When Newroz starts a session, it reads `SOUL.md` from `NEWROZ_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Newroz feels.
...
Newroz already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Newroz should sound.

# Avoid
What Newroz should not do.

# Defaults
How Newroz should behave when ambiguity appears.
```
...
Then restart Newroz or start a new session.
...
### I edited SOUL.md but Newroz still sounds the same

Check:
- you edited `~/.newroz/SOUL.md` or `$NEWROZ_HOME/SOUL.md`
...
### Newroz is ignoring parts of my SOUL.md
