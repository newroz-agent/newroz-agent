---
sidebar_position: 6
title: "Use MCP with Newroz"
description: "A practical guide to connecting MCP servers to Newroz Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Newroz

This guide shows how to actually use MCP with Newroz Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Newroz tool
- you want Newroz to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Newroz to internal APIs, databases, or company systems without modifying Newroz core

Do not use MCP when:
- a built-in Newroz tool already solves the job well
...
- Newroz remains the agent
- MCP servers contribute tools
- Newroz discovers those tools at startup or reload time
...
If you installed Newroz with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Newroz banner/status should show MCP integration when configured
- ask Newroz what tools it has available
...
## WSL2: bridge Newroz in WSL to Windows Chrome

This is the practical setup when:

- Newroz runs inside WSL2
...
In this setup, Newroz does **not** connect to Chrome directly. Instead:

- Newroz runs in WSL
- Newroz starts a local stdio MCP server
...
```text
Newroz (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Newroz stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Newroz core browser transport
...
```bash
newroz mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
newroz mcp test chrome-devtools-win
```

Then start a fresh Newroz session or run:
...
Once loaded, Newroz can use the MCP-prefixed browser tools directly. For example:
...
If Newroz runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Newroz from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Newroz from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Newroz:
...
2. Newroz-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Newroz will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Newroz to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want Newroz to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. Newroz remains the agent; Open Scaffold remains the repo-local record.

Add the server for one scaffolded repository:

```bash
newroz mcp add open_scaffold --command npx --args -y open-scaffold@latest mcp serve --repo /absolute/path/to/repo
newroz mcp test open_scaffold
```

Then keep the exposed surface read-oriented. Choose `select` in the `newroz mcp add` prompt, or edit `config.yaml` afterward:
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want Newroz to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize Newroz to merge, publish, deploy, or spawn runtimes.
...
Now Newroz can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Newroz core.
...
Because Newroz now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
