...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Newroz Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Newroz instantly when PRs are opened or updated.
:::
...
- **Newroz Agent installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  newroz gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.newroz/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Newroz can access GitHub. Start a chat:
...
```
Run: gh pr list --repo newroz-agent/newroz-agent --state open --limit 3
```
...
Still in the chat, ask Newroz to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo newroz-agent/newroz-agent
```
...
A skill gives Newroz consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.newroz/skills/code-review
```

Create `~/.newroz/skills/code-review/SKILL.md`:
...
Verify it loaded — start `newroz` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Newroz your team's standards:
...
2. Teach Newroz your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
newroz gateway status    # Is the gateway running?
newroz cron list         # Is the job enabled?
```
