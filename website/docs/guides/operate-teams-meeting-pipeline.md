...
```bash
newroz teams-pipeline validate
```
...
```bash
newroz teams-pipeline token-health
newroz teams-pipeline token-health --force-refresh
```
...
```bash
newroz teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
newroz teams-pipeline maintain-subscriptions
newroz teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Newroz cron (recommended if you already run the Newroz gateway)

Newroz ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.newroz/scripts/`. First create the script:

```bash
mkdir -p ~/.newroz/scripts
cat > ~/.newroz/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec newroz teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.newroz/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
newroz cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/newroz-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Newroz Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=newroz
EnvironmentFile=/etc/newroz/env
ExecStart=/usr/local/bin/newroz teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/newroz-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Newroz Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now newroz-teams-pipeline-maintain.timer
systemctl list-timers newroz-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/newroz teams-pipeline maintain-subscriptions >> /var/log/newroz/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.newroz/.env` at the top of a wrapper script that crontab calls.
...
```bash
newroz teams-pipeline subscriptions   # should show expirationDateTime advanced
newroz teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
newroz teams-pipeline list
newroz teams-pipeline list --status failed
newroz teams-pipeline show <job-id>
```

### Replay a stored job

```bash
newroz teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
newroz teams-pipeline fetch --meeting-id <meeting-id>
newroz teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `newroz teams-pipeline maintain-subscriptions --dry-run`
- inspect `newroz teams-pipeline list --status failed`
...
- run `newroz teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `newroz teams-pipeline run`
...
- [ ] `newroz teams-pipeline validate` returns an OK snapshot
- [ ] `newroz teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Newroz cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
