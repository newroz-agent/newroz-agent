...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `newroz send`."
---

# Pipe Script Output to Messaging Platforms

`newroz send` is a small, scriptable CLI that pushes a message to any
messaging platform Newroz is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | newroz send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `newroz
...
newroz send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
newroz send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
newroz send --list

# Filter by platform
newroz send --list telegram
```
...
Any platform Newroz ships adapters for works as a target:
...
`newroz send` resolves the message body in this order:

1. **Positional argument** — `newroz send --to telegram "hi"`
2. **`--file PATH`** — `newroz send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | newroz send --to telegram`

When stdin is a TTY (no pipe), Newroz does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  newroz send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `newroz send` reuses your Newroz config, the same script works on
any host where Newroz is installed — no need to export bot tokens into
...
`newroz send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  newroz send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | newroz send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.newroz/bin/newroz send \
...
```bash
./train.py --epochs 200 && \
  newroz send --to telegram "training done" || \
  newroz send --to telegram "training failed (exit $?)"
```
...
newroz send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(newroz send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `newroz send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `newroz send` calls
the platform's REST endpoint directly using credentials from
`~/.newroz/.env` and `~/.newroz/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `newroz gateway start` and retry.
...
# Machine-readable
newroz send --list --json
```

The listing is built from `~/.newroz/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`newroz gateway
...
| Approach | Multi-platform | Reuses Newroz creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `newroz send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`newroz send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `newroz send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `newroz send` shares with cron delivery.
