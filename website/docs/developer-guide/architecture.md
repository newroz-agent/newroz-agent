...
sidebar_position: 1
title: "Architecture"
description: "Newroz Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Newroz Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ newroz_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # NewrozCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── newroz_state.py           # SQLite session/state database with FTS5
├── newroz_constants.py       # NEWROZ_HOME, profile-aware paths
...
│
├── newroz_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `newroz` subcommands (large file)
...
│   ├── skills_config.py      # newroz skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # newroz tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # newroz gateway start/stop
│
...
```text
User input → NewrozCLI.process_input()
...
Three discovery sources: `~/.newroz/plugins/` (user), `.newroz/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `newroz plugins` or `config.yaml`.
...
Exposes Newroz as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`newroz -p <name>`) gets its own NEWROZ_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
