...
Newroz Agent uses a SQLite database (`~/.newroz/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `newroz_state.py`


## Architecture Overview

```
~/.newroz/state.db (SQLite, WAL mode)
...
Multiple newroz processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.newroz/state.db
...
Default path: `~/.newroz/state.db`

This is derived from `newroz_constants.get_newroz_home()` which resolves to
`~/.newroz/` by default, or the value of `NEWROZ_HOME` environment variable.
