...
The ACP adapter wraps Newroz' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
newroz acp / newroz-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.newroz/.env
  -> configure stderr logging
  -> construct NewrozACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'newroz-agent[acp]==<version>' newroz-acp`, pointed at the `newroz-agent` PyPI release.
...
- `allow_once` -> Newroz `once`
- `allow_always` -> Newroz `always`
- reject options -> Newroz `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Newroz tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["newroz-acp"])
...
Instead it reuses Newroz' runtime resolver:

- `acp_adapter/auth.py`
- `newroz_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Newroz provider/credentials. It also always advertises a terminal setup auth method (`newroz-setup`, args `--setup`) so first-run registry clients can open Newroz' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.newroz/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `newroz-acp` toolset definition
- `newroz_cli/main.py` — `newroz acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `newroz-acp` script
