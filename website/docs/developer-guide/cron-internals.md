...
description: "How Newroz stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `newroz_cli/cron.py` | CLI `newroz cron` subcommands |
...
Jobs are stored in `~/.newroz/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `newroz cron` commands are run or during active CLI sessions.
...
```python
# ~/.newroz/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.newroz/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `newroz cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `newroz cron` CLI provides direct job management:

```bash
newroz cron list                    # Show all jobs
newroz cron create                  # Interactive job creation (alias: add)
newroz cron edit <job_id>           # Edit job configuration
newroz cron pause <job_id>          # Pause a running job
newroz cron resume <job_id>         # Resume a paused job
newroz cron run <job_id>            # Trigger immediate execution
newroz cron remove <job_id>         # Delete a job
```
