...
description: "Durable SQLite-backed task board for coordinating multiple Newroz profiles"
---
...
Newroz Kanban is a durable task board, shared across all your Newroz profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.newroz/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.

### Two surfaces: the model talks through tools, you talk through the CLI

The board has two front doors, both backed by the same `~/.newroz/kanban.db`:
...
- **You (and scripts, and cron) drive the board through `newroz kanban …`** on the CLI, `/kanban …` as a slash command, or the dashboard. These are for humans and automation — the places without a tool-calling model behind them.
...
For the full design rationale, comparative analysis against Cline Kanban / Paperclip / NanoClaw / Google Gemini Enterprise, and the eight canonical collaboration patterns, see `docs/newroz-kanban-v1-spec.pdf` in the repository.
...
- **Dispatcher** — a long-lived loop that, every N seconds (default 60): reclaims stale claims, reclaims crashed workers (PID gone but TTL not yet expired), promotes ready tasks, atomically claims, spawns assigned profiles. Runs **inside the gateway** by default (`kanban.dispatch_in_gateway: true`). One dispatcher sweeps all boards per tick; workers are spawned with `NEWROZ_KANBAN_BOARD` pinned so they can't see other boards. After `kanban.failure_limit` consecutive spawn failures on the same task (default: 2) the dispatcher auto-blocks it with the last error as the reason — prevents thrashing on tasks whose profile doesn't exist, workspace can't mount, etc.
...
newroz kanban boards list

# Create a new board.
newroz kanban boards create atm10-server \
    --name "ATM10 Server" \
...
newroz kanban --board atm10-server list
newroz kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
newroz kanban boards switch atm10-server
newroz kanban boards show             # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
newroz kanban boards rename atm10-server "ATM10 (Prod)"
...
2. `NEWROZ_KANBAN_BOARD` env var (set by the dispatcher when spawning a
   worker, so workers can't see other boards).
3. `~/.newroz/kanban/current` — the slug persisted by `newroz kanban
   boards switch`.
4. `default`.
...
`newroz dashboard` → Kanban tab shows a board switcher at the top as soon
...
- **Storage** — files land under
  `<newroz-home>/kanban/attachments/<task_id>/` for the default board, or
  `<newroz-home>/kanban/boards/<slug>/attachments/<task_id>/` for a named
  board. Set `NEWROZ_KANBAN_ATTACHMENTS_ROOT` to pin a custom location.
...
newroz kanban create "research AI funding landscape" --assignee researcher
...
When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `newroz kanban show t_abcd`.
...
Override the config flag at runtime via `NEWROZ_KANBAN_DISPATCH_IN_GATEWAY=0`
for debugging. Standard gateway supervision applies: run `newroz gateway
...
until one comes up — `newroz kanban create` warns about this at creation
time.

Running `newroz kanban daemon` as a separate process is **deprecated**;
...
newroz kanban create "nightly ops review" \
    --assignee ops \
...
```bash
newroz kanban complete t_abc t_def t_hij --result "batch wrap"
newroz kanban archive  t_abc t_def t_hij
newroz kanban unblock  t_abc t_def
newroz kanban block    t_abc "need input" --ids t_def t_hij
```

## How workers interact with the board

**Workers do not shell out to `newroz kanban`.** When the dispatcher spawns a worker it sets `NEWROZ_KANBAN_TASK=t_abcd` in the child's env, and that env var flips on a dedicated **kanban toolset** in the model's schema. The same toolset is also available to orchestrator profiles that enable `kanban` in their toolsets config. These tools read and mutate the board directly via the Python `kanban_db` layer, same as the CLI does. A running worker calls these like any other tool; it never sees or needs the `newroz kanban` CLI.
...
kanban_show()                                     # no args — uses NEWROZ_KANBAN_TASK
...
### Why tools instead of shelling to `newroz kanban`

Three reasons:

1. **Backend portability.** Workers whose terminal tool points at a remote backend (Docker / Modal / Singularity / SSH) would run `newroz kanban complete` *inside* the container, where `newroz` isn't installed and `~/.newroz/kanban.db` isn't mounted. The kanban tools run in the agent's own Python process and always reach `~/.newroz/kanban.db` regardless of terminal backend.
...
**Zero schema footprint on normal sessions.** A regular `newroz chat` session has zero `kanban_*` tools in its schema unless the active profile explicitly enables the `kanban` toolset for orchestrator work. Dispatcher-spawned task workers get task-scoped tools because `NEWROZ_KANBAN_TASK` is set; orchestrator profiles get the broader routing surface through config. No tool bloat for users who never touch kanban.
...
```json
{
  ...
  "verification": ["pytest tests/newroz_cli/test_kanban_db.py -q"],
...
2. `cd $NEWROZ_KANBAN_WORKSPACE` (via the terminal tool) and do the work there.
...
```bash
newroz kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

newroz kanban create "audit auth flow" \
    --assignee reviewer \
...
The dispatcher emits one `--skills <name>` flag per skill listed, so the worker spawns with all of them loaded on top of the auto-injected kanban guidance. The skill names must match skills that are actually installed on the assignee's profile (run `newroz skills list` to see what's available); there's no runtime install.
...
```bash
newroz kanban create "Translate the docs site to French" \
...
**Manual** — `kanban.auto_decompose: false`. Triage tasks stay in triage until you act. Click the **⚗ Decompose** button on a card, run `newroz kanban decompose <id>` (or `--all`), or use `/kanban decompose <id>` from a chat. This matches the pre-decomposer behavior of the board, useful when you want full control over what runs when.

Flip between the two modes from the **Orchestration: Auto/Manual** pill at the top of the kanban page (emerald = Auto, muted gray = Manual), or by editing `config.yaml` directly. Both modes coexist with `newroz kanban specify` — that's still available as a single-task spec rewrite when you don't want fan-out.

The decomposer's routing decisions depend on profile descriptions, which is a per-profile labeling primitive you set with `newroz profile create --description "..."`, `newroz profile describe <name> --text "..."`, `newroz profile describe <name> --auto` (LLM-generates from the profile's installed skills + model), or the dashboard's per-profile editor in the expanded **Orchestration settings** panel. Profiles without a description still appear in the roster — they're routable by name, just less precisely. The decomposer NEVER lands a child task with `assignee=None`: when the LLM picks an unknown profile, the child gets routed to `kanban.default_assignee` (or the active default profile if that's unset).
...
Config knobs (all under `kanban:` in `~/.newroz/config.yaml`):
...
| `auxiliary.profile_describer` | Model that auto-generates profile descriptions (called by `newroz profile describe --auto`). |
...
└──────────┬─────────────┘                                    │
           ...
           ▼                                                  │
┌────────────────────────┐                                    │
│  ~/.newroz/kanban.db   │ ───── append task_events ──────────┘
│  (WAL, shared)         │
└────────────────────────┘
```
...
The plugin uses the standard Newroz dashboard plugin contract — see [Extending the Dashboard](./extending-the-dashboard) for the full manifest reference, shell slots, page-scoped slots, and the Plugin SDK. Extra columns, custom card chrome, tenant-filtered layouts, or full `tab.override` replacements are all expressible without forking this plugin.
...
```
newroz kanban init                                     # create kanban.db + print daemon hint
newroz kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
newroz kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived]
        ...
        [--json]
newroz kanban show <id> [--json]
newroz kanban assign <id> <profile>                    # or 'none' to unassign
newroz kanban reassign <id>... <profile>               # bulk re-assign tasks to a profile
newroz kanban edit <id> [--title ...] [--body ...]     # edit task title / body / priority in place
        [--priority N]
newroz kanban promote <id>...                          # move todo/blocked tasks to ready (recovery)
newroz kanban schedule <id> --at <ISO8601>             # set/clear a task's scheduled_at start time
newroz kanban diagnostics [--json]                     # board health snapshot (alias: diag)
newroz kanban link <parent_id> <child_id>
newroz kanban unlink <parent_id> <child_id>
newroz kanban claim <id> [--ttl SECONDS]
newroz kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
newroz kanban complete <id>... [--result "..."]
newroz kanban block <id> "<reason>" [--ids <id>...]
newroz kanban unblock <id>...
newroz kanban archive <id>...

newroz kanban tail <id>                                # follow a single task's event stream
newroz kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
newroz kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
newroz kanban runs <id> [--json]                       # attempt history (one row per run)
newroz kanban assignees [--json]                       # profiles on disk + per-assignee task counts
newroz kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
newroz kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `newroz gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
newroz kanban stats [--json]                           # per-status + per-assignee counts
newroz kanban log <id> [--tail BYTES]                  # worker log from ~/.newroz/kanban/logs/
newroz kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
newroz kanban notify-list [<id>] [--json]
newroz kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
newroz kanban context <id>                             # what a worker sees
newroz kanban specify [<id> | --all] [--tenant T]      # flesh out a triage-column idea
        [--author NAME] [--json]                       #   into a full spec and promote to todo
newroz kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
```bash
newroz kanban create "nightly backup audit" \
...
Every `newroz kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `newroz chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `newroz_cli.kanban.run_slash()` entry point that reuses the `newroz kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `newroz kanban`. You don't have to leave the chat to drive the board.
...
The gateway normally queues slash commands and user messages while an agent is still thinking — that's what stops you from accidentally starting a second turn while the first is in flight. **`/kanban` is explicitly exempted from this guard.** The board lives in `~/.newroz/kanban.db`, not in the running agent's state, so reads (`list`, `show`, `context`, `tail`, `watch`, `stats`, `runs`) and writes (`comment`, `unblock`, `block`, `assign`, `archive`, `create`, `link`, …) all go through immediately, even mid-turn.
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`newroz kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
| **P9 Triage specifier** | rough idea → `triage` → `newroz kanban specify` expands body → `todo` | "turn this one-liner into a spec'd task" |

For worked examples of each, see `docs/newroz-kanban-v1-spec.pdf`.
...
```bash
newroz kanban create "monthly report" \
    --assignee researcher \
    --tenant business-a \
    --workspace dir:~/tenants/business-a/data/
```

Workers receive `$NEWROZ_TENANT` and namespace their memory writes by prefix. The board, the dispatcher, and the profile definitions are all shared; only the data is scoped.
...
```bash
newroz kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
newroz kanban notify-list
newroz kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
```bash
newroz kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
newroz kanban runs t_abcd
...
**Bulk close caveat.** `newroz kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`newroz kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `newroz kanban heartbeat $TASK` to signal liveness during long operations. |
...
`newroz kanban tail <id>` shows these for a single task. `newroz kanban watch` streams them board-wide.

## Out of scope

Kanban is deliberately single-host. `~/.newroz/kanban.db` is a local SQLite file and the dispatcher spawns workers on the same machine. Running a shared board across two hosts is not supported — there's no coordination primitive for "worker X on host A, worker Y on host B," and the crash-detection path assumes PIDs are host-local. If you need multi-host, run an independent board per host and use `delegate_task` / a message queue to bridge them.

## Design spec

The complete design — architecture, concurrency correctness, comparison with other systems, implementation plan, risks, open questions — lives in `docs/newroz-kanban-v1-spec.pdf`. Read that before filing any behavior-change PR.
