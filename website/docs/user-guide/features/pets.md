...
Newroz can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<NEWROZ_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Newroz renders the sprite at full fidelity when your
...
newroz pets list
newroz pets list cat

# Install a pet and make it active in one step
newroz pets install boba --select
...
## `newroz pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `newroz pets list [query] [--limit N]` |
| List installed pets | `newroz pets list --installed` |
| Install a pet | `newroz pets install <slug> [--select] [--force]` |
...
| Preview/animate | `newroz pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `newroz pets off` |
| Remove an installed pet | `newroz pets remove <slug>` |
| Diagnose setup | `newroz pets doctor` |

`newroz pets show` flags:
...
Beyond installing pre-made pets from the gallery, Newroz can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `newroz pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `newroz tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `NEWROZ_PET_IMAGE_PROVIDER` env var (e.g. `NEWROZ_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Newroz is
...
Run `newroz pets doctor` — it reports:
