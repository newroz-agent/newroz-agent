...
description: "Plugins shipped with Newroz Agent that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

Newroz ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.newroz/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a Newroz Plugin](/developer-guide/plugins) to write your own.
...
2. **User** — `~/.newroz/plugins/<name>/`
3. **Project** — `./.newroz/plugins/<name>/` (requires `NEWROZ_ENABLE_PROJECT_PLUGINS=1`)
4. **Pip entry points** — `newroz_agent.plugins`

On name collision, later sources win — a user plugin named `disk-cleanup` would replace the bundled one.

`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `newroz memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `newroz plugins list` and the interactive `newroz plugins` UI), but none load until you explicitly enable them:

```bash
newroz plugins enable disk-cleanup
```

Or via `~/.newroz/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Newroz. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
newroz plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `newroz plugins enable <name>`.
...
| `newroz-achievements` | dashboard tab | Steam-style collectible badges generated from your real Newroz session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `newroz memory` and `newroz plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `NEWROZ_HOME` or `/tmp/newroz-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under NEWROZ_HOME | always | Never |
...
**State** — everything lives at `$NEWROZ_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `NEWROZ_HOME` or `/tmp/newroz-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `newroz plugins enable disk-cleanup` (or check the box in `newroz plugins`).

**Disabling again:** `newroz plugins disable disk-cleanup`.
...
**Enabling:** `newroz plugins enable security-guidance` (or check the box in `newroz plugins`).

**Disabling again:** `newroz plugins disable security-guidance`.
...
Traces Newroz turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of Newroz' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `newroz logs`.
...
```bash
newroz tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart Newroz and the next turn ships a trace.
...
Then put the credentials in `~/.newroz/.env`:
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "Newroz turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `NEWROZ_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the Newroz session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `newroz chat` session lives under one Langfuse session.

**Verify:**

```bash
newroz plugins list                 # observability/langfuse should show "enabled"
newroz chat -q "hello"              # check the Langfuse UI for a "Newroz turn" trace
```
...
| `NEWROZ_LANGFUSE_RELEASE` | — | Release/version tag |
| `NEWROZ_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `NEWROZ_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `NEWROZ_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

Newroz-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — Newroz-prefixed wins when both are set.
...
**Disabling:** `newroz plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
- Post-meeting artifacts (transcript, speaker-attributed notes, action items) saved under `~/.newroz/cache/google_meet/<meeting_id>/`

**Setup:**

```bash
newroz plugins enable google_meet
...
**Disabling:** `newroz plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.newroz/cache/google_meet/` until you remove them.

### newroz-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real Newroz session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with Newroz feature changes.

**How it works:**

- Scans your entire `~/.newroz/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$NEWROZ_HOME/plugins/newroz-achievements/state.json`
...
| Secret | Hidden until Newroz detects the first related signal in your history |

**API** — routes mount under `/api/plugins/newroz-achievements/`:
...
**State files** — live under `$NEWROZ_HOME/plugins/newroz-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Newroz updates. |
...
**Enabling:** Nothing to enable — `newroz-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `newroz dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/newroz-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.newroz/plugins/newroz-achievements/` that ships no dashboard. The plugin's state files under `$NEWROZ_HOME/plugins/newroz-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other Newroz plugin — see [Build a Newroz Plugin](/developer-guide/plugins). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.newroz/plugins/<name>/`
- Manifest source is reported as `bundled` in `newroz plugins list`
