...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Newroz's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Newroz instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
newroz memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `newroz` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Newroz syncs them back at session init.
...
The `newroz honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `newroz memory setup honcho` (or run `newroz memory setup` and pick it from the list); the `newroz honcho` subcommand then appears on the next invocation.

```bash
newroz memory setup honcho    # Configure Honcho directly (works before activation)
newroz honcho status          # Connection status, config, and key settings
newroz honcho setup           # Redirects to `newroz memory setup` (post-activation alias)
newroz honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
newroz honcho peer            # Show or update peer names + dialectic reasoning level
newroz honcho mode            # Show or set recall mode (hybrid/context/tools)
newroz honcho tokens          # Show or set token budget for context and dialectic
newroz honcho identity        # Seed or show the AI peer's Honcho identity
newroz honcho sync            # Sync Honcho config to all existing profiles
newroz honcho peers           # Show peer identities across all profiles
newroz honcho sessions        # List known Honcho session mappings
newroz honcho map             # Map current directory to a Honcho session name
newroz honcho enable          # Enable Honcho for the active profile
newroz honcho disable         # Disable Honcho for the active profile
newroz honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `newroz honcho`

If you previously used the standalone `newroz honcho setup`:
...
No re-login or re-setup needed. Run `newroz memory setup` and select "honcho" — the wizard detects your existing config.
