...
chat completions — use your Newroz-managed provider subscription as their
...
| Use case | "Use Newroz as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Newroz stores
the refresh token in `~/.newroz/auth.json` — the same place all Newroz
...
interface in `newroz_cli/proxy/adapters/`.
...
If you see `not logged in`, run `newroz portal`. If you see
...
`newroz_cli/proxy/adapters/<provider>.py` and registering it in
