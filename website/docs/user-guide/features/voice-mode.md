...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Newroz Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Newroz Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Newroz](/guides/use-voice-mode-with-newroz).

## Prerequisites

Before using voice features, make sure you have:

1. **Newroz Agent installed** — via the install script (see [Installation](/getting-started/installation))
2. **An LLM provider configured** — run `newroz model` or set your preferred provider credentials in `~/.newroz/.env`
3. **A working base setup** — run `newroz` to verify the agent responds to text before enabling voice

:::tip
The `~/.newroz/` directory and default `config.yaml` are created automatically the first time you run `newroz`. You only need to create `~/.newroz/.env` manually for API keys.
:::

:::tip Nous Portal covers both
A paid [Nous Portal](/user-guide/features/tool-gateway) subscription supplies the LLM (step 2) **and** OpenAI TTS via the Tool Gateway — no separate OpenAI key needed. On a fresh install, `newroz setup --portal` wires both up at once.
:::
...
cd ~/.newroz/newroz-agent && uv pip install -e ".[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
cd ~/.newroz/newroz-agent && uv pip install -e ".[messaging]"

# Premium TTS (ElevenLabs)
cd ~/.newroz/newroz-agent && uv pip install -e ".[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
cd ~/.newroz/newroz-agent && uv pip install -e ".[all]"
```
...
Voice mode is available in both the **classic CLI** (`newroz chat`) and the **TUI** (`newroz --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.newroz/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
newroz                # Start the interactive CLI
```
...
1. Start the CLI with `newroz` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.newroz/config.yaml` (default: `ctrl+b`).
:::
...
```bash
newroz gateway        # Start the gateway (connects to configured platforms)
newroz gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@newrozbyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.newroz/.env`:
```bash
...
```bash
newroz gateway        # Start with existing configuration
```
...
If you are running Newroz inside Docker on a Linux desktop, the container also needs access to your host audio socket. See the [Docker audio bridge](/user-guide/docker#optional-linux-desktop-audio-bridge) notes for a PulseAudio/PipeWire-compatible setup.
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.newroz/.env`
...
- Review gateway logs: `tail -f ~/.newroz/logs/gateway.log`
