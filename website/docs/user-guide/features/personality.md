...
If the file is empty, whitespace-only, or cannot be read, Newroz falls back to a built-in default identity ("You are Newroz Agent, an intelligent AI assistant..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
