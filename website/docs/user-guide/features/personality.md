...
description: "Customize Newroz Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Newroz Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `NEWROZ_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Newroz is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Newroz now seeds a default `SOUL.md` automatically in:

```text
~/.newroz/SOUL.md
```

More precisely, it uses the current instance's `NEWROZ_HOME`, so if you run Newroz with a custom home directory, it will use:
...
- Newroz creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Newroz loads `SOUL.md` only from `NEWROZ_HOME`
- Newroz does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Newroz falls back to a built-in default identity
...
If Newroz loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `NEWROZ_HOME`, the personality belongs to the Newroz instance itself.

That also makes it easier to teach users:
- "Edit `~/.newroz/SOUL.md` to change Newroz' default personality."
...
- how Newroz should handle uncertainty, disagreement, or ambiguity
...
## What Newroz injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Newroz falls back to a built-in default identity ("You are Newroz Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Newroz ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Newroz, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Newroz its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.newroz/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.newroz/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-newroz)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Newroz speaks
- `display.skin` and `/skin` affect how Newroz looks in the terminal
