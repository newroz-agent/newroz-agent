...
When Newroz Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.newroz/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.newroz/config.yaml` (switchable per session
via `/personality`).
...
If you use Newroz' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.newroz/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Newroz Agent's deliverable
