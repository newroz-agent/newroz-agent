...
title: "Git Worktrees"
description: "Run multiple Newroz agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Newroz Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Newroz so each session has a clean, isolated working directory.

## Why Use Worktrees with Newroz?

Newroz treats the **current working directory** as the project root:

- CLI: the directory where you run `newroz` or `newroz chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.newroz/config.yaml`
...
git worktree add ../repo-feature feature/newroz-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/newroz-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Newroz there:

```bash
cd ../repo-feature

# Start Newroz in the worktree
newroz
```
...
git worktree add ../repo-experiment-a feature/newroz-a
git worktree add ../repo-experiment-b feature/newroz-b
```
...
- Works on its own branch (`feature/newroz-a` vs `feature/newroz-b`).
...
- Newroz checkpoint data under `~/.newroz/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Newroz experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/newroz-checkpoints-docs`, `feature/newroz-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Newroz from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `newroz -w` (Automatic Worktree Mode)

Newroz has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `newroz/newroz-<hash>`).
...
```bash
newroz -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `newroz -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Newroz session its own clean checkout.
