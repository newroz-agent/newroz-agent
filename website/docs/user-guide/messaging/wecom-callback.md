...
Connect Newroz to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Newroz supports two WeCom integration modes:
...
> Run `newroz gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Newroz decrypts the message, queues it for the agent
...
(Use `newroz gateway start` only after `newroz gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Newroz and the token the
...
in `~/.newroz/.env` values around `=` will also break signature checks. After
fixing, restart `newroz gateway run`.
...
Check `newroz gateway run` logs for the bound host/port. If the adapter bound to
