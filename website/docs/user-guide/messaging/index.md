...
description: "Chat with Newroz from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Newroz from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Newroz](/guides/use-voice-mode-with-newroz).
...
```mermaid
flowchart TB
    subgraph Gateway["Newroz Gateway"]
...
For group chats, hooks, and automation flows, Newroz supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Newroz keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Newroz does not hide failures just because the text resembles a silence token.
...
```bash
newroz gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
newroz gateway              # Run in foreground
newroz gateway setup        # Configure messaging platforms interactively
newroz gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo newroz gateway install --system   # Linux only: install a boot-time system service
newroz gateway start        # Start the default service
newroz gateway stop         # Stop the default service
newroz gateway status       # Check default service status
newroz gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Newroz Agent to the latest version |
...
Configure per-platform overrides in `~/.newroz/gateway.json`:
...
newroz pairing approve telegram XKGH5N7P

# Other pairing commands:
newroz pairing list          # View pending + approved users
newroz pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Newroz appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.newroz/config.yaml`:
...
Off by default. When enabled, Newroz prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.newroz/config.yaml`:
...
```bash
NEWROZ_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
newroz gateway install               # Install as user service
newroz gateway start                 # Start the service
newroz gateway stop                  # Stop the service
newroz gateway status                # Check status
journalctl --user -u newroz-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo newroz gateway install --system
sudo newroz gateway start --system
sudo newroz gateway status --system
journalctl -u newroz-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit Newroz installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit newroz-gateway` (or `sudo systemctl edit newroz-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `newroz update`. When `newroz update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart newroz-gateway` command (it never blocks on an interactive password prompt).
...
```bash
newroz gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `newroz update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo newroz update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/newroz-gateway`:

```
newroz ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed newroz-gateway*, /usr/bin/systemctl --no-ask-password start newroz-gateway*, /usr/bin/systemctl --no-ask-password restart newroz-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Newroz will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Newroz installations on the same machine (with different `NEWROZ_HOME` directories), each gets its own systemd service name. The default `~/.newroz` uses `newroz-gateway`; other installations use `newroz-gateway-<hash>`. The `newroz gateway` commands automatically target the correct service for your current `NEWROZ_HOME`.
:::

### macOS (launchd)

```bash
newroz gateway install               # Install as launchd agent
newroz gateway start                 # Start the service
newroz gateway stop                  # Stop the service
newroz gateway status                # Check status
tail -f ~/.newroz/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.newroz.gateway.plist`. It includes three environment variables:
...
- **NEWROZ_HOME** — scopes the gateway to your Newroz installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `newroz gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `NEWROZ_HOME` directory gets its own launchd label. The default `~/.newroz` uses `ai.newroz.gateway`; other installations use `ai.newroz.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `newroz-cli` | Full access |
| Telegram | `newroz-telegram` | Full tools including terminal |
| Discord | `newroz-discord` | Full tools including terminal |
| WhatsApp | `newroz-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `newroz-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `newroz-slack` | Full tools including terminal |
| Google Chat | `newroz-google_chat` | Full tools including terminal |
| Signal | `newroz-signal` | Full tools including terminal |
| SMS | `newroz-sms` | Full tools including terminal |
| Email | `newroz-email` | Full tools including terminal |
| Home Assistant | `newroz-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `newroz-mattermost` | Full tools including terminal |
| Matrix | `newroz-matrix` | Full tools including terminal |
| DingTalk | `newroz-dingtalk` | Full tools including terminal |
| Feishu/Lark | `newroz-feishu` | Full tools including terminal |
| WeCom | `newroz-wecom` | Full tools including terminal |
| WeCom Callback | `newroz-wecom-callback` | Full tools including terminal |
| Weixin | `newroz-weixin` | Full tools including terminal |
| BlueBubbles | `newroz-bluebubbles` | Full tools including terminal |
| QQBot | `newroz-qqbot` | Full tools including terminal |
| Yuanbao | `newroz-yuanbao` | Full tools including terminal |
| Microsoft Teams | `newroz-teams` | Full tools including terminal |
| API Server | `newroz-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `newroz-webhook` | Full tools including terminal |
| Raft | `newroz-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
1. **Gateway log** (`~/.newroz/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
