...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Newroz Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Newroz to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.newroz/.env`:
...
All standard Newroz commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.newroz/logs/gateway.log`
...
4. Consider enabling verbose logging: `NEWROZ_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Newroz automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
NEWROZ_LOG_LEVEL=debug newroz gateway
```
...
```bash
newroz chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
