...
Connect Newroz to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Newroz — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Newroz runs it in a
...
newroz photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Newroz Agent` project on your account.
...
Runtime credentials are written to `~/.newroz/.env`
...
(device token, dashboard project id) lives in `~/.newroz/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Newroz
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Newroz replies with a pairing code. Approve it with:

```bash
newroz pairing approve photon <CODE>
```

Use `newroz pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.newroz/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.newroz/.env`):
...
By default Newroz responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Newroz` and
`@Newroz agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Newroz will reply.

## Status & troubleshooting

```bash
newroz photon status
```

Prints saved credentials, sidecar health, your registered number, and the
assigned iMessage line Newroz uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run newroz photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `newroz photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `newroz photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `newroz photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Newroz sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Newroz wake words  | JSON list / comma / newline regex patterns for group mentions |
