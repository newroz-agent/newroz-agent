...
If you only run one Newroz agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more Newroz agents that should all
...
(`ai.newroz.gateway-<name>.plist`) or systemd user service
(`newroz-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
newroz profile create coder
newroz profile create personal-bot
newroz profile create research
...
```bash
newroz config set gateway.multiplex_profiles true
newroz gateway restart
```

Equivalently, in the default profile's `~/.newroz/config.yaml`:
...
You do **not** run `newroz gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `newroz gateway start` / `run` is a
...
home). `newroz status` reports the multiplexer and the profiles it serves;
`newroz status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/newroz-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: newroz-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    newroz gateway "$action"
  else
    newroz -p "$profile" gateway "$action"
  fi
}
...
```bash
newroz-gateways start      # start every configured profile
newroz-gateways stop       # stop every configured profile
newroz-gateways restart    # restart all
newroz-gateways status     # status across all
newroz-gateways list       # delegates to `newroz gateway list`
```

:::tip
The `default` profile is targeted with `newroz gateway <action>` (no `-p`),
not `newroz -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `newroz -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.newroz.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/newroz-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.newroz.gateway.plist` /
`newroz-gateway.service`.
...
```bash
# Default profile
tail -f ~/.newroz/logs/gateway.log
tail -f ~/.newroz/logs/gateway.error.log

# Named profile
tail -f ~/.newroz/profiles/<name>/logs/gateway.log
tail -f ~/.newroz/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.newroz/logs/gateway.log ~/.newroz/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
newroz logs -f                  # follow default profile
newroz -p coder logs -f         # follow one profile
newroz logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
newroz profile list             # profiles + model + gateway state
newroz-gateways status          # full status across every profile
launchctl list | grep newroz    # macOS — PIDs and labels
systemctl --user list-units 'newroz-gateway-*'   # Linux — units
```
...
```
~/.newroz/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.newroz/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
newroz config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.newroz/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=newroz --why="gateways running" \
  sleep infinity &
...
`newroz-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.newroz/.env ~/.newroz/profiles/*/.env
```

## Updating the code

`newroz update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `newroz gateway start` after a previous `newroz gateway stop`. The
...
```bash
ps -ef | grep "newroz_cli.*-p <profile>"
cat ~/.newroz/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.newroz.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.newroz.gateway-<profile>.plist

# Linux
systemctl --user restart newroz-gateway-<profile>.service
```

### Health check

```bash
newroz doctor                  # default profile
newroz -p <profile> doctor     # one profile
```
