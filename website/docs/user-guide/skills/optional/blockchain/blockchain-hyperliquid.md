...
| | |
|---|---|
| Source | Optional — install with `newroz skills install official/blockchain/hyperliquid` |
| Path | `optional-skills/blockchain/hyperliquid` |
| Version | `0.1.0` |
| Author | Hugo Sequier (Hugo-SEQUIER), Newroz Agent |
| License | MIT |
...
The script reads `${NEWROZ_HOME:-~/.newroz}/.env` for two optional defaults:
...
Helper script: `~/.newroz/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `${NEWROZ_HOME:-~/.newroz}/.env`.
