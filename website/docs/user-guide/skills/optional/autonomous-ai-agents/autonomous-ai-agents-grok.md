...
| | |
|---|---|
| Source | Optional — install with `newroz skills install official/autonomous-ai-agents/grok` |
| Path | `optional-skills/autonomous-ai-agents/grok` |
| Version | `0.1.0` |
| Author | Matt Maximo (MattMaximo), Newroz Agent |
| License | MIT |
...
# Grok Build CLI — Newroz Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the Newroz terminal. Grok
...
1. Prepare stable input files first with Newroz tools (`read_file`,
...
2. **Don't conflate Newroz' xAI auth with the `grok` CLI's auth.** Newroz'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Newroz' xAI auth carries
   over.
