...
description: "Configure and use Honcho memory with Newroz -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Newroz -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `newroz skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Newroz

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Newroz profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/newroz#running-honcho-locally-with-newroz

### Verify

```bash
newroz honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Newroz profile's AI peer
...
Honcho models conversations as interactions between **peers**. Newroz creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Newroz instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Newroz run |
| `global` | Single session across all directories |

Manual override: `newroz honcho map my-project-name`
...
Each Newroz profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block newroz.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `newroz.coder` host block in `honcho.json`
...
```bash
newroz honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Newroz when Honcho memory is active.
...
Config file: `$NEWROZ_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `newroz honcho setup`. Ensure `memory.provider: honcho` is in `~/.newroz/config.yaml`.

### Memory not persisting across sessions
Check `newroz honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `newroz profile create <name> --clone`. For existing profiles: `newroz honcho sync`.
...
| Command | Description |
|---------|-------------|
| `newroz honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `newroz honcho status` | Show resolved config, connection test, peer info for active profile |
| `newroz honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `newroz honcho disable` | Disable Honcho for the active profile |
| `newroz honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `newroz honcho peers` | Show peer identities across all profiles |
| `newroz honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `newroz honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `newroz honcho sessions` | List known directory-to-session-name mappings |
| `newroz honcho map <name>` | Map current working directory to a Honcho session name |
| `newroz honcho identity` | Seed AI peer identity or show both peer representations |
| `newroz honcho sync` | Create host blocks for all Newroz profiles that don't have one yet |
| `newroz honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Newroz + Honcho |
| `newroz memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `newroz memory status` | Show active memory provider and config |
| `newroz memory off` | Disable external memory provider |
