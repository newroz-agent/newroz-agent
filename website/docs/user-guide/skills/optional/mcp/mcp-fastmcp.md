...
| | |
|---|---|
| Source | Optional — install with `newroz skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Newroz or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Newroz. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.newroz/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.newroz/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Newroz integration, either:

- configure the server in `~/.newroz/config.yaml` using the `native-mcp` skill, or
...
### Newroz cannot see the deployed server

The server-building part may be correct while the Newroz config is not. Load the `native-mcp` skill and configure the server in `~/.newroz/config.yaml`, then restart Newroz.
