...
| | |
|---|---|
| Source | Optional — install with `newroz skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Newroz Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Newroz terminal calls (desktop app flow only)
...
### Service Account (recommended for Newroz)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${NEWROZ_HOME:-~/.newroz}/.env` (the skill will prompt for this on first load).
...
## Newroz Execution Pattern (desktop app flow)

Newroz terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/newroz-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/newroz-op.sock"
