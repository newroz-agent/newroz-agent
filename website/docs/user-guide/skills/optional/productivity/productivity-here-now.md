...
| | |
|---|---|
| Source | Optional — install with `newroz skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${NEWROZ_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${NEWROZ_SKILL_DIR}/scripts/drive.sh` for private Drive storage
...
| `--client {name}`      | Agent name for attribution (e.g. `newroz`)    |
