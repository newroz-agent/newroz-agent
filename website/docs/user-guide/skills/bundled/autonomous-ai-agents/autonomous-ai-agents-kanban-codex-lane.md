...
description: "Use when a Newroz Kanban worker wants to run Codex CLI as an isolated implementation lane while Newroz keeps ownership of task lifecycle, reconciliation, tes..."
---
...
Use when a Newroz Kanban worker wants to run Codex CLI as an isolated implementation lane while Newroz keeps ownership of task lifecycle, reconciliation, testing, and handoff.
...
- A bounded diff can be evaluated by Newroz in one run.
- The repo can be copied or checked out in an isolated git worktree/branch.
- Newroz can run the relevant tests itself after Codex exits.
...
1. Newroz owns the Kanban lifecycle. Codex must never call `kanban_complete`, `kanban_block`, `kanban_create`, gateway messaging, or any Newroz board CLI as a substitute for the worker.
2. Newroz owns final acceptance. Treat Codex commits/diffs as untrusted patches until reviewed and verified.
3. Newroz owns test execution. Codex may run tests, but those runs are advisory; repeat required verification from Newroz with the repo's canonical wrapper.
4. Newroz owns safety. If Codex changes safety boundaries, risk gates, live trading behavior, or secrets handling, reject the lane even if tests pass.
5. Newroz owns cleanup. Kill stuck Codex processes and remove temporary worktrees when they are no longer needed.
...
```bash
TASK_ID="${NEWROZ_KANBAN_TASK:-t_manual}"
REPO="/path/to/repo"
...
If the current Kanban workspace is already an isolated git worktree created for this task, you may create a sibling Codex branch inside it only if `git status --short` is clean except for intentional Newroz edits. Otherwise create a separate temporary worktree and cherry-pick or copy accepted commits back after reconciliation.
...
Run these before spawning Codex. Missing Codex is a normal reason to skip the lane, not a task blocker if Newroz can do the task directly.
...
Newroz owns the Kanban lifecycle; do not call Newroz kanban tools or messaging.
...
Do not use `--yolo` for prediction-market-bot or safety-sensitive repos. Prefer `--full-auto` inside the isolated worktree, then rely on Newroz reconciliation.
...
- Explicit statement: Newroz owns Kanban lifecycle; Codex is an input lane only.
...
- Verification commands Codex may run and commands Newroz will run afterward.
...
Newroz must perform this checklist before accepting any Codex lane result:

- [ ] `git -C <WORKTREE> status --short --branch` shows only expected files.
- [ ] `git -C <WORKTREE> diff --stat` and `git diff` were reviewed by Newroz.
...
- [ ] Newroz ran the canonical tests itself, using `scripts/run_tests.sh` for Newroz Agent or the repo's documented wrapper for other repos.
- [ ] Any Codex-run tests are listed separately from Newroz-run tests.
- [ ] Accepted commits/diffs were applied to the Newroz-owned workspace/branch.
...
```json
{
  "codex_lane": {
    ...
    "tests_run": [
      {"command": "scripts/run_tests.sh tests/tools/test_x.py", "exit_code": 0, "owner": "newroz"},
...
```json
{
  "codex_lane": {
    ...
    "result": "rejected",
    "accepted_commits": [],
    "rejected_reason": "Direct Newroz edit was smaller and safer than spawning Codex.",
    "tests_run": [],
    "artifacts": []
  }
}
...
1. Treating Codex self-report as verification. Always inspect the diff and rerun tests from Newroz.
2. Running Codex in the user's dirty main checkout. Always isolate in a worktree/branch.
3. Letting Codex own Kanban. Codex may summarize progress, but Newroz writes board state.
...
- [ ] Newroz reviewed `git diff` and safety-sensitive files.
- [ ] Newroz ran canonical tests independently.
