...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`newroz-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-newroz-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Newroz terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Newroz itself, `model.provider: openai-codex` uses Newroz-managed Codex
OAuth from `~/.newroz/auth.json` after `newroz auth add openai-codex`. For the
...
## Newroz Gateway Caveat

When invoking the Codex CLI from a Newroz gateway/service context (for example,
