...
- Give it a name like "newroz-agent"
- Select scopes:
...
- Give it a title like "newroz-agent-&lt;machine-name>"
...
elif _newroz_env="${NEWROZ_HOME:-$HOME/.newroz}/.env"; [ -f "$_newroz_env" ] && grep -q "^GITHUB_TOKEN=" "$_newroz_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_newroz_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
