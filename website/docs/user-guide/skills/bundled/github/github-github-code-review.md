...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _newroz_env="${NEWROZ_HOME:-$HOME/.newroz}/.env"; [ -f "$_newroz_env" ] && grep -q "^GITHUB_TOKEN=" "$_newroz_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_newroz_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
