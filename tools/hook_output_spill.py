...
* Never raises. Any I/O error (disk full, permission denied, missing
  NEWROZ_HOME, etc.) falls back to a byte-length truncation with an
...
def _resolve_spill_dir(directory_override: Optional[str], session_id: Optional[str]) -> Path:
    ...
    if directory_override:
        ...
        try:
            ...
            base = Path(get_newroz_home()) / "hook_outputs"
        except Exception:
            # Last-resort fallback: NEWROZ_HOME env var, then ~/.newroz
            home = os.environ.get("NEWROZ_HOME") or os.path.expanduser("~/.newroz")
