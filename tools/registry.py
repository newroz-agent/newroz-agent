"""Central registry for all newroz-agent tools.
...
# timescales. Cache results for ~30 s so env-var flips via ``newroz tools``
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``newroz tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def _plugin_owner_of(self, handler: Callable) -> Optional[str]:
        ...
        if isinstance(mod, str) and mod.startswith("newroz_plugins."):
            return mod
        return None
    ...
    def deregister(self, name: str) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # (``newroz_plugins.{name}``), not the exact module string.
                # A handler defined in ``newroz_plugins.pkg.handlers`` is
                # still owned by the ``newroz_plugins.pkg`` package — exact
                ...
                if (
                    caller_mod.startswith("newroz_plugins.")
                    and not same_plugin
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``newroz tools enable foo``)
