...
This module provides the glue:
    - ``NewrozTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Newroz Agent"
"""
...
from newroz_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__newroz_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses NEWROZ_HOME so each profile gets its own OAuth tokens.
    Layout: ``NEWROZ_HOME/mcp-tokens/``
    """
    try:
        from newroz_constants import get_newroz_home
        base = Path(get_newroz_home())
    except ImportError:
        base = Path(os.environ.get("NEWROZ_HOME", str(Path.home() / ".newroz")))
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``newroz mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `newroz mcp login <server>` interactively to (re)authorize, "
...
# ---------------------------------------------------------------------------
# NewrozTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class NewrozTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        NEWROZ_HOME/mcp-tokens/<server_name>.json         -- tokens
        NEWROZ_HOME/mcp-tokens/<server_name>.client.json   -- client info
        NEWROZ_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Newroz records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Newroz.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://newroz-agent.nousresearch.com/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `newroz mcp login <server>` later to "
...
def remove_oauth_tokens(server_name: str) -> None:
    ...
    storage = NewrozTokenStorage(server_name)
    storage.remove()
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Newroz Agent")
    scope = cfg.get("scope")
...
def build_oauth_auth(
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`newroz mcp login {server_name}` interactively first to complete "
