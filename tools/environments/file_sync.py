...
_FORCE_SYNC_ENV = "NEWROZ_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.newroz") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.newroz to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``NEWROZ_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, newroz_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.newroz/`` directory as a tar archive,
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="newroz-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.newroz/skills/a.md`` →
        ``~/.newroz/skills/a.md``, a new remote file at
        ``/root/.newroz/skills/b.md`` maps to ``~/.newroz/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
