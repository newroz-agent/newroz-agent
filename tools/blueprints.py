...
`newroz skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.newroz.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.newroz.blueprint`` is a mapping containing
    ...
    newroz = meta.get("newroz") if isinstance(meta, dict) else None
    blueprint = newroz.get("blueprint") if isinstance(newroz, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.newroz.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.newroz.blueprint`` block) they can hand
    to ``newroz skills publish`` to share. ``body`` is the plain-language
