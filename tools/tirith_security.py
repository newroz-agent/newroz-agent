...
it is automatically downloaded from GitHub releases to $NEWROZ_HOME/bin/tirith.
...
def _get_newroz_home() -> str:
    """Return the Newroz home directory, respecting NEWROZ_HOME env var."""
    return str(get_newroz_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_newroz_home(), ".tirith-install-failed")
...
def _newroz_bin_dir() -> str:
    """Return $NEWROZ_HOME/bin, creating it if needed."""
    d = os.path.join(_get_newroz_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $NEWROZ_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_newroz_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $NEWROZ_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $NEWROZ_HOME/bin/tirith
    ...
    newroz_bin = os.path.join(_newroz_bin_dir(), "tirith")
    if os.path.isfile(newroz_bin) and os.access(newroz_bin, os.X_OK):
        _resolved_path = newroz_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return newroz_bin
