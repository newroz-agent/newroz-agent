...
`mcp_tool.py`, and `newroz_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# NewrozMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_newroz_provider_class() -> Optional[type]:
    ...
    class NewrozMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._newroz_server_name = server_name
            ...
            self._newroz_preregistered = preregistered
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`NewrozTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, NewrozTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._newroz_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import NewrozTokenStorage
                        if isinstance(storage, NewrozTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._newroz_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import NewrozTokenStorage
            if not isinstance(storage, NewrozTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``newroz mcp reauth``.
            ...
            back to ``newroz mcp reauth``.
            """
            try:
                if self._newroz_preregistered:
                    return
    ...
    return NewrozMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_NEWROZ_PROVIDER_CLS: Optional[type] = _make_newroz_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`NewrozMCPOAuthProvider` directly using the helpers
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _NEWROZ_PROVIDER_CLS is None:
            logger.warning(
        ...
        if not _is_interactive() and not storage.has_cached_tokens():
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `newroz mcp login "
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``newroz mcp remove <name>`` and (indirectly) by
        ``newroz mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
