...
One ``CDPSupervisor`` runs per Newroz ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Newroz logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "newroz-dialog-bridge.invalid"
...
(() => {
  if (window.__newrozDialogBridgeInstalled) return;
  window.__newrozDialogBridgeInstalled = true;
  const ENDPOINT = "http://newroz-dialog-bridge.invalid/";
