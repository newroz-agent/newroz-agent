# Newroz Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: newroz-achievements plugin
...
### `GET /api/plugins/newroz-achievements/achievements`
...
### `POST /api/plugins/newroz-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/newroz-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/newroz-achievements/overview`
...
- Persist plugin data under:
  - `~/.newroz/plugins/newroz-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.newroz/plugins/newroz-achievements/scan_checkpoint.json`
...
- `~/.newroz/plugins/newroz-achievements/`
