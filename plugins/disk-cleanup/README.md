...
Auto-tracks and cleans up ephemeral files created during Newroz Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$NEWROZ_HOME` and `/tmp/newroz-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `NEWROZ_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `NEWROZ_HOME` or `/tmp/newroz-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$NEWROZ_HOME/disk-cleanup/` is itself excluded
- `$NEWROZ_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
