"""disk_cleanup — ephemeral file cleanup for Newroz Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under NEWROZ_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly NEWROZ_HOME and /tmp/newroz-*
Never touches: ~/.newroz/logs/ or any system directory.
"""
...
try:
    ...
    def get_newroz_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("NEWROZ_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".newroz").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$NEWROZ_HOME/logs/``."""
    return get_newroz_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$NEWROZ_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under NEWROZ_HOME or ``/tmp/newroz-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    newroz_home = get_newroz_home()
    try:
        path.resolve().relative_to(newroz_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/newroz-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("newroz-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "newroz-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $NEWROZ_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so NEWROZ_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        newroz_home = get_newroz_home()
        for parent in ("cron", "cronjobs"):
            base = newroz_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside NEWROZ_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under NEWROZ_HOME, but never recurse into known
    # durable state trees.  Some installs place the Newroz checkout, venv,
    # and desktop build under NEWROZ_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    newroz_home = get_newroz_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in newroz_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    newroz_home = get_newroz_home()
    try:
        rel = path.resolve().relative_to(newroz_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "newroz-agent",
        }:
            return None
        ...
        # Path isn't under NEWROZ_HOME (e.g. /tmp/newroz-*) — fall through.
        pass
