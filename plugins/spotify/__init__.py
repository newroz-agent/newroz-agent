...
``_check_spotify_available()`` — when the user has not run ``newroz auth
spotify``, the tools remain registered (so they appear in ``newroz
...
The Spotify auth flow (``newroz auth spotify``), CLI plumbing, and docs
