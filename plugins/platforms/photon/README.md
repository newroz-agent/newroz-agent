...
This plugin connects Newroz Agent to iMessage (and other Spectrum
...
TypeScript-only, Newroz runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-Newroz-Sidecar-Token`.
...
There is no separate `login` command; like every other Newroz channel,
...
Run `newroz photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.newroz/.env` (the same place every other
...
Management metadata lives in `~/.newroz/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `newroz photon telemetry on\|off` (restart the gateway to apply) |
...
6. Verify end-to-end: `newroz photon status`, a DM and a group roundtrip,
