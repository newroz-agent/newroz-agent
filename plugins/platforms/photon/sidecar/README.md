...
Small Node helper that bridges Newroz Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Newroz is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Newroz-Sidecar-Token`)
...
The Newroz plugin's `newroz photon setup` command runs `npm install`
here automatically.
