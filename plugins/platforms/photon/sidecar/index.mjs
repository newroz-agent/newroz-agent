// Newroz Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Newroz): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Newroz -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Newroz-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `newroz photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Newroz can restart the Photon adapter"
    );
    process.exit(75);
...
// instead of a cryptic module-resolution error during import. Apply Newroz'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Newroz allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Newroz, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-newroz-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Newroz supplied them so a known-good
