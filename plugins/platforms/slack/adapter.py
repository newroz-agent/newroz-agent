...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `newroz slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        tokens_file = get_newroz_home() / "slack_tokens.json"
        ...
        try:
            ...
            # Newroz does not currently need to route them into the agent loop.
            ...
            # /model work directly without /newroz prefix). A single regex
            ...
            # manifest (see `newroz slack manifest`). In Socket Mode, Slack
            ...
            from newroz_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/newroz$")

            @self._app.command(_slash_pattern)
            async def handle_newroz_command(ack, command):
            ...
            for _action_id in (
                "newroz_approve_once",
                "newroz_approve_session",
                "newroz_approve_always",
                "newroz_deny",
            ):
            ...
            for _action_id in (
                "newroz_confirm_once",
                "newroz_confirm_always",
                "newroz_confirm_cancel",
            ):
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Newroz handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Newroz session — matching the per-thread behavior channels already
    ...
    async def _handle_slack_message(self, event: dict) -> None:
        ...
        if original_text.startswith("!"):
            try:
                ...
                # forms like ``!stop@newroz`` still resolve.
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "newroz_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "newroz_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "newroz_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "newroz_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "newroz_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "newroz_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "newroz_confirm_once": "once",
            "newroz_confirm_always": "always",
            "newroz_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "newroz_approve_once": "once",
            "newroz_approve_session": "session",
            "newroz_approve_always": "always",
            "newroz_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/newroz <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/newroz
        ...
        if slash_name in {"newroz", ""}:
            # Legacy /newroz <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from newroz_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/newroz   ``).
        ...
        # "Running /cmd…" ack shown by handle_newroz_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/newroz <question>" must produce public replies so
...
# ``newroz_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``newroz_cli/setup.py::_setup_slack``.
    """
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under NEWROZ_HOME, and print
        paste-into-Slack instructions. Failures are non-fatal."""
        try:
            from newroz_cli.slack_cli import _build_full_manifest
            from newroz_constants import get_newroz_home
            import json as _json

            manifest = _build_full_manifest(
                bot_name="Newroz",
                bot_description="Your Newroz agent on Slack",
            )
            target = Path(get_newroz_home()) / "slack-manifest.json"
            ...
            print_info(
                "   Re-run `newroz slack manifest --write` anytime to refresh after "
                "Newroz adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `newroz update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://newroz-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``newroz_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'newroz-agent[slack]'",
        # Interactive setup wizard — replaces newroz_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in newroz_cli/gateway.py.
