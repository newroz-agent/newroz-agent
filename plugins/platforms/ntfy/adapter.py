"""ntfy platform adapter (Newroz plugin).
...
``plugins/platforms/ntfy/``. The Newroz plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "newroz-in"              # subscribe topic (incoming)
          publish_topic: "newroz-out"     # optional — defaults to topic
...
_ECHO_TAG = "newroz-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``newroz cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Newroz dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `newroz gateway status` without
