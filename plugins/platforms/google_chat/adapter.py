...
# ``model_tools`` import time, so ``newroz status``, ``newroz chat``, etc.
...
# ``newroz_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Newroz is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a newroz MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${NEWROZ_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _newroz_home = _get_newroz_home()
        except (ModuleNotFoundError, ImportError):
            _newroz_home = _Path.home() / ".newroz"
        self._thread_count_store = _ThreadCountStore(
            _newroz_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Newroz is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("NEWROZ_HOME", str(_Path.home() / ".newroz"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${NEWROZ_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a newroz MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Newroz is thinking…' marker message.
        ...
        "Newroz is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Newroz is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Newroz is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Newroz is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Newroz is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``newroz setup``.

    The setup wizard at ``newroz_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.newroz/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. newroz-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.newroz/.env")
    print_info("Restart the gateway: newroz gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``newroz cron`` running as a
    separate process from ``newroz gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Newroz plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'newroz-agent[google_chat]'",
        ...
        # the "Newroz is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Newroz is thinking…' marker message "
