"""
Microsoft Teams platform adapter for Newroz Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        newroz_action = data.get("newroz_action", "")
        session_key = data.get("session_key", "")

        if not newroz_action or not session_key:
        ...
        choice = choice_map.get(newroz_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="newroz_approve",
                    data={**btn_data_base, "newroz_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="newroz_approve",
                    data={**btn_data_base, "newroz_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="newroz_approve",
                    data={**btn_data_base, "newroz_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="newroz_approve",
                    data={**btn_data_base, "newroz_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Newroz\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.newroz/.env")
