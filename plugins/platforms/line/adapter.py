"""
LINE Messaging API platform adapter for Newroz Agent.
...
to Newroz Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``NEWROZ_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            newroz_home = Path(get_newroz_home()).resolve()
        except Exception:
            newroz_home = Path.home().joinpath(".newroz").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``newroz status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``newroz status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``newroz setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.newroz/.env`` via ``newroz_cli.config``.
    """
    print()
    ...
    try:
        from newroz_cli.config import get_env_var, set_env_var
    except ImportError:
        print("newroz_cli.config not available; set LINE_* vars manually in ~/.newroz/.env")
        return
