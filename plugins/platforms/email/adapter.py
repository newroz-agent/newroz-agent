"""
Email platform adapter for the Newroz gateway.

Allows users to interact with Newroz by sending emails.
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `newroz config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `newroz gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Newroz Agent")
        ...
        msg_id = f"<newroz-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# newroz_cli/gateway.py, and the _send_email dispatch in
