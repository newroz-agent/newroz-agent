...
``{title, url, description, position}`` rows every other Newroz web
provider produces.
...
prefers Newroz-managed xAI Grok OAuth (via ``newroz auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.newroz/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``newroz tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `newroz auth` to sign in with "
