...
Let the newroz agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where newroz runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `newroz meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `newroz meet` CLI |
...
| `cli.py` | `newroz meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$NEWROZ_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `newroz meet node {run,list,approve,remove,status,ping}` |
...
newroz meet install                                      # pip + Chromium
newroz meet setup                                        # preflight
newroz meet auth                                         # optional
newroz meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
newroz meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.newroz/.env
newroz meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
newroz meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
newroz meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.newroz/.env
newroz meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, newroz will **not** switch your system audio input automatically — the
...
newroz plugins enable google_meet
newroz meet node run --display-name my-mac --host 0.0.0.0 --port 18789
