...
session-scoped state file under ``$NEWROZ_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $NEWROZ_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_newroz_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this newroz install, leave it first —
    ...
    env = os.environ.copy()
    env["NEWROZ_MEET_URL"] = url
    env["NEWROZ_MEET_OUT_DIR"] = str(out)
    env["NEWROZ_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["NEWROZ_MEET_HEADED"] = "1"
    if auth_state:
        env["NEWROZ_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["NEWROZ_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if NEWROZ_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["NEWROZ_MEET_MODE"] = mode
    if realtime_model:
        env["NEWROZ_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["NEWROZ_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["NEWROZ_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["NEWROZ_MEET_REALTIME_KEY"] = realtime_api_key
