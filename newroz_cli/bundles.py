"""Implementation of the ``newroz bundles`` CLI subcommand.

Mirrors the structure of ``newroz_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `newroz bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  newroz bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``newroz bundles`` argparse tree.

    Called from ``newroz_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `newroz bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``newroz bundles <subcommand>`` to the right handler."""
