...
Usage:
    newroz pairing list              # Show all pending + approved users
    newroz pairing approve <platform> <code>  # Approve a pairing code
    newroz pairing revoke <platform> <user_id> # Revoke user access
    newroz pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle newroz pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: newroz pairing {list|approve|revoke|clear-pending}")
        print("Run 'newroz pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.newroz/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'newroz pairing list' to see pending codes.\n")
