"""Slash command definitions and autocomplete for the Newroz CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Newroz config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Newroz works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Newroz is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.newroz/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Newroz Agent to the latest version", "Info"),
    CommandDef("version", "Show Newroz Agent version", "Info", aliases=("v",)),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`newroz_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/newroz`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. Newroz ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Newroz
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``newroz skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``newroz skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``newroz skills list`` and
...
# unrelated command landed. These claim their slots right after /newroz,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /newroz <command>).
...
# native slot, the alias spelling stays reachable via /newroz reset).
...
# ``/newroz <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/newroz" decision, not a silent clamp.
#   - credits: the billing/top-up surface; reached via /newroz credits on Slack.
#   - billing: the terminal-billing surface (buy/auto-reload/limit); /newroz billing.
#   - moa: high-cost slash mode, available through /newroz moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /newroz debug on Slack.
_SLACK_VIA_NEWROZ_ONLY = frozenset({"credits", "billing", "moa", "debug"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/newroz <verb>`` subcommand.
    ...
    can still reach them via ``/newroz <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/newroz`` is always reserved as the first entry so the
    legacy ``/newroz <subcommand>`` form keeps working for anything that
    ...
    # Reserve /newroz as the catch-all top-level command.
    entries.append(("newroz", "Talk to Newroz or run a subcommand", "[subcommand] [args]"))
    seen.add("newroz")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_NEWROZ_ONLY:
            # Intentionally Slack-via-/newroz only (see _SLACK_VIA_NEWROZ_ONLY).
            return
    ...
    # everything except /newroz, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://newroz-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /newroz handler.

    Maps both canonical names and aliases so /newroz bg do stuff works
    the same as /newroz background do stuff.

    Plugin-registered slash commands are included so ``/newroz <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from newroz_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
