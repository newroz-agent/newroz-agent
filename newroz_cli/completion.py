"""Shell completion script generation for newroz CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_newroz_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Newroz Agent bash completion
# Add to ~/.bashrc:
#   eval "$(newroz completion bash)"

_newroz_profiles() {{
    local profiles_dir="$HOME/.newroz/profiles"
...
_newroz_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_newroz_profiles)" -- "$cur"))
        return
    fi
...
complete -F _newroz_completion newroz
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef newroz
# Newroz Agent zsh completion
# Add to ~/.zshrc:
...
_newroz_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.newroz/profiles" ]]; then
        profiles+=($HOME/.newroz/profiles/*(N/:t))
    fi
...
_newroz() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_newroz_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'newroz command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Newroz Agent fish completion",
        "# Add to your config:",
        "#   newroz completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __newroz_profiles",
        "    echo default",
        "    if test -d $HOME/.newroz/profiles",
        "        for d in $HOME/.newroz/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c newroz -f -s p -l profile"
        " -d 'Profile name' -xa '(__newroz_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__newroz_profiles)' -d 'Profile name'"
                )
