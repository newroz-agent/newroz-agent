...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or newroz -c.",
    ...
    "Ctrl+Z suspends Newroz to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "newroz -c resumes your most recent CLI session. newroz -c \"project name\" resumes by title.",
    "newroz -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "newroz -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "newroz chat -t web,terminal enables only specific toolsets for a focused session.",
    "newroz chat -s github-pr-workflow preloads a skill at launch.",
    "newroz chat -q \"query\" runs a single non-interactive query and exits.",
    "newroz chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "newroz chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "newroz --yolo bypasses all dangerous command approval prompts for the entire session.",
    "newroz chat --source telegram tags the session for filtering in newroz sessions list.",
    "newroz -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "newroz doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "newroz dump outputs a compact setup summary — great for bug reports.",
    "newroz config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "newroz config edit opens config.yaml in your default editor.",
    "newroz config check scans for missing or stale configuration options.",
    "newroz sessions browse opens an interactive session picker with search.",
    "newroz sessions stats shows session counts by platform and database size.",
    "newroz sessions prune --older-than 30 cleans up old sessions.",
    "newroz skills search react --source skills-sh searches the skills.sh public directory.",
    "newroz skills check scans installed hub skills for upstream updates.",
    "newroz skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "newroz skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "newroz mcp add github --command npx adds MCP servers from the command line.",
    "newroz mcp serve runs Newroz itself as an MCP server for other agents.",
    "newroz auth add lets you add multiple API keys for credential pool rotation.",
    "newroz completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "newroz logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "newroz backup creates a zip backup of your entire Newroz home directory.",
    "newroz profile create coder creates an isolated profile that becomes its own command.",
    "newroz profile create work --clone copies your current config and keys to a new profile.",
    "newroz update syncs new bundled skills to ALL profiles automatically.",
    "newroz gateway install sets up Newroz as a system service (systemd/launchd).",
    "newroz memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "newroz webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: newroz tools disables unused tools, newroz skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "newroz models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as newroz -w).",
    ...
    "NEWROZ_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Newroz tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'newroz profile create coder' creates the 'coder' command.",
    "newroz profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Newroz prints a resume command with session ID and stats.",
    "newroz sessions export backup.jsonl exports all sessions for backup or analysis.",
    "newroz -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "newroz skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .newroz/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: newroz cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Newroz runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "newroz gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Newroz via a local macOS server.",
    ...
    "Context files (.newroz.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Newroz loads project context from .newroz.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "newroz mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "newroz mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "newroz mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — newroz tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.newroz/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "newroz chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "newroz plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "newroz acp runs Newroz as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "NEWROZ_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "newroz auth add nous or newroz auth add openai-codex sets up OAuth-based providers.",
    ...
    "newroz status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.newroz/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Newroz your own.",
    ...
    "Before a gateway session reset, Newroz auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.newroz/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Profiles scope Newroz state via NEWROZ_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "NEWROZ_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in NEWROZ_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.newroz/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'newroz profile create ls' would be rejected.",
    "newroz profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "newroz claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "NEWROZ_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via newroz tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.newroz/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.newroz/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Newroz auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Newroz config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Newroz is working.',
    ...
    '/reload reloads ~/.newroz/.env into the running session — pick up new API keys without restarting.',
    ...
    "NEWROZ_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.newroz/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.newroz/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'newroz curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "newroz curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'newroz curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'newroz auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'NEWROZ_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "NEWROZ_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'newroz dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'newroz dashboard embeds the full Newroz TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.newroz/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.newroz/dashboard-plugins/ — no npm build required.',
    ...
    'NEWROZ_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'NEWROZ_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'NEWROZ_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'NEWROZ_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.newroz/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.newroz/skills/ so drop-in skills appear without restarting the session.',
    ...
    'newroz -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'newroz chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'newroz chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'newroz chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "newroz chat --source tool tags programmatic chats so they don't clutter newroz sessions list.",
    'newroz dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'newroz sessions rename <ID> "new title" renames any past session; newroz sessions delete <ID> removes one.',
    'newroz import restores a session export or profile archive produced by sessions export or profile export.',
    'newroz fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'newroz pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'newroz setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'newroz status --deep runs the full health sweep across every component; plain newroz status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'NEWROZ_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'NEWROZ_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.newroz/plugins/ — trust-gated by design.',
    "NEWROZ_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'NEWROZ_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'NEWROZ_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'NEWROZ_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'NEWROZ_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'NEWROZ_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'NEWROZ_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'NEWROZ_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'NEWROZ_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'NEWROZ_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'NEWROZ_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Newroz block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from newroz sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.newroz/dashboard-plugins/.',
]
