"""newroz hooks — inspect and manage shell-script hooks.

Usage::

    newroz hooks list
    newroz hooks test <event> [--for-tool X] [--payload-file F]
    newroz hooks revoke <command>
    newroz hooks doctor

Consent records live under ``~/.newroz/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.newroz/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``newroz hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: newroz hooks {list|test|revoke|doctor}")
        print("Run 'newroz hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.newroz/config.yaml.")
        print("See `newroz hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `newroz hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `newroz hooks test` and `newroz hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Newroz wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `newroz hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `newroz hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `newroz hooks doctor`.")
