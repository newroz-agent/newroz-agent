"""`newroz checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.newroz/checkpoints/``.  Actions:

    newroz checkpoints               # same as `status`
    newroz checkpoints status        # total size, project count, breakdown
    newroz checkpoints list          # per-project checkpoint counts + workdir
    newroz checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    newroz checkpoints clear [-f]    # nuke the entire base (asks first)
    newroz checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    newroz checkpoints
    newroz checkpoints prune --retention-days 3 --max-size-mb 200
    newroz checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: newroz checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``newroz checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `newroz checkpoints` → status
