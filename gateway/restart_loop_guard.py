...
Defenses 1 and 2 (the ``_NEWROZ_GATEWAY`` guard on ``newroz gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.newroz.gateway")``,
...
State lives in ``<NEWROZ_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_newroz_home() / "gateway" / "restart_loop.json"
