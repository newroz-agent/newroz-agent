...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``newroz setup`` display).
    ...
    # built-ins).  Used by ``newroz gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `newroz`
        # invocation -- including plain `newroz chat`, which never touches any
        ...
        # `newroz setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``newroz setup``/``gateway status``, channel
