...
There are two ways to add a platform to the Newroz gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.newroz/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Newroz code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `newroz gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `newroz_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Newroz core.
...
And add it to the `newroz-gateway` composite:

```python
"newroz-gateway": {
    "includes": [..., "newroz-your-platform"]
}
```
...
## 12. Status Display (`newroz_cli/status.py`)
...
## 13. Gateway Setup Wizard (`newroz_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ newroz_cli/ toolsets.py \
  --include="*.py" -l | sort -u
