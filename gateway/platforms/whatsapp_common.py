...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *Newroz Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to NEWROZ_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/newroz), this function
    mirrors the bridge source to a writable NEWROZ_HOME location and returns that
    ...
    # Try NEWROZ_HOME location first
    newroz_home = get_newroz_home()
    newroz_home_bridge = newroz_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to NEWROZ_HOME if needed
    if newroz_home_bridge.exists():
        return newroz_home_bridge

    # Mirror the bridge source to NEWROZ_HOME
    try:
        newroz_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            newroz_home_bridge,
            dirs_exist_ok=False,
