...
Contract (presence-based, mirroring ``.restart_notify.json``):
  ...
  * begin-drain  → write ``{NEWROZ_HOME}/.drain_request.json`` with
...
Why the epoch (NS-570). ``NEWROZ_HOME`` is a **durable** store — on Newroz
...
def current_instantiation_epoch() -> str:
    ...
    Together they discriminate every restart mode that matters:
      ...
      | host ``newroz gateway restart``| same    | same(init) | SAME   | honour |
...
def drain_request_path(home: Optional[Path] = None) -> Path:
    """Absolute path to the drain-request marker, respecting NEWROZ_HOME."""
    base = home if home is not None else get_newroz_home()
...
def write_drain_request(
    *,
    ...
    later survives a machine restart on the durable NEWROZ_HOME volume can be
...
def drain_requested(*, home: Optional[Path] = None) -> bool:
    ...
    treated as absent: it survived a container/VM restart (NEWROZ_HOME is a
    durable Fly volume on Newroz Cloud) and the lifecycle action that triggered
...
def drain_notification_suppressed(*, home: Optional[Path] = None) -> bool:
    ...
    marker that survived a machine restart on the durable NEWROZ_HOME volume is
