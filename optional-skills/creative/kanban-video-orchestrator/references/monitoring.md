...
newroz kanban watch --tenant <project-slug>

# Snapshot of the board
newroz kanban list --tenant <project-slug>
newroz kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
newroz kanban stats --tenant <project-slug>

# Visual dashboard (browser)
newroz dashboard

# Inspect a specific task (includes comments + events)
newroz kanban show <task-id>

# Follow a single task's event stream
newroz kanban tail <task-id>
```

Verify available subcommands with `newroz kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `newroz kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `newroz kanban block <id>` then `newroz kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `newroz kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `newroz kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `newroz profile list` (profile exists?), `newroz status` (gateway/dispatcher up?), and verify tenant. |
...
newroz kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
newroz kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(newroz kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
newroz kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
newroz kanban block <task-id>
newroz kanban archive <task-id>

# Diagnose what happened
newroz kanban show <task-id>      # task body, comments, recent events
newroz kanban tail <task-id>      # follow the live event stream
newroz kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `newroz kanban create` for the director
...
while true; do
    clear
    newroz kanban list --tenant <slug>
    echo "---"
    newroz kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `newroz kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `newroz profile list` and start
