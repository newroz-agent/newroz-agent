...
the actual `setup.sh` that creates the project workspace, configures Newroz
...
2. **Create profiles** — `newroz profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.newroz/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `newroz kanban create` assigned to the director
...
```bash
newroz profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.newroz/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.newroz/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Newroz
...
Each profile gets a `SOUL.md` at `~/.newroz/profiles/<name>/SOUL.md` that
...
```bash
newroz kanban create "Direct production of <video title>" \
    --assignee director \
...
the Newroz `.env` (`${NEWROZ_HOME:-$HOME/.newroz}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _newroz_env="${NEWROZ_HOME:-$HOME/.newroz}/.env"
    if grep -q "^${var}=" "$_newroz_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_newroz_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_newroz_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY newroz ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY newroz OPENROUTER_API_KEY || exit 1
# ...
```
