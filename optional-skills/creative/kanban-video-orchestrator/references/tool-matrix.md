...
Maps each role archetype to the Newroz skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public newroz-agent
...
## Newroz skills relevant to video production

### Visual / rendering skills (`newroz-agent/skills/creative/`)
...
### Design / pre-production skills (`newroz-agent/skills/creative/`)
...
### Audio / media skills (`newroz-agent/skills/creative/` + `skills/media/`)
...
These are **not** Newroz skills but external CLIs / APIs that profiles invoke.
...
## Built-in Newroz tools for media review

These are native Newroz tools — not invoked via terminal but through their own
...
key is present in `${NEWROZ_HOME:-~/.newroz}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Newroz profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${NEWROZ_HOME:-~/.newroz}/.env` → environment variable.
...
When a new Newroz-public video skill ships:
