...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("NEWROZ_HOME", os.path.expanduser("~/.newroz")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("NEWROZ_HOME", Path.home() / ".newroz")) / "skills" / "red-teaming" / "godmode"
...
NEWROZ_HOME = Path(os.getenv("NEWROZ_HOME", Path.home() / ".newroz"))
CONFIG_PATH = NEWROZ_HOME / "config.yaml"
PREFILL_PATH = NEWROZ_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Newroz models — already uncensored, just needs clean prompt
    "newroz": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "newroz" in model_lower or "nous" in model_lower:
        return "newroz"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Newroz config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.newroz/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Newroz for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Newroz for changes to take effect.")
