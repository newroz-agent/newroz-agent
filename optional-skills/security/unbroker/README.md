...
The design is **Newroz-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
newroz skills install official/security/unbroker
```

Then start a new Newroz session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Newroz session:
...
- **PII handling.** Dossiers live under the Newroz home directory (`0600`, optionally
