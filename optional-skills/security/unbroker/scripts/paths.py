...
All per-subject data lives under PDD_DATA_DIR (default: $NEWROZ_HOME/unbroker),
which is the same trust boundary Newroz uses for .env and OAuth tokens.
"""
...
def newroz_home() -> Path:
    return Path(os.environ.get("NEWROZ_HOME") or (Path.home() / ".newroz"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else newroz_home() / "unbroker"
