#!/usr/bin/env python3
"""Canvas LMS API CLI for Newroz Agent.
...
def _check_config():
    ...
    if missing:
        newroz_env = os.path.join(
            os.environ.get("NEWROZ_HOME", os.path.expanduser("~/.newroz")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {newroz_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Newroz Agent"
    )
