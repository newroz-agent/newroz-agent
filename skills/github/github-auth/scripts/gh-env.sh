#!/usr/bin/env bash
# GitHub environment detection helper for Newroz Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _newroz_env="${NEWROZ_HOME:-$HOME/.newroz}/.env"; [ -f "$_newroz_env" ] && grep -q "^GITHUB_TOKEN=" "$_newroz_env" 2>/dev/null; then
