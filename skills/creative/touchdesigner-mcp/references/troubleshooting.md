...
## 5. Newroz Configuration

### Config location

`$NEWROZ_HOME/config.yaml` (defaults to `~/.newroz/config.yaml` when `NEWROZ_HOME` is unset)
...
Restart the Newroz session for changes to take effect. The MCP connection is
