#!/usr/bin/env python3
"""Google Workspace API CLI for Newroz Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Newroz-facing JSON contract and falls back to the Python client
...
from _newroz_home import get_newroz_home
...
def _gws_binary() -> str | None:
    override = os.getenv("NEWROZ_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Newroz Agent")
