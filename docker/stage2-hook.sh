...
#
# Wired into the image as /etc/cont-init.d/01-newroz-setup by the
...
NEWROZ_HOME="${NEWROZ_HOME:-/opt/data}"
INSTALL_DIR="/opt/newroz"

# Drop to newroz via s6-setuidgid, but skip it when already non-root.
as_newroz() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid newroz "$@"; }
...
# starts non-root. The baked install tree under /opt/newroz is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $NEWROZ_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass NEWROZ_UID/NEWROZ_GID — or the PUID/PGID aliases — which the
...
# `id -u newroz` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# newroz UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u newroz)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-newroz UID).
...
and the baked /opt/newroz install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e NEWROZ_UID=\$(id -u) -e NEWROZ_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the newroz user to that UID/GID at boot and chowns the data
...
# --- Bootstrap NEWROZ_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid newroz mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom NEWROZ_HOME paths whose parents only
# root can create (e.g. `NEWROZ_HOME=/home/newroz/.newroz` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for NEWROZ_UID/NEWROZ_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  NEWROZ_UID/
# NEWROZ_GID still win when both are set.  See #15290, salvages #25872.
NEWROZ_UID="${NEWROZ_UID:-${PUID:-}}"
NEWROZ_GID="${NEWROZ_GID:-${PGID:-}}"

if [ -n "${NEWROZ_UID:-}" ] && validate_uid_gid "$NEWROZ_UID" && [ "$NEWROZ_UID" != "$(id -u newroz)" ]; then
    echo "[stage2] Changing newroz UID to $NEWROZ_UID"
    usermod -u "$NEWROZ_UID" newroz
fi
if [ -n "${NEWROZ_GID:-}" ] && validate_uid_gid "$NEWROZ_GID" && [ "$NEWROZ_GID" != "$(id -g newroz)" ]; then
    echo "[stage2] Changing newroz GID to $NEWROZ_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$NEWROZ_GID" newroz 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised newroz user (UID 10000)
...
# newroz process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes newroz. Idempotent across container
...
#   - newroz is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G newroz 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] newroz already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" newroz 2>/dev/null; then
        echo "[stage2] Added newroz to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group newroz failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When NEWROZ_UID is remapped or the top-level $NEWROZ_HOME isn't owned by
# the runtime newroz UID, restore ownership to newroz — but ONLY for the
# directories newroz actually writes to. The full $NEWROZ_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of newroz-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_newroz_uid=$(id -u newroz)
...
chown_newroz_tree() {
    ...
    chown -R newroz:newroz "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

needs_chown=false
if [ "$(stat -c %u "$NEWROZ_HOME" 2>/dev/null)" != "$actual_newroz_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $NEWROZ_HOME (targeted) to newroz ($actual_newroz_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $NEWROZ_HOME: chown the directory itself (not its contents)
    # so newroz can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$NEWROZ_HOME"; then
        :
    else
        chown newroz:newroz "$NEWROZ_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $NEWROZ_HOME failed (rootless container?) — continuing"
    fi
    # Newroz-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by newroz (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$NEWROZ_HOME/$sub" ]; then
            chown_newroz_tree "$NEWROZ_HOME/$sub"
        fi
    done
fi
...
# newroz user. Hosted/container instances keep mutable state under
# $NEWROZ_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# NEWROZ_DISABLE_LAZY_INSTALLS=1. Keeping /opt/newroz root-owned and
...
# $NEWROZ_HOME/lazy-packages dir on the data volume (Dockerfile sets
# NEWROZ_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to newroz
...
# Always reset ownership of $NEWROZ_HOME/profiles to newroz on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> newroz …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as newroz and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$NEWROZ_HOME/profiles" ]; then
    chown_newroz_tree "$NEWROZ_HOME/profiles"
fi

# Always reset ownership of $NEWROZ_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged newroz runtime even
# after root-context maintenance commands or scheduler writes.
if [ -d "$NEWROZ_HOME/cron" ]; then
    chown_newroz_tree "$NEWROZ_HOME/cron"
fi
...
# newroz pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged newroz gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $NEWROZ_HOME is
...
# is negligible.
if [ -d "$NEWROZ_HOME/platforms/pairing" ]; then
    chown_newroz_tree "$NEWROZ_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$NEWROZ_HOME/pairing" ]; then
    chown_newroz_tree "$NEWROZ_HOME/pairing"
fi

# Reset ownership of newroz-owned top-level state files on every boot.
# The targeted data-volume chown above only covers newroz-owned
# *subdirectories*; loose state files living directly under $NEWROZ_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> newroz …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged newroz runtime then hits
...
# sweep so host-owned files in a bind-mounted $NEWROZ_HOME are never
...
# entries of newroz_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$NEWROZ_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$NEWROZ_HOME/$f"; then
            :
        else
            chown newroz:newroz "$NEWROZ_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the newroz runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$NEWROZ_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$NEWROZ_HOME/config.yaml"; then
        :
    else
        chown newroz:newroz "$NEWROZ_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$NEWROZ_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as newroz user ---
# Run as newroz via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $NEWROZ_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_newroz mkdir -p \
    "$NEWROZ_HOME/backups" \
    "$NEWROZ_HOME/cron" \
    "$NEWROZ_HOME/sessions" \
    "$NEWROZ_HOME/logs" \
    "$NEWROZ_HOME/logs/gateways" \
    "$NEWROZ_HOME/hooks" \
    "$NEWROZ_HOME/memories" \
    "$NEWROZ_HOME/skills" \
    "$NEWROZ_HOME/skins" \
    "$NEWROZ_HOME/plans" \
    "$NEWROZ_HOME/workspace" \
    "$NEWROZ_HOME/home" \
    "$NEWROZ_HOME/pairing" \
    "$NEWROZ_HOME/platforms/pairing" \
    "$NEWROZ_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/newroz/.install_method (see Dockerfile), NOT written here into
# $NEWROZ_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $NEWROZ_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.newroz:/opt/data) and sometimes shared with a
...
# 'newroz update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $NEWROZ_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/newroz stamp, so deleting the data-dir copy is safe).
if [ -f "$NEWROZ_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$NEWROZ_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$NEWROZ_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$NEWROZ_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$NEWROZ_HOME/$dest"; then
            :
        else
            as_newroz cp "$INSTALL_DIR/$src" "$NEWROZ_HOME/$dest"
        fi
    fi
}
...
if [ -f "$NEWROZ_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$NEWROZ_HOME/.env"; then
        :
    else
        chown newroz:newroz "$NEWROZ_HOME/.env" 2>/dev/null || true
        chmod 600 "$NEWROZ_HOME/.env" 2>/dev/null || true
    fi
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $NEWROZ_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `newroz update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set NEWROZ_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$NEWROZ_HOME/config.yaml" ]; then
    s6-setuidgid newroz "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$NEWROZ_HOME/auth.json" ] && [ -n "${NEWROZ_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$NEWROZ_HOME/auth.json"; then
        :
    else
        printf '%s' "$NEWROZ_AUTH_JSON_BOOTSTRAP" > "$NEWROZ_HOME/auth.json"
        chown newroz:newroz "$NEWROZ_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$NEWROZ_HOME/auth.json"
    fi
fi
...
# can set NEWROZ_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# NEWROZ_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$NEWROZ_HOME/gateway_state.json" ] && \
        [ "${NEWROZ_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$NEWROZ_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$NEWROZ_HOME/gateway_state.json"
        chown newroz:newroz "$NEWROZ_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$NEWROZ_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_newroz "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/newroz/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Newroz spawns for the
...
# main-wrapper.sh propagates it into the supervised ``newroz`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-newroz, dashboard, etc.).
