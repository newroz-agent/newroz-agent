...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with newroz-agent)
2. User plugins: ``$NEWROZ_HOME/plugins/model-providers/<name>/``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$NEWROZ_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$NEWROZ_HOME/plugins/model-providers/`` if it exists."""
    try:
        from newroz_constants import get_newroz_home

        d = get_newroz_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple NEWROZ_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_newroz_user_provider_{safe_name}"
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$NEWROZ_HOME/plugins/model-providers/<name>/``
    ...
    # 1. Bundled plugins — shipped with newroz-agent.
    ...
    # 2. User plugins — under $NEWROZ_HOME/plugins/model-providers/<name>/.
