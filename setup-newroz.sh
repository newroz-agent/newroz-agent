...
# 5. Symlinks the 'newroz' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Newroz Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/newroz-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink newroz into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up newroz command..."

NEWROZ_BIN="$SCRIPT_DIR/venv/bin/newroz"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$NEWROZ_BIN" "$COMMAND_LINK_DIR/newroz"
echo -e "${GREEN}✓${NC} Symlinked newroz → $COMMAND_LINK_DISPLAY_DIR/newroz"
...
# ============================================================================
# Seed bundled skills into ~/.newroz/skills/
# ============================================================================

NEWROZ_SKILLS_DIR="${NEWROZ_HOME:-$HOME/.newroz}/skills"
mkdir -p "$NEWROZ_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.newroz/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$NEWROZ_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  newroz status        # Check configuration"
if is_termux; then
    echo "  newroz gateway       # Run gateway in foreground"
else
    echo "  newroz gateway install # Install gateway service (messaging + cron)"
fi
echo "  newroz cron list     # View scheduled jobs"
echo "  newroz doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m newroz_cli.main setup
fi
