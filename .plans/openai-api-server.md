# OpenAI-Compatible API Server for Newroz Agent
...
SSE streaming. By exposing this endpoint, newroz-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  newroz-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.newroz/.env`
2. Run `newroz gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with newroz-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns newroz-agent as a model) |
...
Authorization: Bearer newroz-api-key-here
...
But newroz-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "newroz-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured newroz-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of newroz-agent's core prompt
...
Once implemented, newroz-agent works as a drop-in backend for:
